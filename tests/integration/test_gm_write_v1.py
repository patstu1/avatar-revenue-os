...
async def test_gm_approve_approval_writes_audit_and_emits_event(
    ...
    approval = GMApproval(
        org_id=org_id, action_type="test_high_value_deal",
        entity_type="proposal", entity_id=uuid.uuid4(),
        title="Test approval",
        description="needs operator sign-off",
        risk_level="high", status="pending", confidence=0.82,
    )
...
async def test_gm_reject_approval_transitions_status(api_client, db_session, sample_org_data):
    headers, org_id = await _auth(api_client, sample_org_data)
    approval = GMApproval(
        org_id=org_id, action_type="test_reject",
        entity_type="proposal", entity_id=uuid.uuid4(),
        title="Test reject approval", status="pending", risk_level="low",
    )
    db_session.add(approval)
    await db_session.commit()
    ...
    r = await api_client.post(
        ...
        headers=headers, json={"notes": "not worth it"},
    )
...
async def test_gm_approve_approval_requires_auth(api_client, db_session, sample_org_data):
    headers, org_id = await _auth(api_client, sample_org_data)
    approval = GMApproval(
        org_id=org_id, action_type="test_auth", entity_type="proposal",
        entity_id=uuid.uuid4(), title="x", status="pending", risk_level="low",
    )
    db_session.add(approval)
    await db_session.commit()
...
async def test_gm_resolve_escalation_emits_event(api_client, db_session, sample_org_data):
    headers, org_id = await _auth(api_client, sample_org_data)
    esc = GMEscalation(
        org_id=org_id, entity_type="proposal", entity_id=uuid.uuid4(),
        reason_code="test_reason", title="Test escalation",
        description="need operator decision", status="open", severity="warning",
    ...
    r = await api_client.post(
        ...
        headers=headers, json={"notes": "resolved in gm write test"},
    )
    assert r.status_code == 200, r.text
...
async def _seed_draft(db_session, org_id: uuid.UUID, status: str = "pending") -> EmailReplyDraft:
    ...
    inbox = InboxConnection(
        org_id=org_id, email_address=f"r-{suffix}@inbound.test",
        provider="sendgrid_inbound", status="active",
    )
    db_session.add(inbox); await db_session.flush()
    thread = EmailThread(
        inbox_connection_id=inbox.id, org_id=org_id,
        provider_thread_id=f"t-{suffix}", direction="inbound",
        from_email=f"lead-{suffix}@acme.test",
    )
    db_session.add(thread); await db_session.flush()
    msg = EmailMessage(
        thread_id=thread.id, inbox_connection_id=inbox.id, org_id=org_id,
        provider_message_id=f"<m-{suffix}@acme.test>", direction="inbound",
        from_email=f"lead-{suffix}@acme.test",
    )
    db_session.add(msg); await db_session.flush()
    classification = EmailClassification(
        message_id=msg.id, thread_id=thread.id,
        intent="warm_interest", confidence=0.88, reply_mode="draft",
    )
    db_session.add(classification); await db_session.flush()
    draft = EmailReplyDraft(
        thread_id=thread.id, message_id=msg.id, classification_id=classification.id,
        org_id=org_id, to_email=f"lead-{suffix}@acme.test",
        subject="Re: test", body_text="hi", reply_mode="draft", status=status,
    )
...
async def test_gm_reject_draft_transitions_status(api_client, db_session, sample_org_data):
    ...
    r = await api_client.post(
        ...
        headers=headers, json={"reason": "off topic"},
    )
    assert r.status_code == 200
    assert r.json()["status"] == "rejected"
...
async def test_gm_create_proposal_writes_rows(api_client, db_session, sample_org_data):
    headers, org_id = await _auth(api_client, sample_org_data)
    r = await api_client.post(
        "/api/v1/gm/write/proposals", headers=headers,
        json={
...
async def test_gm_send_proposal_transitions(api_client, db_session, sample_org_data):
    ...
    r = await api_client.post(
        f"/api/v1/gm/write/proposals/{pid}/send", headers=headers,
    )
    assert r.status_code == 200, r.text
...
async def test_gm_create_payment_link_persists(api_client, db_session, sample_org_data, monkeypatch):
    ...
    create = await api_client.post(
        "/api/v1/gm/write/proposals", headers=headers,
        json={
            "recipient_email": "pl@test.local", "title": "pack",
...
async def test_gm_activate_dormant_avenue_writes_audit_row(api_client, db_session, sample_org_data):
    ...
    audit = (
        await db_session.execute(
            select(OperatorAction).where(OperatorAction.id == audit_id)
        )
    ).scalar_one()
...
async def test_gm_mark_stage_requires_backing_event(api_client, sample_org_data):
    ...
    headers, _ = await _auth(api_client, sample_org_data)
    r = await api_client.post(
        "/api/v1/gm/write/stages/mark", headers=headers,
        json={
...
async def test_gm_mark_stage_succeeds_with_valid_event(api_client, db_session, sample_org_data):
    ...
    await emit_event(
        db_session, domain="monetization", event_type="proposal.sent",
        summary="test event", org_id=org_id,
        entity_type="proposal", entity_id=target_id,
        new_state="sent", actor_type="test",
    )
    await db_session.commit()
    ...
    r = await api_client.post(
        "/api/v1/gm/write/stages/mark", headers=headers,
        json={
            "entity_type": "proposal", "entity_id": str(target_id),
            "stage": "sent", "backing_event_id": str(evt.id),
...
async def test_all_gm_write_endpoints_require_auth(api_client):
    ...
    calls = [
        ...
        ("POST", "/api/v1/gm/write/proposals",
          {"recipient_email": "x@y", "title": "x",
           "line_items": [{"description": "x", "unit_amount_cents": 100}]}),
        ("POST", f"/api/v1/gm/write/proposals/{fake}/send", None),
        ("POST", f"/api/v1/gm/write/proposals/{fake}/payment-link", {}),
        ("POST", "/api/v1/gm/write/avenues/consulting/activate",
          {"authorization_notes": "x", "acknowledge_unlock_plan": True}),
        ("POST", "/api/v1/gm/write/stages/mark",
          {"entity_type": "proposal", "entity_id": fake,
           "stage": "sent", "backing_event_id": fake}),
    ]
