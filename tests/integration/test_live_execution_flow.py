...
async def test_analytics_import_creates_events(db_session: AsyncSession, seed_brand):
    ...
    result = await create_analytics_import(
        db_session,
        BRAND_ID,
        "buffer_analytics",
        "social",
    ...
    imports = list(
        (await db_session.execute(select(AnalyticsImport).where(AnalyticsImport.brand_id == BRAND_ID))).scalars().all()
    )
    assert len(imports) == 1
    assert imports[0].events_imported == 2

    events = list(
        (await db_session.execute(select(AnalyticsEvent).where(AnalyticsEvent.brand_id == BRAND_ID))).scalars().all()
    )
    assert len(events) == 2
...
async def test_conversion_import_calculates_profit(db_session: AsyncSession, seed_brand):
    ...
    result = await create_conversion_import(
        db_session,
        BRAND_ID,
        "stripe_payments",
        "checkout",
    ...
    events = list(
        (await db_session.execute(select(ConversionEvent).where(ConversionEvent.brand_id == BRAND_ID))).scalars().all()
    )
    assert len(events) == 2
...
async def test_experiment_observation_import(db_session: AsyncSession, seed_brand):
    ...
    result = await create_experiment_observation_import(
        db_session,
        BRAND_ID,
        "google_analytics",
        [
            {
                "observation_type": "conversion",
                "metric_name": "ctr",
                "metric_value": 0.045,
                "sample_size": 200,
                "confidence": 0.92,
    ...
    results = list(
        (await db_session.execute(select(ExperimentLiveResult).where(ExperimentLiveResult.brand_id == BRAND_ID)))
        .scalars()
        .all()
    )
...
async def test_crm_contact_creation_and_sync_blocked(db_session: AsyncSession, seed_brand):
    ...
    contact = await create_crm_contact(
        db_session,
        BRAND_ID,
        {
    ...
    syncs = list((await db_session.execute(select(CrmSync).where(CrmSync.brand_id == BRAND_ID))).scalars().all())
    assert len(syncs) == 1
