...
@pytest.mark.asyncio
async def test_start_onboarding_uses_high_ticket_schema_for_high_ticket_client(db_session, sample_org_data):
    ...
    profile = (
        await db_session.execute(select(ClientHighTicketProfile).where(ClientHighTicketProfile.client_id == client.id))
    ).scalar_one()
...
@pytest.mark.asyncio
async def test_schedule_discovery_sets_first_class_field(db_session, sample_org_data):
...
@pytest.mark.asyncio
async def test_record_sow_sent_promotes_url_to_first_class(db_session, sample_org_data):
    _o, client = await _seed_org_and_ht_client(db_session, sample_org_data)
    await ensure_profile(db_session, client=client)
...
@pytest.mark.asyncio
async def test_record_sow_countersigned_is_idempotent(db_session, sample_org_data):
    _o, client = await _seed_org_and_ht_client(db_session, sample_org_data)
    await ensure_profile(db_session, client=client)
...
@pytest.mark.asyncio
async def test_set_kickoff_date_flips_status_and_stores_first_class_ts(db_session, sample_org_data):
    _o, client = await _seed_org_and_ht_client(db_session, sample_org_data)
    await ensure_profile(db_session, client=client)
...
async def _seed_draft_for_client(db_session, client: Client) -> EmailReplyDraft:
    ...
    t = EmailThread(
        ...
        subject="Contract disagreement",
        direction="inbound",
        sales_stage="issue",
        reply_status="pending",
        message_count=1,
        from_email=client.primary_email,
...
@pytest.mark.asyncio
async def test_classify_high_ticket_issue_severity_scales_with_dollars(db_session, sample_org_data):
    _o, client = await _seed_org_and_ht_client(db_session, sample_org_data)
    ...
    assert {"high_ticket_scope_creep", "high_ticket_timeline_slip", "high_ticket_deliverable_dispute"}.issubset(reasons)
    await db_session.commit()


@pytest.mark.asyncio
async def test_issue_credit_writes_amount_cents_first_class(db_session, sample_org_data):
    _o, client = await _seed_org_and_ht_client(db_session, sample_org_data)
    ...
    evt = (
        await db_session.execute(
            select(ClientRetentionEvent).where(ClientRetentionEvent.id == uuid.UUID(r["retention_event_id"]))
        )
    ).scalar_one()
...
@pytest.mark.asyncio
async def test_issue_credit_rejects_non_positive_amount(db_session, sample_org_data):
    _o, client = await _seed_org_and_ht_client(db_session, sample_org_data)
...
async def test_batch12_endpoints_require_auth(api_client):
    ...
    checks = [
        (
            "POST",
            f"/api/v1/gm/write/clients/{stub}/high-ticket/schedule-discovery",
            {"when_iso": "2026-05-01T00:00:00Z"},
        ),
        ("POST", f"/api/v1/gm/write/clients/{stub}/high-ticket/sow-sent", {"sow_url": "https://x"}),
        ("POST", f"/api/v1/gm/write/clients/{stub}/high-ticket/sow-countersigned", {}),
        ("POST", f"/api/v1/gm/write/clients/{stub}/high-ticket/kickoff", {"kickoff_at_iso": "2026-05-05T00:00:00Z"}),
        (
            "POST",
            f"/api/v1/gm/write/issues/drafts/{stub}/high-ticket-classify",
            {"subtype": "scope_creep", "affected_cents": 0},
        ),
        ("POST", f"/api/v1/gm/write/clients/{stub}/high-ticket/credit", {"amount_cents": 100, "reason": "x"}),
    ]
