...
async def _auth_brand(api_client, sample_org_data):
    ...
    brand = await api_client.post(
        "/api/v1/brands/", json={"name": "Routing Brand", "slug": "routing-brand", "niche": "tech"}, headers=headers
    )
    bid = brand.json()["id"]
    return headers, bid
...
async def test_route_task_returns_provider(api_client, sample_org_data):
    ...
    r = await api_client.post(
        f"/api/v1/brands/{bid}/content-routing/route",
        json={
            "task_description": "Write a product review",
            "platform": "instagram",
            "content_type": "text",
        },
        headers=headers,
...
async def test_decision_persisted_after_route(api_client, sample_org_data):
    ...
    await api_client.post(
        f"/api/v1/brands/{bid}/content-routing/route",
        json={
            "task_description": "Generate thumbnail",
            "platform": "youtube",
            "content_type": "image",
        },
        headers=headers,
...
async def test_promoted_routes_to_hero(api_client, sample_org_data):
    ...
    r = await api_client.post(
        f"/api/v1/brands/{bid}/content-routing/route",
        json={
            "task_description": "Ad creative for paid campaign",
            "platform": "instagram",
            "content_type": "text",
            "is_promoted": True,
        },
