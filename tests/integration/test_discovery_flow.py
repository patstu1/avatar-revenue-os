...
async def test_signal_ingestion(api_client, sample_org_data):
    ...
    response = await api_client.post(
        f"/api/v1/brands/{bid}/signals/ingest",
        json={
            "source_type": "manual_seed",
            "topics": [
                {
                    "title": "Best Budget Apps 2026",
                    "keywords": ["budget", "app"],
                    "category": "budgeting",
                    "volume": 10000,
                    "velocity": 0.7,
                ...
                {
                    "title": "How to Save $1000 Fast",
                    "keywords": ["savings", "fast"],
                    "category": "savings",
                    "volume": 8000,
                    "velocity": 0.5,
                },
...
async def test_opportunity_scoring(api_client, sample_org_data):
    ...
    await api_client.post(
        ...
        json={
            "source_type": "manual_seed",
            "topics": [
                {"title": "Scoring Test", "keywords": ["finance"], "volume": 10000, "velocity": 0.8, "relevance": 0.9}
            ],
        },
        headers=headers,
    )
