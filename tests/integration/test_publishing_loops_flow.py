...
async def _auth_brand_with_content(api_client, sample_org_data):
    ...
    brand = await api_client.post(
        "/api/v1/brands/", json={"name": "PubLoop Brand", "slug": "publoop-brand", "niche": "tech"}, headers=headers
    )
    bid = brand.json()["id"]
...
async def test_content_routing_persists_decision(api_client, sample_org_data):
    ...
    r = await api_client.post(
        ...
        json={
            "task_description": "Product review for YouTube",
            "platform": "youtube",
            "content_type": "text",
        },
        headers=headers,
