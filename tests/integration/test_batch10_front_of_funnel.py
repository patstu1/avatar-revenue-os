...
async def test_bulk_import_tags_every_row_with_avenue_slug(db_session, sample_org_data):
    ...
    rows = (
        await db_session.execute(
            select(SponsorTarget).where(SponsorTarget.brand_id == brand_id)
        )
    ).scalars().all()
    assert len(rows) == 2
...
async def test_bulk_import_refuses_missing_company_name(db_session, sample_org_data):
    ...
    result = await bulk_import_leads_with_avenue(
        db_session,
        org_id=org_id, avenue_slug="b2b_services",
...
@pytest.mark.asyncio
async def test_qualify_lead_sets_tier_fit_score_and_avenue_override(
    db_session, sample_org_data
):
    org_id, brand_id = await _ensure_org_with_brand(db_session, sample_org_data)
    # Seed lead
    imp = await bulk_import_leads_with_avenue(
        db_session, org_id=org_id, avenue_slug="ugc_services",
...
async def test_qualify_rejects_bad_tier(db_session, sample_org_data):
    org_id, _ = await _ensure_org_with_brand(db_session, sample_org_data)
    imp = await bulk_import_leads_with_avenue(
        db_session, org_id=org_id, avenue_slug="b2b_services",
        rows=[{"company_name": "Bad Tier Co", "email": "bt@example.com"}],
    )
    lid = uuid.UUID(imp["first_lead_id"])
    with pytest.raises(ValueError):
        await qualify_lead(
            db_session, org_id=org_id, lead_id=lid,
            intent="offer_request", tier="super_hot",  # invalid
        )
...
@pytest.mark.asyncio
async def test_outreach_launch_creates_sequences_with_avenue_slug(
    db_session, sample_org_data
):
    org_id, brand_id = await _ensure_org_with_brand(db_session, sample_org_data)
    ...
    result = await launch_outreach_for_segment(
        db_session,
        org_id=org_id, avenue_slug="sponsor_deals",
    ...
    rows = (
        await db_session.execute(
            select(SponsorOutreachSequence).where(
                SponsorOutreachSequence.avenue_slug == "sponsor_deals"
            )
        )
    ).scalars().all()
    assert len(rows) == 2
...
@pytest.mark.asyncio
async def test_rewrite_draft_appends_version_and_flips_to_pending(
    db_session, sample_org_data
):
    org_id, _ = await _ensure_org_with_brand(db_session, sample_org_data)
    ic = InboxConnection(
        org_id=org_id, email_address="rw@test.com", display_name="Rw",
    ...
    t = EmailThread(
        ...
        subject="rw thread", direction="inbound", sales_stage="discovery",
        reply_status="pending", message_count=1,
        from_email="prospect@test.com", from_name="P",
        first_message_at=now, last_message_at=now, last_inbound_at=now,
        is_active=True, avenue_slug="b2b_services",
    )
    ...
    m = EmailMessage(
        ...
        direction="inbound", from_email="prospect@test.com",
        subject="pricing?", snippet="pricing", message_date=now,
        size_bytes=10, is_active=True, avenue_slug="b2b_services",
    )
    db_session.add(m); await db_session.flush()
    d = EmailReplyDraft(
        thread_id=t.id, message_id=m.id, org_id=org_id,
        to_email="prospect@test.com", subject="Original subject",
        body_text="Original body", reply_mode="draft",
        status="approved", confidence=0.8, is_active=True,
        avenue_slug="b2b_services",
    )
...
@pytest.mark.asyncio
async def test_route_lead_to_proposal_carries_avenue_forward(
    db_session, sample_org_data
):
    org_id, _ = await _ensure_org_with_brand(db_session, sample_org_data)
    imp = await bulk_import_leads_with_avenue(
        db_session, org_id=org_id, avenue_slug="high_ticket",
        rows=[{"company_name": "Big Co", "email": "buyer@big.test",
               "contact_name": "Big CEO"}],
    )
    lid = uuid.UUID(imp["first_lead_id"])
    ...
    proposal = await route_lead_to_proposal(
        ...
        line_items=[{"description": "Starter engagement",
                     "unit_amount_cents": 2500000, "quantity": 1,
...
async def test_batch10_endpoints_require_auth(api_client):
    checks = [
        ...
        ("POST", "/api/v1/gm/write/leads/00000000-0000-0000-0000-000000000000/qualify",
         {"intent": "offer_request", "tier": "warm"}),
        ("POST", "/api/v1/gm/write/leads/00000000-0000-0000-0000-000000000000/route-to-proposal",
        ...
        ("POST", "/api/v1/gm/write/replies/drafts/00000000-0000-0000-0000-000000000000/rewrite",
         {"subject": "x"}),
