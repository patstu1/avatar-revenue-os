...
async def setup_farm(db_session):
    ...
    brand = Brand(
        organization_id=org.id, name="Farm Brand", slug=f"fb-{uuid.uuid4().hex[:6]}", niche="personal_finance"
    )
    db_session.add(brand)
    await db_session.flush()
    acct = CreatorAccount(
        brand_id=brand.id, platform=Platform.YOUTUBE, platform_username=f"@farm_{uuid.uuid4().hex[:6]}"
    )
    db_session.add(acct)
    await db_session.flush()
...
async def test_niche_score_persistence(db_session, setup_farm):
    ...
    ns = NicheScore(
        ...
        monetization_score=scored["monetization_score"],
        opportunity_score=scored["opportunity_score"],
        trend_velocity=scored["trend_velocity"],
        competition=scored["competition"],
        avg_cpm=scored["avg_cpm"],
        affiliate_density=scored["affiliate_density"],
...
async def test_fleet_status_persistence(db_session, setup_farm):
    ...
    report = FleetStatusReport(
        ...
        accounts_warming=5,
        accounts_scaling=15,
        accounts_plateaued=3,
        accounts_suspended=1,
        accounts_retired=1,
        total_posts_today=42,
        total_revenue_30d=5000.0,
...
async def test_competitor_account_persistence(db_session, setup_farm):
    ...
    comp = CompetitorAccount(
        ...
        monetization_methods=["affiliate", "ads"],
...
async def test_repurpose_record_persistence(db_session, setup_farm):
    ...
    db_session.add(ci)
    await db_session.flush()
    derived_brief = ContentBrief(
        brand_id=brand.id,
        title="Short clip from Original Long Video",
        content_type=ContentType.SHORT_VIDEO,
    ...
    record = ContentRepurposeRecord(
        source_content_id=ci.id,
        derived_brief_id=derived_brief.id,
        brand_id=brand.id,
        target_platform="tiktok",
        target_content_type="SHORT_VIDEO",
    )
