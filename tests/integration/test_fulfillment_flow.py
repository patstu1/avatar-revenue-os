...
async def test_intake_completion_cascades_to_project_brief_production(
    ...
    # Project created
    project = (await db_session.execute(select(ClientProject).where(ClientProject.org_id == org_id))).scalar_one()
    assert project.status == "active"
    assert project.intake_submission_id is not None

    # Brief v1 created
    briefs = (
        (await db_session.execute(select(ProjectBrief).where(ProjectBrief.project_id == project.id))).scalars().all()
    )
    assert len(briefs) == 1
...
@pytest.mark.asyncio
async def test_cascade_is_idempotent_on_reruns(api_client, db_session, sample_org_data, monkeypatch):
    ...
    sub = (await db_session.execute(select(IntakeSubmission).where(IntakeSubmission.org_id == org_id))).scalar_one()
    ...
    projects = (await db_session.execute(select(ClientProject).where(ClientProject.org_id == org_id))).scalars().all()
    assert len(projects) == 1

    briefs = (
        (await db_session.execute(select(ProjectBrief).where(ProjectBrief.project_id == projects[0].id)))
        .scalars()
        .all()
    )
    assert len(briefs) == 1
    ...
    jobs = (
        (await db_session.execute(select(ProductionJob).where(ProductionJob.project_id == projects[0].id)))
        .scalars()
        .all()
    )
    assert len(jobs) == 1
...
@pytest.mark.asyncio
async def test_regenerate_brief_creates_v2_supersedes_v1(api_client, db_session, sample_org_data, monkeypatch):
    headers, org_id = await _auth(api_client, sample_org_data)
    await _full_activation(api_client, db_session, headers, org_id, monkeypatch)
    ...
    briefs = (
        (
            await db_session.execute(
                select(ProjectBrief).where(ProjectBrief.project_id == project.id).order_by(ProjectBrief.version)
            )
        )
        .scalars()
        .all()
...
@pytest.mark.asyncio
async def test_launch_production_idempotent_when_job_active(api_client, db_session, sample_org_data, monkeypatch):
    headers, org_id = await _auth(api_client, sample_org_data)
    await _full_activation(api_client, db_session, headers, org_id, monkeypatch)

    brief = (await db_session.execute(select(ProjectBrief).where(ProjectBrief.org_id == org_id))).scalar_one()
    ...
    jobs = (await db_session.execute(select(ProductionJob).where(ProductionJob.brief_id == brief.id))).scalars().all()
    assert len(jobs) == 1


@pytest.mark.asyncio
async def test_get_project_returns_nested_briefs_and_jobs(api_client, db_session, sample_org_data, monkeypatch):
    headers, org_id = await _auth(api_client, sample_org_data)
    await _full_activation(api_client, db_session, headers, org_id, monkeypatch)
...
@pytest.mark.asyncio
async def test_cross_org_project_access_returns_403(api_client, db_session, sample_org_data, monkeypatch):
    ...
    project_a = (await db_session.execute(select(ClientProject).where(ClientProject.org_id == org_a))).scalar_one()
