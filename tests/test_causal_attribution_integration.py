    ...
    for i in range(8):
        ...
        db_session.add(
            PerformanceMetric(
                ...
                engagement_rate=engagement_series[i],
...
async def test_reports_created(db_session, brand_with_perf_series):
    brand = brand_with_perf_series
    await recompute_attribution(db_session, brand.id)
    await db_session.commit()
    reports = (
        (await db_session.execute(select(CausalAttributionReport).where(CausalAttributionReport.brand_id == brand.id)))
        .scalars()
        .all()
    )
    ...
    hypos = (
        (
            await db_session.execute(
                ...
                .where(CausalAttributionReport.brand_id == brand.id)
            )
        )
        .scalars()
        .all()
