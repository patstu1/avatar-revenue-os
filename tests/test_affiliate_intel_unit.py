...
class TestRanking:
    def test_high_epc_ranks_high(self):
        score = rank_offer({"epc": 4.0, "conversion_rate": 0.05, "commission_rate": 30, "trust_score": 0.8, "content_fit_score": 0.7, "platform_fit_score": 0.7, "audience_fit_score": 0.7})
        assert score > 0.5
    ...
    def test_select_best(self):
        offers = [{"id": "a", "epc": 4.0, "platform_fit_score": 0.8}, {"id": "b", "epc": 1.0, "platform_fit_score": 0.2}]
...
class TestLinkBuilder:
    def test_builds_utm(self):
        link = build_affiliate_link({"affiliate_url": "https://example.com/offer?ref=abc"}, content_item_id="ci123", campaign_id="camp456", platform="tiktok")
...
class TestLeakDetection:
    ...
    def test_high_refund(self):
        leaks = detect_leaks([{"id": "o1", "is_active": True, "blocker_state": None, "refund_rate": 0.25, "epc": 2.0}], [])
        assert any(l["leak_type"] == "high_refund_rate" for l in leaks)

    def test_no_leaks_clean(self):
        leaks = detect_leaks([{"id": "o1", "is_active": True, "blocker_state": None, "refund_rate": 0.02}], [{"id": "l1", "offer_id": "o1", "click_count": 10, "conversion_count": 2}])
        assert len(leaks) == 0


class TestBlockerDetection:
    def test_no_url(self):
        blockers = detect_blockers([{"id": "o1", "product_name": "Test", "affiliate_url": None, "destination_url": None, "epc": 1.0, "commission_rate": 10}])
        assert any(b["blocker_type"] == "no_destination_url" for b in blockers)

    def test_no_commission(self):
        blockers = detect_blockers([{"id": "o1", "product_name": "Test", "affiliate_url": "https://x.com", "epc": 0, "commission_rate": 0}])
