...
async def test_create_experiment(db_session, brand):
    exp = await create_experiment(db_session, brand.id, {
...
async def test_add_observations(db_session, brand):
    ...
    v0 = (await db_session.execute(select(PWExperimentVariant).where(PWExperimentVariant.id == variants[0].id))).scalar_one()
...
async def test_evaluate_finds_winner(db_session, brand):
    ...
    winners = (await db_session.execute(
        select(PWExperimentWinner).where(PWExperimentWinner.brand_id == brand.id)
    )).scalars().all()
    assert len(winners) == 1
    assert winners[0].promoted is True

    rules = (await db_session.execute(
        select(PromotedWinnerRule).where(PromotedWinnerRule.brand_id == brand.id)
    )).scalars().all()
    assert len(rules) >= 1
    assert any(r.rule_type == "default_offer_angle" for r in rules)

    losers = (await db_session.execute(
        select(PWExperimentLoser).where(PWExperimentLoser.brand_id == brand.id)
    )).scalars().all()
    assert len(losers) == 1
