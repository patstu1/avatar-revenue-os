...
async def org_with_affiliate(db_session: AsyncSession):
    ...
    good = AffiliateOffer(
        ...
        product_name="Good Offer",
        epc=3.0,
        conversion_rate=0.05,
        commission_rate=20,
        trust_score=0.8,
        affiliate_url="https://aff.com/good",
    )
    risky = AffiliateOffer(
        brand_id=brand.id,
        merchant_id=merchant.id,
        product_name="Risky Offer",
        epc=0,
        conversion_rate=0,
        commission_rate=5,
        trust_score=0.2,
    ...
    db_session.add_all([good, risky])
    await db_session.flush()

    db_session.add(
        AffiliateGovernanceRule(
            organization_id=org.id, rule_type="max_commission_rate", rule_key="global_max", rule_value={"max": 50}
        )
    )
    db_session.add(
        AffiliateBannedEntity(
            organization_id=org.id, entity_type="merchant", entity_name="ScamCo", reason="Known fraud"
        )
    )

    partner = OwnedAffiliatePartner(
        organization_id=org.id,
        partner_name="Good Partner",
        partner_status="active",
        total_conversions=50,
        conversion_quality=0.7,
        fraud_risk=0.1,
        total_revenue_generated=2000,
    )
    db_session.add(partner)
    await db_session.flush()
    for i in range(5):
        db_session.add(
            OwnedPartnerConversion(partner_id=partner.id, conversion_value=40, commission_paid=8, quality_score=0.8)
        )
    await db_session.flush()
...
async def test_risk_flags_persisted(db_session, org_with_affiliate):
    org, _ = org_with_affiliate
    await recompute_governance(db_session, org.id)
    await db_session.commit()
    flags = (
        (await db_session.execute(select(AffiliateRiskFlag).where(AffiliateRiskFlag.organization_id == org.id)))
        .scalars()
        .all()
    )
    assert len(flags) >= 1
...
async def test_partner_score_updated(db_session, org_with_affiliate):
    org, _ = org_with_affiliate
    await recompute_partner_scores(db_session, org.id)
    await db_session.commit()
    partners = (
        (await db_session.execute(select(OwnedAffiliatePartner).where(OwnedAffiliatePartner.organization_id == org.id)))
        .scalars()
        .all()
    )
