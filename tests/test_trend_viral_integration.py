...
async def brand_with_trends(db_session: AsyncSession):
    ...
    db_session.add(DiscoveryTrend(brand_id=brand.id, keyword="AI agents taking over", signal_type="trending_topic", strength=SignalStrength.STRONG, velocity=2.5, volume=5000))
    db_session.add(DiscoveryTrend(brand_id=brand.id, keyword="Budget productivity tools", signal_type="trending_topic", strength=SignalStrength.MODERATE, velocity=1.0, volume=1500))
...
async def test_deep_analysis_creates_opportunities(db_session, brand_with_trends):
    brand = brand_with_trends
    await light_scan(db_session, brand.id); await db_session.flush()
...
async def test_opportunities_have_scores(db_session, brand_with_trends):
    brand = brand_with_trends
    await light_scan(db_session, brand.id); await db_session.flush()
    await deep_analysis(db_session, brand.id); await db_session.commit()
    opps = (await db_session.execute(select(ViralOpportunity).where(ViralOpportunity.brand_id == brand.id))).scalars().all()
    assert len(opps) >= 1
    for o in opps:
        assert o.composite_score > 0
        assert o.recommended_platform
        assert o.recommended_content_form
        assert o.opportunity_type in ("monetization", "pure_reach", "authority_building", "growth", "community_engagement")
...
async def test_velocity_tracked(db_session, brand_with_trends):
    ...
    await light_scan(db_session, brand.id); await db_session.flush()
    await deep_analysis(db_session, brand.id); await db_session.commit()
    vel = (await db_session.execute(select(TrendVelocityReport).where(TrendVelocityReport.brand_id == brand.id))).scalars().all()
    assert len(vel) >= 1
...
async def test_source_health_tracked(db_session, brand_with_trends):
    ...
    await light_scan(db_session, brand.id); await db_session.flush()
    await deep_analysis(db_session, brand.id); await db_session.commit()
    health = (await db_session.execute(select(TrendSourceHealth).where(TrendSourceHealth.brand_id == brand.id))).scalars().all()
    assert len(health) >= 1
...
async def test_list_signals(db_session, brand_with_trends):
    brand = brand_with_trends
    await light_scan(db_session, brand.id); await db_session.commit()
...
async def test_dedup_prevents_duplicates(db_session, brand_with_trends):
    brand = brand_with_trends
    await light_scan(db_session, brand.id); await db_session.flush()
    await deep_analysis(db_session, brand.id); await db_session.flush()
    r1_count = len((await db_session.execute(select(ViralOpportunity).where(ViralOpportunity.brand_id == brand.id))).scalars().all())
    await deep_analysis(db_session, brand.id); await db_session.commit()
    r2_count = len((await db_session.execute(select(ViralOpportunity).where(ViralOpportunity.brand_id == brand.id))).scalars().all())
