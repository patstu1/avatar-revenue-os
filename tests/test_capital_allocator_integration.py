...
async def brand_with_data(db_session: AsyncSession):
    ...
    offer = Offer(brand_id=brand.id, name="Test Offer", monetization_method="affiliate", payout_amount=30.0, epc=2.5, conversion_rate=0.05)
    db_session.add(offer)
    await db_session.flush()

    ci = ContentItem(brand_id=brand.id, creator_account_id=acct.id, title="Test content", content_type=ContentType.SHORT_VIDEO, platform="tiktok")
    db_session.add(ci)
    await db_session.flush()

    pm = PerformanceMetric(brand_id=brand.id, content_item_id=ci.id, creator_account_id=acct.id, platform=Platform.TIKTOK, impressions=10000, clicks=500, engagement_rate=0.08, revenue=50.0)
    db_session.add(pm)
    await db_session.flush()
...
async def test_recompute_allocation(db_session, brand_with_data):
    ...
    reports = (await db_session.execute(
        select(CapitalAllocationReport).where(CapitalAllocationReport.brand_id == brand.id)
    )).scalars().all()
...
async def test_decisions_created(db_session, brand_with_data):
    ...
    decisions = (await db_session.execute(
        select(CAAllocationDecision).where(CAAllocationDecision.brand_id == brand.id)
    )).scalars().all()
...
async def test_targets_created(db_session, brand_with_data):
    ...
    targets = (await db_session.execute(
        select(AllocationTarget).where(AllocationTarget.brand_id == brand.id)
    )).scalars().all()
