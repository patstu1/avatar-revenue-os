...
async def brand_with_affiliate(db_session: AsyncSession):
    ...
    good = AffiliateOffer(
        ...
        commission_rate=25,
        trust_score=0.8,
        affiliate_url="https://aff.com/good?ref=123",
    )
    weak = AffiliateOffer(
        brand_id=brand.id,
    ...
    db_session.add_all([good, weak, no_url])
    await db_session.flush()

    link = AffiliateLink(
        brand_id=brand.id, offer_id=good.id, full_url="https://aff.com/good?ref=123", click_count=80, conversion_count=0
    )
    db_session.add(link)
    await db_session.flush()
    ...
    offers = (
        (
            await db_session.execute(
                ...
                .where(AffiliateOffer.brand_id == brand.id)
...
async def test_blockers_detected(db_session, brand_with_affiliate):
    ...
    await db_session.commit()
    blockers = (
        (await db_session.execute(select(AffiliateBlocker).where(AffiliateBlocker.brand_id == brand.id)))
        .scalars()
        .all()
    )
