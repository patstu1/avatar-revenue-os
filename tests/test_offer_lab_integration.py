...
async def brand_with_offers(db_session: AsyncSession):
    ...
    o1 = Offer(brand_id=brand.id, name="Premium Course", monetization_method="product", payout_amount=97, epc=4.0, conversion_rate=0.05)
    o2 = Offer(brand_id=brand.id, name="Budget Tool", monetization_method="affiliate", payout_amount=19, epc=1.5, conversion_rate=0.08)
    db_session.add_all([o1, o2]); await db_session.flush()
    return brand
...
async def test_offers_created(db_session, brand_with_offers):
    brand = brand_with_offers
    await recompute_offer_lab(db_session, brand.id); await db_session.commit()
...
async def test_variants_created(db_session, brand_with_offers):
    brand = brand_with_offers
    await recompute_offer_lab(db_session, brand.id); await db_session.commit()
    variants = (await db_session.execute(select(OfferLabVariant).join(OfferLabOffer).where(OfferLabOffer.brand_id == brand.id))).scalars().all()
...
async def test_pricing_tests(db_session, brand_with_offers):
    brand = brand_with_offers
    await recompute_offer_lab(db_session, brand.id); await db_session.commit()
    tests = (await db_session.execute(select(OfferLabPricingTest).join(OfferLabOffer).where(OfferLabOffer.brand_id == brand.id))).scalars().all()
    assert len(tests) == 2
...
async def test_bundles_created(db_session, brand_with_offers):
    brand = brand_with_offers
    await recompute_offer_lab(db_session, brand.id); await db_session.commit()
    bundles = (await db_session.execute(select(OfferLabBundle).where(OfferLabBundle.brand_id == brand.id))).scalars().all()
...
async def test_idempotent(db_session, brand_with_offers):
    brand = brand_with_offers
    r1 = await recompute_offer_lab(db_session, brand.id); await db_session.commit()
    r2 = await recompute_offer_lab(db_session, brand.id); await db_session.commit()
