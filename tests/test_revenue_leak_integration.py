...
async def brand_with_perf(db_session: AsyncSession):
    ...
    ci_leak = ContentItem(
        brand_id=brand.id,
        creator_account_id=acct.id,
        title="Leaking content",
        content_type=ContentType.SHORT_VIDEO,
        platform="tiktok",
        status="published",
    )
    db_session.add_all([ci_good, ci_leak])
    await db_session.flush()

    db_session.add(
        PerformanceMetric(
            brand_id=brand.id,
            content_item_id=ci_good.id,
            creator_account_id=acct.id,
...
async def test_report_created(db_session, brand_with_perf):
    brand = brand_with_perf
    await recompute_leaks(db_session, brand.id)
    await db_session.commit()
    reports = (
        (await db_session.execute(select(RevenueLeakReport).where(RevenueLeakReport.brand_id == brand.id)))
        .scalars()
        .all()
    )
...
async def test_events_created(db_session, brand_with_perf):
    ...
    await db_session.commit()
    events = (
        (await db_session.execute(select(RevenueLeakEvent).where(RevenueLeakEvent.brand_id == brand.id)))
        .scalars()
        .all()
    )
    assert len(events) >= 1
...
async def test_corrections_created(db_session, brand_with_perf):
    ...
    await db_session.commit()
    corrs = (
        (await db_session.execute(select(LeakCorrectionAction).where(LeakCorrectionAction.brand_id == brand.id)))
        .scalars()
        .all()
    )
    assert len(corrs) >= 1
...
async def test_idempotent(db_session, brand_with_perf):
    brand = brand_with_perf
    r1 = await recompute_leaks(db_session, brand.id)
    await db_session.commit()
    r2 = await recompute_leaks(db_session, brand.id)
    await db_session.commit()
