...
async def test_start_instance(db_session, org_with_user):
    ...
    ist_steps = (
        (await db_session.execute(select(WorkflowInstanceStep).where(WorkflowInstanceStep.instance_id == inst.id)))
        .scalars()
        .all()
    )
...
async def test_approve_advances(db_session, org_with_user):
    ...
    wf = await create_from_template(db_session, org.id, "content_publish")
    await db_session.flush()
    inst = await start_instance(db_session, wf.id, "content_item", uuid.uuid4(), initiated_by=user.id)
    await db_session.flush()
...
async def test_full_approval_completes(db_session, org_with_user):
    ...
    await approve_step(db_session, inst.id, user.id, "super_admin")
    await db_session.flush()
    await approve_step(db_session, inst.id, user.id, "super_admin")
    await db_session.flush()
    r3 = await approve_step(db_session, inst.id, user.id, "super_admin")
    await db_session.flush()
...
async def test_get_workflow_status(db_session, org_with_user):
    ...
    await db_session.flush()
    rid = uuid.uuid4()
    await start_instance(db_session, wf.id, "content_item", rid, initiated_by=user.id)
    await db_session.commit()
...
async def test_list_definitions(db_session, org_with_user):
    org, _ = org_with_user
    await create_from_template(db_session, org.id, "content_publish")
    await db_session.commit()
...
async def test_list_instances(db_session, org_with_user):
    ...
    wf = await create_from_template(db_session, org.id, "content_publish")
    await db_session.flush()
    await start_instance(db_session, wf.id, "content_item", uuid.uuid4(), initiated_by=user.id)
    await db_session.commit()
