...
async def org_with_failures(db_session: AsyncSession):
    ...
    db_session.add(ProviderBlocker(brand_id=brand.id, provider_key="stripe", blocker_type="api_failure", severity="critical", description="Stripe API returning 500 errors", operator_action_needed="Check Stripe dashboard"))
...
async def test_incidents_created(db_session, org_with_failures):
    org = org_with_failures
    await recompute_recovery(db_session, org.id); await db_session.commit()
    incidents = (await db_session.execute(select(RecoveryIncidentV2).where(RecoveryIncidentV2.organization_id == org.id))).scalars().all()
...
async def test_reroute_actions_created(db_session, org_with_failures):
    org = org_with_failures
    await recompute_recovery(db_session, org.id); await db_session.commit()
    reroutes = (await db_session.execute(select(RerouteAction).join(RecoveryIncidentV2).where(RecoveryIncidentV2.organization_id == org.id))).scalars().all()
...
async def test_throttle_actions_created(db_session, org_with_failures):
    org = org_with_failures
    await recompute_recovery(db_session, org.id); await db_session.commit()
    throttles = (await db_session.execute(select(ThrottlingAction).join(RecoveryIncidentV2).where(RecoveryIncidentV2.organization_id == org.id))).scalars().all()
