...
async def brand_with_governance(db_session: AsyncSession):
    ...
    db_session.add(
        BrandGovernanceProfile(
            brand_id=brand.id, tone_profile="professional, data-driven, transparent", governance_level="strict"
        )
    )
    db_session.add(
        BrandVoiceRule(brand_id=brand.id, rule_type="banned_phrase", rule_key="guaranteed results", severity="hard")
    )
    db_session.add(
        BrandVoiceRule(brand_id=brand.id, rule_type="required_phrase", rule_key="affiliate disclosure", severity="soft")
    )
    ...
    good_ci = ContentItem(
        ...
        title="Professional data-driven review with affiliate disclosure",
        description="Transparent analysis of the product. Affiliate disclosure included.",
    ...
    bad_ci = ContentItem(
        brand_id=brand.id,
        creator_account_id=acct.id,
        title="Get guaranteed results NOW!",
        description="This product gives guaranteed results fast.",
        content_type=ContentType.SHORT_VIDEO,
        platform="tiktok",
        status="draft",
    )
    ...
    viols = (
        (
            await db_session.execute(
                select(BrandGovernanceViolation).where(BrandGovernanceViolation.content_item_id == bad_ci.id)
            )
        )
        .scalars()
        .all()
...
async def test_list_violations(db_session, brand_with_governance):
    brand, _, bad_ci = brand_with_governance
    await evaluate_content(db_session, brand.id, bad_ci.id)
    await db_session.commit()
...
async def test_idempotent(db_session, brand_with_governance):
    brand, _, _ = brand_with_governance
    r1 = await recompute_governance(db_session, brand.id)
    await db_session.commit()
    r2 = await recompute_governance(db_session, brand.id)
    await db_session.commit()
