...
class TestOpportunityScoring:
    ...
    def test_strong_signals_produce_high_score(self):
        inp = OpportunityInput(
            ...
            offer_fit=0.85,
            expected_profit_score=0.9,
            platform_suitability=0.8,
        )
    ...
    def test_penalties_reduce_score(self):
        base_inp = OpportunityInput(
            ...
            content_gap=0.6,
            offer_fit=0.7,
            expected_profit_score=0.8,
            platform_suitability=0.7,
        )
    ...
    def test_score_clamped_to_0_1(self):
        inp = OpportunityInput(
            ...
            offer_fit=1.0,
            expected_profit_score=1.0,
            platform_suitability=1.0,
            seasonal_boost=0.15,
            brand_fit_boost=0.10,
        )
        result = compute_opportunity_score(inp)
...
class TestForecast:
    ...
    def test_basic_forecast(self):
        inp = ForecastInput(
            ...
            expected_value_per_conversion=50.0,
            expected_generation_cost=3.0,
            expected_distribution_cost=1.0,
        )
    ...
    def test_costs_reduce_profit(self):
        base = compute_profit_forecast(
            ForecastInput(
                expected_impressions=5000,
                expected_ctr=0.02,
                expected_conversion_rate=0.03,
                expected_value_per_conversion=30.0,
            )
        )
        with_costs = compute_profit_forecast(
            ForecastInput(
                expected_impressions=5000,
                expected_ctr=0.02,
                expected_conversion_rate=0.03,
                expected_value_per_conversion=30.0,
                expected_generation_cost=10.0,
                fatigue_penalty_dollars=5.0,
            )
        )
        assert with_costs.expected_profit < base.expected_profit

    def test_rpm_calculation(self):
        result = compute_profit_forecast(
            ForecastInput(
                expected_impressions=1000,
                expected_ctr=0.05,
                expected_conversion_rate=0.10,
                expected_value_per_conversion=20.0,
            )
        )
    ...
    def test_explanation_present(self):
        result = compute_profit_forecast(
            ForecastInput(
                ...
                expected_ctr=0.02,
                expected_conversion_rate=0.03,
                expected_value_per_conversion=25.0,
            )
        )
...
class TestOfferFit:
    def test_no_input(self):
        result = compute_offer_fit(
            OfferFitInput(
                topic_keywords=[],
                offer_audience_tags=[],
                offer_niche_relevance=0.0,
                topic_buyer_intent=0.0,
            )
        )
        assert result.confidence == "insufficient"
    ...
    def test_good_fit(self):
        result = compute_offer_fit(
            OfferFitInput(
                ...
                offer_epc=3.0,
                offer_conversion_rate=0.06,
                offer_payout=45.0,
                topic_buyer_intent=0.8,
                offer_niche_relevance=0.8,
            )
        )
    ...
    def test_platform_restriction_increases_friction(self):
        no_restriction = compute_offer_fit(
            OfferFitInput(
                topic_keywords=["test"],
                offer_audience_tags=["test"],
                offer_epc=2.0,
                topic_buyer_intent=0.5,
            )
        )
        with_restriction = compute_offer_fit(
            OfferFitInput(
                topic_keywords=["test"],
                offer_audience_tags=["test"],
                offer_epc=2.0,
                topic_buyer_intent=0.5,
                platform="tiktok",
                offer_platform_restrictions=["youtube"],
            )
        )
...
class TestSaturation:
    ...
    def test_high_saturation(self):
        result = compute_saturation(
            SaturationInput(
                unique_topics_covered=18,
                total_topics_available=20,
                posts_last_7d=21,
                posts_last_30d=60,
                similar_content_count=8,
                audience_overlap_pct=0.7,
            )
        )
    ...
    def test_engagement_decline_causes_fatigue(self):
        result = compute_saturation(
            SaturationInput(
                avg_engagement_last_7d=2.0,
                avg_engagement_last_30d=5.0,
            )
        )
