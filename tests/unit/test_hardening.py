...
class TestQueueRouting:
    ...
    def test_ep2_tasks_routed_to_expansion_pack2(self):
        ...
        routes = app.conf.task_routes
        ep2_keys = [
            k for k in routes if "lead_qualification" in k or "pricing_recommendations" in k or "referral_program" in k
        ]
        for k in ep2_keys:
    ...
    def test_rc_a_tasks_routed_correctly(self):
        ...
        rc_a_keys = [
            k
            for k in routes
            if "offer_ladders" in k or "owned_audience" in k or "message_sequences" in k or "funnel_leaks" in k
        ]
        for k in rc_a_keys:
