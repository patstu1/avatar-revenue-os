...
def test_niche_trend_velocity_boost():
    ...
    with_trends = score_niche(finance, "youtube", trend_signals=[
        {"title": "New budgeting app released"}, {"title": "Investing in 2026"},
        {"title": "How to save money fast"}, {"title": "Credit score tips"},
    ])
...
def test_voice_profile_generation():
    ...
    assert profile["style"] in [
        ...
        "dry_witty_humor", "academic_authoritative", "street_smart_practical", "minimalist_punchy",
    ]
...
def test_build_generation_prompt():
    ...
    prompt = _build_generation_prompt(brief, None, {
