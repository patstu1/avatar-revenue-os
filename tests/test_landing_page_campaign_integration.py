...
async def brand_with_offer(db_session: AsyncSession):
    ...
    offer = Offer(brand_id=brand.id, name="LPC Offer", monetization_method="affiliate", payout_amount=30.0, epc=2.5, conversion_rate=0.05)
    db_session.add(offer); await db_session.flush()
...
async def test_variants_created(db_session, brand_with_offer):
    ...
    await recompute_landing_pages(db_session, brand.id); await db_session.commit()
    variants = (await db_session.execute(select(LandingPageVariant).join(LandingPage).where(LandingPage.brand_id == brand.id))).scalars().all()
...
async def test_quality_scored(db_session, brand_with_offer):
    brand, _, _ = brand_with_offer
    await recompute_landing_pages(db_session, brand.id); await db_session.commit()
    quality = (await db_session.execute(select(LandingPageQualityReport).where(LandingPageQualityReport.brand_id == brand.id))).scalars().all()
...
async def test_recompute_campaigns(db_session, brand_with_offer):
    ...
    await recompute_landing_pages(db_session, brand.id); await db_session.flush()
...
async def test_campaign_linked_to_page(db_session, brand_with_offer):
    brand, offer, _ = brand_with_offer
    await recompute_landing_pages(db_session, brand.id); await db_session.flush()
    await recompute_campaigns(db_session, brand.id); await db_session.commit()
...
async def test_campaign_blockers(db_session, brand_with_offer):
    ...
    await recompute_landing_pages(db_session, brand.id); await db_session.flush()
    await recompute_campaigns(db_session, brand.id); await db_session.commit()
    blockers = (await db_session.execute(select(CampaignBlocker).where(CampaignBlocker.brand_id == brand.id))).scalars().all()
...
async def test_campaign_variants(db_session, brand_with_offer):
    ...
    await recompute_landing_pages(db_session, brand.id); await db_session.flush()
    await recompute_campaigns(db_session, brand.id); await db_session.commit()
    variants = (await db_session.execute(select(CampaignVariant).join(Campaign).where(Campaign.brand_id == brand.id))).scalars().all()
...
async def test_idempotent(db_session, brand_with_offer):
    brand, _, _ = brand_with_offer
    await recompute_landing_pages(db_session, brand.id); await db_session.commit()
    r1 = await recompute_landing_pages(db_session, brand.id); await db_session.commit()
    r2 = await recompute_landing_pages(db_session, brand.id); await db_session.commit()
