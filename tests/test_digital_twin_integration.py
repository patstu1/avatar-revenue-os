...
async def test_run_created(db_session, brand_with_state):
    brand = brand_with_state
    await run_simulation(db_session, brand.id)
    await db_session.commit()
    ...
    scenarios = (
        (
            await db_session.execute(
                select(SimulationScenario).join(SimulationRun).where(SimulationRun.brand_id == brand.id)
            )
        )
        .scalars()
        .all()
...
async def test_recommendations_created(db_session, brand_with_state):
    ...
    recs = (
        (
            await db_session.execute(
                select(SimulationRecommendation).where(SimulationRecommendation.brand_id == brand.id)
            )
        )
        .scalars()
        .all()
