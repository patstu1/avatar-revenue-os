...
async def main():
    ...
    async with factory() as db:
        ...
        await db.flush()
        brand = Brand(
            organization_id=org.id, name="Compound Brand", slug=f"cp-{uuid.uuid4().hex[:6]}", niche="business"
        )
        db.add(brand)
        await db.flush()
        ...
        db.add_all([strong_offer, weak_offer])
        await db.flush()

        # Accounts: one active, one to be reduced
        active_acct = CreatorAccount(
            brand_id=brand.id,
            platform=Platform.YOUTUBE,
            platform_username="@active",
            is_active=True,
            follower_count=30000,
            ctr=0.05,
            scale_role="active",
        )
        dead_acct = CreatorAccount(
            brand_id=brand.id,
            platform=Platform.PINTEREST,
            platform_username="@dead",
            is_active=True,
            follower_count=500,
            ctr=0.001,
            scale_role="active",
        )
        db.add_all([active_acct, dead_acct])
        await db.flush()

        # Content: one unmonetized
        content = ContentItem(
            brand_id=brand.id, title="Unmonetized Guide", content_type=ContentType.SHORT_VIDEO, status="published"
        )
        db.add(content)
        await db.flush()
        ...
        failed_webhook = WebhookEvent(
            ...
            external_event_id="evt_recovery_test",
            raw_payload={"data": {"object": {"amount": 15000, "payment_intent": "pi_test"}}},
            processed=False,
            idempotency_key="stripe:evt_recovery_test",
        )
        ...
        await mon.record_affiliate_commission_to_ledger(
            db, brand_id=brand.id, gross_amount=500, offer_id=strong_offer.id, creator_account_id=active_acct.id
        )
        await db.flush()
        ...
        await emit_action(
            db,
            org_id=org.id,
            action_type="repair_broken_attribution",
            title="Repair attribution",
            category="monetization",
            priority="medium",
            brand_id=brand.id,
            entity_type="revenue_ledger",
            entity_id=unattr_ledger.id,
            source_module="test",
            action_payload={"autonomy_level": "autonomous", "confidence": 0.8},
        )
        ...
        await emit_action(
            db,
            org_id=org.id,
            action_type="recover_failed_webhook",
        ...
        ok(
            f"Dispatched: {result['total_autonomous']} autonomous, {len(result['executed'])} executed",
        ...
        ok(
            f"  Compound: fit score {dead_score_before:.3f} → {dead_score_after:.3f} (reduced)",
            dead_score_after < dead_score_before,
        )
        ...
        sponsor_opps = [
            o for o in opps_after2 if o["type"] == "sponsor_ready" and o.get("entity_id") == str(dead_acct.id)
        ]
        ...
        ok(
            f"  State: attribution_state = '{unattr_ledger.attribution_state}'",
            unattr_ledger.attribution_state == "auto_attributed",
        )
        ...
        ok(
            "  Compound: original attribution repaired (state=auto_attributed)",
            unattr_ledger.attribution_state == "auto_attributed",
        )
        ...
        if recovery_ledger:
            ok(
                f"  Compound: recovery amount = ${recovery_ledger.gross_amount:.2f}",
                recovery_ledger.gross_amount == 150.0,
            )

        # Verify recovery compounds into revenue state
        rev_state2 = await mon.get_brand_revenue_state(db, brand.id)
        ok(
            "  Compound: revenue state includes recovered amount",
            rev_state2.get("ledger_revenue_30d", 0) > rev_state.get("ledger_revenue_30d", 0)
            or recovery_ledger is not None,
        )
        print()
