...
async def main():
    ...
    async with factory() as db:
        ...
        await db.flush()
        brand = Brand(
            organization_id=org.id, name="Full Engine Brand", slug=f"full-{uuid.uuid4().hex[:6]}", niche="business"
        )
        db.add(brand)
        await db.flush()
        ...
        c1 = ContentItem(
            brand_id=brand.id,
            title="Top SaaS Tools Review",
            content_type=ContentType.LONG_VIDEO,
            status="published",
            offer_id=o1.id,
            creator_account_id=a1.id,
        )
        c2 = ContentItem(
            brand_id=brand.id,
            title="Creator Course Launch",
            content_type=ContentType.SHORT_VIDEO,
            status="published",
            offer_id=o2.id,
            creator_account_id=a2.id,
        )
        c3 = ContentItem(
            brand_id=brand.id,
            title="Lead Gen Guide",
            content_type=ContentType.TEXT_POST,
            status="published",
            offer_id=o3.id,
        )
        c4 = ContentItem(
            brand_id=brand.id, title="Unmonetized Tutorial", content_type=ContentType.LONG_VIDEO, status="published"
        )
        ...
        # Sponsor
        sp = SponsorProfile(
            brand_id=brand.id, sponsor_name="TechCorp", industry="tech", budget_range_min=2000, budget_range_max=10000
        )
        db.add(sp)
        await db.flush()
        deal = SponsorOpportunity(
            brand_id=brand.id, sponsor_id=sp.id, title="TechCorp Sponsorship", deal_value=5000, status="negotiation"
        )
        db.add(deal)
        await db.flush()

        # Revenue ledger (4 sources)
        await mon.record_affiliate_commission_to_ledger(
            db, brand_id=brand.id, gross_amount=800, offer_id=o1.id, content_item_id=c1.id, creator_account_id=a1.id
        )
        await mon.record_sponsor_payment_to_ledger(
            db, brand_id=brand.id, gross_amount=3000, sponsor_id=sp.id, payment_state="confirmed"
        )
        await mon.record_service_payment_to_ledger(db, brand_id=brand.id, gross_amount=2500, platform_fee=72.50)
        await mon.record_product_sale_to_ledger(
            db, brand_id=brand.id, gross_amount=591, offer_id=o2.id, content_item_id=c2.id
        )
        await mon.record_ad_revenue_to_ledger(
            db, brand_id=brand.id, gross_amount=340, content_item_id=c1.id, creator_account_id=a1.id
        )
        await db.flush()
        ...
        ok(
            "E8: Simulation",
            "projected" in sim and "delta" in sim and sim["projected"]["gross_90d"] > sim["current"]["gross_90d"],
        )
        ...
        ok(
            "E9: Has recommendation",
            all(m["recommendation"] in ("scale", "maintain", "reduce", "suppress") for m in margin),
        )
        ...
        ok(
            "E11: Has entry/core/premium",
            any(p["role"] == "entry" for p in pkg) or any(p["role"] == "core" for p in pkg),
        )
        ...
        ok(
            "E13: Has recommendation",
            all(s["recommendation"] in ("prioritize", "acceptable", "slow_payer") for s in speed),
        )
        ...
        ok(
            "E17: Has recommendation",
            all(d["recommendation"] in ("exploit", "diversify", "stabilize", "reduce") for d in durability),
        )
        print()
        ...
        ok(
            "Governance applied (3 tiers)",
            all(
                a["autonomy_level"] in ("surface", "assisted", "autonomous")
                for tier in [
                    exec_result["autonomous_executed"],
                    exec_result["awaiting_approval"],
                    exec_result["surfaced_for_review"],
                ]
                for a in tier
            ),
        )
