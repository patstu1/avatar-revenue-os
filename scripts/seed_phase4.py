    ...
    with Session(engine) as db:
        ...
        for i, item in enumerate(items):
            ...
            pm = PerformanceMetric(
                ...
                views=int(impressions * 0.85),
                likes=int(impressions * 0.04),
                comments=int(impressions * 0.008),
                shares=int(impressions * 0.005),
                saves=int(impressions * 0.003),
                clicks=clicks,
                ...
                engagement_rate=round(
                    (int(impressions * 0.04) + int(impressions * 0.008) + int(impressions * 0.005)) / impressions, 4
                ),
            )
            metrics.append(pm)
        db.add_all(metrics)
        ...
        memories = [
            MemoryEntry(
                ...
                category="winner",
                key="winner:budget_hacks",
                value="Budget hacks topic consistently outperforms",
                structured_value={"avg_rpm": 3.80, "avg_engagement": 0.053},
                confidence=0.8,
            ),
            MemoryEntry(
                brand_id=bid,
                ...
                category="preference",
                key="audience:prefers_data",
                value="Audience responds best to data-driven content",
                structured_value={"signal_count": 12},
                confidence=0.75,
            ),
            MemoryEntry(
                brand_id=bid,
                memory_type="platform_learning",
                category="timing",
                key="youtube:best_time",
                value="Tuesday/Thursday 9-11am EST performs best",
                structured_value={"best_days": ["tuesday", "thursday"], "best_hours": [9, 10, 11]},
                confidence=0.65,
            ),
        ]
        db.add_all(memories)
