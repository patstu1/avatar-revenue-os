        ...
        actions.append(
            {
                ...
                "diagnostics_json": {
                    "leak_focus": "mid_funnel_drop",
                    "recommended_fixes": ["headline", "social_proof", "form_fields"],
                },
                APCE: True,
            }
        )
...
def compute_self_healing_action(incident: dict[str, Any]) -> dict[str, Any]:
    ...
    routing: dict[str, tuple[str, str, str, str, float]] = {
        "provider_failure": (
            ...
            "guarded",
            "operator_review",
            "Restore throughput via backup provider",
            0.75,
        ),
        "publishing_failures": (
            "retry_with_backoff",
            "guarded",
            "operator_review",
            "Reduce failure rate within 24h",
            0.7,
        ),
        "conversion_drop": (
            "throttle_paid_and_test_funnel",
            "guarded",
            "none",
            "Stabilize CAC while patching funnel",
            0.62,
        ),
        "fatigue_spike": (
            "suppress_output_rotate_creative",
            "autonomous",
            "none",
            "Refresh creative mix and cadence",
            0.68,
        ),
        "queue_congestion": (
            "throttle_enqueue_split_queue",
            "autonomous",
            "none",
            "Relieve congestion via fair scheduling",
            0.72,
        ),
        "budget_overspend": (
            "pause_spend_cap",
            "autonomous",
            "immediate_operator",
            "Halt spend until operator confirms",
            0.85,
        ),
        "sponsor_underperformance": (
            "adjust_package_pitch",
            "guarded",
            "none",
            "Repackage inventory and targets",
            0.55,
        ),
        "account_stagnation": (
            "shift_allocation_test_lane",
            "guarded",
            "none",
            "Reallocate to warmer account mix",
            0.58,
        ),
        "weak_offer_economics": (
            "suppress_offer_queue_review",
            "guarded",
            "operator_review",
            "Pause scale until economics fixed",
            0.65,
        ),
    }
