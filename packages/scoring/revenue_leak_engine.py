...
LEAK_TYPES = [
    ...
    "high_engagement_low_monetization",
    "weak_landing_page",
    "weak_cta_path",
    "wrong_destination",
    "weak_offer_selection",
    "weak_affiliate_choice",
    "underused_winner",
    "weak_followup",
    "blocked_provider",
    "weak_upsell_path",
    "under_monetized_account",
    "under_monetized_platform",
]

LEAK_ACTIONS = {
    "high_impressions_low_ctr": {
        "action": "Revise hook/thumbnail — high reach but no clicks",
        "target": "content_generation",
        "priority": "high",
    },
    "high_clicks_low_conversion": {
        "action": "Fix landing page or offer — clicks aren't converting",
        "target": "landing_page_engine",
        "priority": "critical",
    },
    "high_engagement_low_monetization": {
        "action": "Add monetization path — audience is engaged but not buying",
        "target": "offer_lab",
        "priority": "high",
    },
    "weak_landing_page": {
        "action": "Rebuild landing page — conversion rate below threshold",
        "target": "landing_page_engine",
        "priority": "high",
    },
    "weak_cta_path": {
        "action": "Test different CTA style — current CTA underperforming",
        "target": "campaign_constructor",
        "priority": "medium",
    },
    "wrong_destination": {
        "action": "Route to better landing page for this audience/offer",
        "target": "landing_page_engine",
        "priority": "high",
    },
    "weak_offer_selection": {
        "action": "Switch to higher-ranked offer for this content",
        "target": "offer_lab",
        "priority": "high",
    },
    "weak_affiliate_choice": {
        "action": "Switch affiliate merchant/network for better EPC",
        "target": "affiliate_intel",
        "priority": "medium",
    },
    "underused_winner": {
        "action": "Scale winning content/offer — not getting enough volume",
        "target": "capital_allocator",
        "priority": "high",
    },
    "weak_followup": {
        "action": "Improve email/SMS follow-up sequence",
        "target": "campaign_constructor",
        "priority": "medium",
    },
    "blocked_provider": {
        "action": "Fix blocked provider preventing revenue execution",
        "target": "provider_registry",
        "priority": "critical",
    },
    "weak_upsell_path": {"action": "Add or improve upsell/downsell path", "target": "offer_lab", "priority": "medium"},
    "under_monetized_account": {
        "action": "Increase monetization intensity on this account",
        "target": "account_state",
        "priority": "high",
    },
    "under_monetized_platform": {
        "action": "Expand monetization strategy on this platform",
        "target": "capital_allocator",
        "priority": "medium",
    },
}
        ...
        if imp > 0 and clicks / max(imp, 1) < 0.01:
            leaks.append(
                _leak(
                    ...
                    imp * 0.02,
                    0.7,
                    {"impressions": imp, "ctr": clicks / max(imp, 1)},
                )
            )
        if clicks > 100 and cvr < 0.005:
            leaks.append(
                _leak(
                    "high_clicks_low_conversion", "content_item", ci, clicks * 0.5, 0.8, {"clicks": clicks, "cvr": cvr}
                )
            )
        ...
        if rank > 0.6 and int(offer.get("usage_count", 0)) < 3:
            leaks.append(
                _leak(
                    ...
                    rank * 100,
                    0.7,
                    {"rank": rank, "usage": offer.get("usage_count")},
                )
            )
        ...
        if acct.get("state") in ("scaling", "monetizing") and float(acct.get("revenue", 0)) < 10:
            leaks.append(
                _leak(
                    ...
                    50,
                    0.6,
                    {"state": acct.get("state"), "revenue": acct.get("revenue")},
                )
            )
...
def generate_corrections(leaks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    corrections = []
    for l in leaks:
        action_info = LEAK_ACTIONS.get(
            l["leak_type"], {"action": "Investigate", "target": "copilot", "priority": "medium"}
        )
        corrections.append(
            {
