...
PLAYBOOKS = {
    "provider_failure": {
        "steps": [
            {"action": "reroute", "target": "fallback_provider"},
            {"action": "throttle", "level": "50pct"},
    ...
    "publish_failure": {
        "steps": [
            {"action": "retry", "max": 3},
            {"action": "rollback", "target": "draft_status"},
            {"action": "alert_operator"},
        ],
        "auto_execute": True,
    },
    "bad_scaling_push": {
        "steps": [
            {"action": "throttle", "level": "25pct"},
            {"action": "rollback", "target": "previous_cadence"},
            {"action": "alert_operator"},
        ],
        "auto_execute": True,
    },
    "experiment_failure_cluster": {
        "steps": [
            {"action": "suppress_experiment"},
            {"action": "rollback", "target": "control_variant"},
            {"action": "alert_operator"},
        ],
        "auto_execute": True,
    },
    "broken_routing_path": {
        "steps": [{"action": "reroute", "target": "default_path"}, {"action": "alert_operator"}],
        "auto_execute": True,
    },
    "weak_content_push": {
        "steps": [
            {"action": "throttle", "level": "50pct"},
            {"action": "suppress_content_family"},
            {"action": "alert_operator"},
        ],
        "auto_execute": False,
    },
    "campaign_failure": {
        "steps": [
            ...
            {"action": "reroute", "target": "safer_campaign"},
            {"action": "alert_operator"},
        ],
        "auto_execute": True,
    },
    "dependency_outage": {
        "steps": [
            {"action": "reroute", "target": "fallback"},
            {"action": "throttle", "level": "minimal"},
            {"action": "escalate"},
        ],
        "auto_execute": True,
    },
    "retry_exhaustion": {
        "steps": [{"action": "rollback", "target": "last_good_state"}, {"action": "alert_operator"}],
        "auto_execute": True,
    },
...
def detect_incidents(system_state: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for pf in system_state.get("provider_failures", []):
        incidents.append(
            _incident(
                ...
                "provider",
                pf,
                f"Provider {pf.get('name', '')} failed: {pf.get('error', '')}",
                True,
            )
        )
    ...
    for pf in system_state.get("publish_failures", []):
        incidents.append(
            _incident(
                ...
                "content_item",
                pf,
                f"Publish failed for content {pf.get('id', '')[:8]}",
                True,
            )
        )
    ...
    for bf in system_state.get("bad_scaling", []):
        incidents.append(
            _incident(
                ...
                "account",
                bf,
                f"Account {bf.get('name', '')} health dropped after scaling push",
                True,
            )
        )
    ...
    for ef in system_state.get("experiment_failures", []):
        incidents.append(
            _incident(
                ...
                "experiment",
                ef,
                f"Experiment cluster failure on {ef.get('variable', '')}",
                True,
            )
        )
    ...
    for brk in system_state.get("broken_routes", []):
        incidents.append(
            _incident("broken_routing_path", "high", "routing", brk, f"Route broken: {brk.get('path', '')}", True)
        )

    for dep in system_state.get("dependency_outages", []):
        incidents.append(
            _incident(
                "dependency_outage", "critical", "dependency", dep, f"Dependency outage: {dep.get('name', '')}", True
            )
        )
    ...
    if system_state.get("unsafe_state"):
        incidents.append(
            _incident(
                ...
                "system",
                {},
                "System in unsafe state — operator escalation required",
                False,
            )
        )
...
def decide_recovery(incident: dict[str, Any], playbook: dict[str, Any] = None) -> dict[str, Any]:
    ...
    for step in playbook.get("steps", []):
        ...
        if action == "rollback":
            actions.append(
                {
                    "type": "rollback",
                    "target": step.get("target", "previous_state"),
                    "detail": f"Roll back {incident['affected_scope']} to {step.get('target', 'safe state')}",
                }
            )
        elif action == "reroute":
            actions.append(
                {
                    "type": "reroute",
                    "from": incident.get("affected_scope", ""),
                    "to": step.get("target", "fallback"),
                    "detail": f"Reroute from {incident['affected_scope']} to {step.get('target', 'fallback')}",
                }
            )
            ...
            actions.append(
                {
                    "type": "throttle",
                    "target": incident.get("affected_scope", ""),
                    "level": step.get("level", "50pct"),
                    "detail": f"Throttle {incident['affected_scope']} to {step.get('level', '50%')}",
                }
            )
            ...
            actions.append(
                {
                    "type": "retry",
                    "max_retries": step.get("max", 3),
                    "detail": f"Retry up to {step.get('max', 3)} times",
                }
            )
    ...
    return {
        "decision": "auto_recover" if auto else "operator_review",
        "actions": actions,
        "reason": f"Playbook: {incident['incident_type']}",
    }
