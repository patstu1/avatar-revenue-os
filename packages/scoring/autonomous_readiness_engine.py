...
ACTIVATION_CHECKLIST: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "code_path_live": True,
        "additional_config": "Create HeyGen avatar + clone voice in HeyGen dashboard",
    },
    {
        "provider": "ElevenLabs",
        ...
        "code_path_live": True,
        "additional_config": "Clone brand voice in ElevenLabs dashboard",
    },
    {
        "provider": "Fish Audio",
    ...
    {
        "provider": "SMTP Email",
        "env_vars": ["SMTP_HOST", "SMTP_FROM_EMAIL"],
        "priority": 3,
...
def evaluate_autonomous_readiness() -> dict[str, Any]:
    ...
    conditions.append(
        {
            "id": 1,
            "name": "Critical providers configured",
            "passed": len(critical_missing) == 0,
            "detail": f"Missing: {', '.join(critical_missing)}"
            if critical_missing
            else "All critical providers configured",
        }
    )
    ...
    conditions.append(
        {
            "id": 2,
            "name": "No critical dead-end flows",
            "passed": False,
            "not_verified": True,
    ...
    conditions.append(
        {
            "id": 3,
            "name": "Publishing auto-runs through distribution layer",
            "passed": has_distributor,
            "detail": f"Multi-distributor active ({', '.join(k.replace('_API_KEY', '') for k in ['BUFFER_API_KEY', 'PUBLER_API_KEY', 'AYRSHARE_API_KEY'] if os.environ.get(k))})"
            if has_distributor
            else "Set BUFFER_API_KEY, PUBLER_API_KEY, or AYRSHARE_API_KEY to enable auto-publishing",
        }
    )
    ...
    conditions.append(
        {
            "id": 4,
            "name": "Measured performance data ingested on schedule",
            "passed": has_distributor,
            "detail": "analytics_worker.ingest_performance runs every 30min"
            if has_distributor
            else "No data sources active — configure at least one distribution service",
        }
    )
    ...
    conditions.append(
        {
            "id": 5,
            "name": "Offer economics updated from measured data",
            "passed": False,
            "not_verified": True,
    ...
    conditions.append(
        {
            "id": 6,
            "name": "Expansion advisor runs on schedule and feeds downstream",
            "passed": False,
            "not_verified": True,
    ...
    conditions.append(
        {
            "id": 7,
            "name": "Kill/scale actions surfaced automatically",
            "passed": False,
            "not_verified": True,
    ...
    conditions.append(
        {
            "id": 8,
            "name": "Gatekeeper has no critical acceptance blockers",
            "passed": False,
            "not_verified": True,
    ...
    conditions.append(
        {
            "id": 9,
            "name": "Truth labels correctly show live/queued/blocked/recommendation-only",
            "passed": False,
            "not_verified": True,
    ...
    conditions.append(
        {
            "id": 10,
            "name": "Tiered routing consumed by downstream generation/distribution",
            "passed": False,
            "not_verified": True,
    ...
    if unverified:
        verdict = (
            f"NOT YET AUTONOMOUS — {len(unverified)} condition(s) not yet verified, {len(conditions) - passing - len(unverified)} verified condition(s) failing"
            if not verified_pass
            else f"NOT YET AUTONOMOUS — {len(unverified)} condition(s) not yet verified (all verified checks pass)"
        )
        fully = False
    else:
        fully = all(c["passed"] for c in conditions)
        verdict = (
            "FULLY AUTONOMOUS FOR CURRENT SCOPE"
            if fully
            else f"NOT YET AUTONOMOUS — {len(conditions) - passing} condition(s) failing"
        )
