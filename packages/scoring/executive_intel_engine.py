...
def rollup_kpis(
    revenue_data: dict, content_data: dict, performance_data: dict, account_data: dict, campaign_data: dict
) -> dict[str, Any]:
...
def forecast_metric(historical_values: list[float], periods_ahead: int = 1) -> dict[str, Any]:
    ...
    if len(historical_values) < 2:
        return {
            "predicted_value": historical_values[-1] if historical_values else 0,
            "confidence": 0.2,
    ...
    stability = 1.0 - min(
        1.0,
        sum(abs(historical_values[i] - historical_values[i - 1]) for i in range(1, n)) / max(1, n * max(abs(last), 1)),
    )
...
def compute_uptime(
    provider_key: str, total_requests: int, failed_requests: int, avg_latency_ms: float
) -> dict[str, Any]:
...
def evaluate_oversight(auto_count: int, human_count: int, override_count: int) -> dict[str, Any]:
    ...
    return {
        ...
        "ai_accuracy_estimate": accuracy,
        "recommendation": rec,
    }


def generate_executive_alerts(
    kpis: dict, forecasts: list[dict], uptime: list[dict], oversight: dict
) -> list[dict[str, Any]]:
        ...
        alerts.append(
            {
                ...
                "detail": "Total revenue is $0 — check revenue tracking and offer activity",
                "recommended_action": "Verify affiliate/revenue integrations",
            }
        )
    ...
    for f in forecasts:
        if f.get("trend", 0) < -0.15:
            alerts.append(
                {
                    "alert_type": "declining_forecast",
                    "severity": "high",
                    "title": f"Declining {f.get('forecast_type', '')} forecast",
                    "detail": f"Trend is {f.get('trend', 0):.1%} with confidence {f.get('confidence', 0):.0%}",
                    "recommended_action": "Investigate root cause and adjust strategy",
                }
            )
    ...
    for u in uptime:
        if u.get("reliability_grade") in ("D", "F"):
            alerts.append(
                {
                    "alert_type": "provider_reliability",
                    "severity": "high",
                    "title": f"Provider {u['provider_key']} reliability grade {u['reliability_grade']}",
                    "detail": f"Uptime {u['uptime_pct']:.1f}%, {u['failed_requests']} failures",
                    "recommended_action": "Switch to backup provider or investigate failures",
                }
            )
    ...
    if oversight.get("ai_accuracy_estimate", 1) < 0.75:
        alerts.append(
            {
                "alert_type": "ai_accuracy_low",
                "severity": "high",
                "title": "AI accuracy below threshold",
                "detail": f"Accuracy estimate {oversight.get('ai_accuracy_estimate', 0):.0%}",
                "recommended_action": oversight.get("recommendation", "Increase human review"),
            }
        )
