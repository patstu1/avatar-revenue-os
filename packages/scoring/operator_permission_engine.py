...
def evaluate_action_policy(
    action_class: str, matrix: list[dict[str, Any]], policies: list[dict[str, Any]]
) -> dict[str, Any]:
    ...
    for m in matrix:
        if m.get("action_class") == action_class and m.get("is_active", True):
            return {
                ...
                "mode": m["autonomy_mode"],
                "approval_role": m.get("approval_role"),
                "override_allowed": m.get("override_allowed", True),
                "override_role": m.get("override_role"),
                "source": "matrix",
            }
    ...
    default = DEFAULT_POLICIES.get(
        action_class, {"mode": "guarded_approval", "approval_role": "org_admin", "override_role": "super_admin"}
    )
    return {
        "action_class": action_class,
        "mode": default["mode"],
        "approval_role": default["approval_role"],
        "override_allowed": True,
        "override_role": default["override_role"],
        "source": "default",
    }
...
def can_execute_autonomously(action_class: str, matrix: list[dict], policies: list[dict]) -> dict[str, Any]:
    ...
    if mode == "autonomous_notify":
        return {
            ...
            "reason": "Autonomous with notification",
        }
    if mode == "guarded_approval":
        return {
            ...
            "needs_approval": True,
            "needs_notification": True,
            "reason": f"Requires approval from {policy.get('approval_role', 'admin')}",
        }
    return {
        "allowed": False,
        "needs_approval": True,
        "needs_notification": True,
        "reason": "Manual only — operator must execute",
    }


def evaluate_override_eligibility(
    action_class: str, user_role: str, matrix: list[dict], overrides: list[dict]
) -> dict[str, Any]:
        ...
        if len(active) > 1:
            ...
            if len(modes) > 1:
                conflicts.append(
                    {
                        "action_class": ac,
                        "conflict": f"Multiple active entries with different modes: {modes}",
                        "entries": len(active),
                    }
                )
    return conflicts
