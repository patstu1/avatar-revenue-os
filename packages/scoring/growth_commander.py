...
def _build_execution_spec(command_type: str, cmd: dict, accounts: list[dict], brand_niche: str | None) -> dict:
    ...
    if command_type in ("shift_platform", "shift_niche"):
        return {
            ...
            "niche": nf.get("niche") or brand_niche,
            "sub_niche": nf.get("sub_niche") or "",
            "avatar_persona_strategy": ev.get("avatar_persona_strategy")
            or "Differentiate from flagship; clear sub-audience.",
...
def _finalize_command_metadata(commands: list[dict], accounts: list[dict], brand_niche: str | None) -> None:
    for c in commands:
        ...
        c.setdefault(
            "required_resources",
            _required_resources(
                float(c.get("expected_cost", 0)),
                float(c.get("urgency", 0)),
                ct,
            ),
        )
...
def compute_portfolio_directive(
    ...
    downstream = (
        ...
        if top
        else "Recompute when new performance data is available."
    )
...
def _success_threshold(command_type: str, expected_upside: float) -> dict:
    if command_type == "launch_account":
        return {
            ...
            "target_value": round(max(10, expected_upside * 0.3), 2),
            "timeframe_days": 60,
        }
    if command_type == "increase_output":
...
def _failure_threshold(command_type: str) -> dict:
    if command_type == "launch_account":
        return {
            ...
            "floor_value": -20.0,
            "timeframe_days": 90,
            "action_on_failure": "pause_account",
        }
    if command_type == "increase_output":
        return {
            "metric": "rpm_decline_pct",
            "floor_value": -15.0,
            "timeframe_days": 45,
            "action_on_failure": "revert_output_volume",
        }
            ...
            overbuilt.append(
                {
                    ...
                    "count": count,
                    "share": round(share, 2),
                    "reason": "Over-concentrated — diversification reduces risk.",
                }
            )
        elif count == 1 and total >= 3:
            underbuilt.append(
                {"platform": p, "count": count, "reason": "Single account on active platform — opportunity to expand."}
            )
...
def find_whitespace(
    ...
    used = {
        (a.get("platform", "").lower(), (a.get("niche_focus") or "").lower(), (a.get("geography") or "US").upper())
        for a in accounts
    }
...
def generate_growth_commands(
        ...
        commands.append(
            {
                ...
                "niche_fit": {
                    "niche": brand_niche or "",
                    "sub_niche": "",
                    "fit_score": 0,
                    "reason": "N/A — funnel fix required.",
                },
                ...
                "success_threshold": {
                    "metric": "leak_count",
                    "target_value": max(0, leak_count - 3),
                    "timeframe_days": 14,
                },
                "failure_threshold": {
                    "metric": "leak_count",
                    "floor_value": leak_count + 2,
                    "timeframe_days": 30,
                    "action_on_failure": "escalate_funnel_audit",
                },
                "expected_upside": 0,
                "expected_cost": 0,
                "expected_time_to_signal_days": 7,
                "expected_time_to_profit_days": 0,
                "confidence": 0.9,
                "urgency": 90.0,
    ...
    if (
        ...
        and not (leak_count > 5 and high_severity_blockers)
    ):
        commands.append(
            {
                "command_type": "fix_funnel_first",
                ...
                "niche_fit": {
                    ...
                    "sub_niche": "",
                    "fit_score": 0,
                    "reason": "N/A — funnel readiness.",
                },
                "monetization_path": {"primary_method": "fix_existing", "secondary_method": "", "expected_rpm": 0},
                "cannibalization_analysis": {"risk": 0, "overlap_accounts": [], "mitigation": "N/A"},
                "success_threshold": {"metric": "launch_readiness_score", "target_value": 55, "timeframe_days": 21},
                "failure_threshold": {
                    "metric": "launch_readiness_score",
                    "floor_value": 40,
                    "timeframe_days": 30,
                    "action_on_failure": "deep_funnel_audit",
                },
                "expected_upside": 0,
                "expected_cost": 0,
                ...
                "evidence": {
                    ...
                    "gate": "funnel_readiness",
                    "recommended_action": (readiness or {}).get("recommended_action"),
                },
                COMMANDER_SOURCE: True,
            }
        )
        ...
        commands.append(
            {
                ...
                "niche_fit": {
                    ...
                    "sub_niche": "",
                    "fit_score": 0.5,
                    "reason": "Niche supports multiple offer types.",
                },
                ...
                "failure_threshold": {
                    "metric": "active_offers",
                    "floor_value": 1,
                    "timeframe_days": 30,
                    "action_on_failure": "prioritize_offer_sourcing",
                },
                "expected_upside": inc_vol * 0.5,
                "expected_cost": 50,
                "expected_time_to_signal_days": 14,
    ...
    if (
        ...
        and rec_key == RK_ADD_PLATFORM_SPECIFIC
        and tgt_plat
        and not has_launch_on_tgt
        ...
        commands.append(
            {
                ...
                "rationale": bna.get("rationale")
                or "Diversify platform exposure while reusing proven creative patterns.",
                "comparison": comparison,
                "platform_fit": {
                    "platform": tgt_plat,
                    "fit_score": round(pfit, 2),
                ...
                "monetization_path": {
                    "primary_method": "mirror_flagship",
                    "secondary_method": "",
                    "expected_rpm": round(inc_new / 4, 2) if inc_new > 0 else 5.0,
                },
                "cannibalization_analysis": {
                    "risk": 0.35,
                    "overlap_accounts": [a.get("username", "?") for a in existing_on_tgt][:3],
                ...
                "first_week_plan": _first_week_plan(
                    {
                        "primary_platform": tgt_plat,
                        "niche": brand_niche or "core",
    ...
    if (
        not expansion_blocked
        and rec_key == RK_ADD_NICHE_SPINOFF
        and not candidates
        and len(accounts) >= 1
        and inc_new >= inc_vol * 0.85
    ):
        ...
        commands.append(
            {
                ...
                "platform_fit": {
                    "platform": plat0,
                    "fit_score": 0.75,
                    "reason": "Same platform family — faster creative reuse.",
                },
                "niche_fit": {
                    "niche": brand_niche or "",
                    "sub_niche": spin,
                    "fit_score": 0.8,
                    "reason": "Explicit spinoff path from scale engine.",
                },
                "monetization_path": {
                    "primary_method": "affiliate",
                    "secondary_method": "",
                    "expected_rpm": round(inc_new / 4, 2) if inc_new > 0 else 5.0,
                },
                "cannibalization_analysis": {
                    "risk": 0.4,
                    "overlap_accounts": [accounts[0].get("username", "?")],
    ...
    for candidate in candidates:
        ...
        overlap_accounts = [
            a.get("username", a.get("id", "?"))
            for a in accounts
            if (
                a.get("platform", "").lower() == platform.lower()
                and (a.get("niche_focus") or "").lower() == niche.lower()
            )
        ]
        ...
        commands.append(
            {
                ...
                "platform_fit": {
                    "platform": platform,
                    "fit_score": round(platform_fit_score, 2),
                    "reason": f"{'New platform — max opportunity.' if len(existing_on_platform) == 0 else f'{len(existing_on_platform)} existing on {platform}.'}",
                },
                "niche_fit": {
                    "niche": niche,
                    "sub_niche": candidate.get("sub_niche", ""),
                    "fit_score": round(min(1.0, aud_sep + 0.2), 2),
                    "reason": f"Audience separation {aud_sep:.2f}.",
                },
                "monetization_path": {
                    "primary_method": (candidate.get("monetization_path") or "affiliate").split("|")[0].strip(),
                    "secondary_method": "",
                    "expected_rpm": round(inc_new / 4, 2) if inc_new > 0 else 5.0,
                },
                "cannibalization_analysis": {
                    "risk": round(cann_risk, 3),
                    "overlap_accounts": overlap_accounts[:3],
        ...
        commands.append(
            {
                ...
                "platform_fit": {
                    ...
                    "fit_score": 0.9,
                    "reason": "Already performing on this platform.",
                },
                "niche_fit": {
                    "niche": best.get("niche_focus", ""),
                    "sub_niche": "",
                    "fit_score": 0.9,
                    "reason": "Proven niche performance.",
                },
                "monetization_path": {
                    "primary_method": "existing",
                    "secondary_method": "",
                    "expected_rpm": float(best.get("revenue_per_mille", 0)),
                },
                "cannibalization_analysis": {
                    "risk": 0,
                    "overlap_accounts": [],
                    "mitigation": "Same account — no cannibalization.",
                },
                ...
                "expected_cost": 0,
                "expected_time_to_signal_days": 14,
                "expected_time_to_profit_days": 14,
                "confidence": 0.8,
                "urgency": 65.0,
                "blocking_factors": blocking_factors,
                "first_week_plan": [],
            ...
            commands.append(
                {
                    ...
                    "platform_fit": {
                        ...
                        "fit_score": 0,
                        "reason": "Account underperforming.",
                    },
                    ...
                    "evidence": {
                        "health": health,
                        "profit_per_post": ppp,
                        "fatigue": float(a.get("fatigue_score", 0)),
        ...
        commands.append(
            {
                ...
                "failure_threshold": {
                    "metric": "audience_churn_pct",
                    "floor_value": 15,
                    "timeframe_days": 30,
                    "action_on_failure": "re-evaluate_merge",
                },
        ...
        commands.append(
            {
                ...
                "failure_threshold": {
                    "metric": "profit_decline_pct",
                    "floor_value": -10,
                    "timeframe_days": 30,
                    "action_on_failure": "re-evaluate",
                },
                "expected_upside": 0,
                "expected_cost": 0,
