...
RESPONSE_ANGLES = {
    "price": {
        "content_angle": "Value demonstration — show ROI/savings over time",
        "cta_angle": "Risk-free trial or money-back guarantee CTA",
        "offer_angle": "budget",
    },
    "trust": {
        "content_angle": "Social proof — testimonials, case studies, transparent results",
        "cta_angle": "Low-commitment first step CTA",
        "offer_angle": "trust-first",
    },
    "complexity": {
        "content_angle": "Simplification — step-by-step walkthrough, beginner-friendly",
        "cta_angle": "Guided setup or demo CTA",
        "offer_angle": "convenience",
    },
    "timing": {
        "content_angle": "Urgency + patience balance — show cost of waiting",
        "cta_angle": "Save-for-later / reminder CTA",
        "offer_angle": "timing-sensitive",
    },
    "competitor": {
        "content_angle": "Honest comparison — show unique differentiators",
        "cta_angle": "Side-by-side comparison CTA",
        "offer_angle": "comparison",
    },
    "relevance": {
        "content_angle": "Segment-specific content — address exact use case",
        "cta_angle": "Quiz or segment-finder CTA",
        "offer_angle": "relevance-targeted",
    },
    "proof": {
        "content_angle": "Data-heavy proof — numbers, screenshots, verifiable results",
        "cta_angle": "Case study or results-first CTA",
        "offer_angle": "proof-led",
    },
    "identity": {
        "content_angle": "Identity matching — show aspirational users like them",
        "cta_angle": "Community or identity-aligned CTA",
        "offer_angle": "identity",
    },
    "skepticism": {
        "content_angle": "Transparency content — show behind-the-scenes, limitations",
        "cta_angle": "No-hype honest assessment CTA",
        "offer_angle": "transparency",
    },
}
...
def generate_responses(clusters: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for c in clusters:
        ...
        priority = (
            "critical"
            if c["avg_monetization_impact"] > 0.8
            else "high"
            if c["avg_monetization_impact"] > 0.6
            else "medium"
        )
        ...
        for channel in channels:
            angle_key = (
                "content_angle"
                if channel == "content_brief"
                else "cta_angle"
                if channel == "cta_generation"
                else "content_angle"
            )
            responses.append(
                {
        ...
        top.append(
            {
                ...
                "angle": c.get("recommended_response_angle", ""),
            }
        )
