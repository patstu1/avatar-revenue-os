...
def run_agent_cycle(
    ...
    add(
        "niche_allocator",
        {
            ...
            "recommendation": "shift_allocation_to_healthier_accounts"
            if health < 0.5
        ...
        msgs=[
            {
                ...
                "explanation": f"Avg health {health:.2f}, recommending rebalance."
                if health < 0.5
                else "Allocation stable.",
            }
        ],
    )
    ...
    add(
        "recovery_agent",
        {
            ...
            "recommendation": "reroute_provider"
            if prov_fail
            else ("reduce_suppressions" if supp_count > 3 else "healthy"),
        },
    )
...
def compute_revenue_pressure(brand_snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    if underused:
        commands.append(
            {
                "action": f"activate_monetization_{underused[0]}",
                "priority": "high",
    ...
    if underbuilt:
        commands.append(
            {
                "action": f"build_on_{underbuilt[0]}",
                "priority": "medium",
    ...
    if supp_count > 3:
        commands.append(
            {
                "action": "investigate_suppression_cluster",
                "priority": "medium",
    ...
    if rev_trend == "down":
        commands.append(
            {
                "action": "emergency_revenue_recovery",
                "priority": "critical",
...
def compute_override_policies(
    ...
    for ref in action_refs:
        mode, approval, rollback, plan = _RISK_MAP.get(ref, ("guarded", False, False, None))
...
def generate_escalations(
    ...
    for b in blockers:
        ...
        if b["blocker"] == "funnel_blocked":
            upside = float(
                b.get("consequence_if_ignored", "0").replace("$", "").split("/")[0].replace("Estimated", "").strip()
                or 0
            )
