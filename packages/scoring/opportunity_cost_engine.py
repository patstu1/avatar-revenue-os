...
ACTION_TYPES = [
    "push_volume", "launch_account", "switch_content_form", "promote_winner",
...
def generate_candidates(system_state: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for acct in system_state.get("accounts", []):
        ...
        if state == "scaling":
            candidates.append({"action_type": "push_volume", "action_key": f"volume:{name}", "target_id": aid, "expected_upside": 30, "confidence": 0.7})
        if state == "weak":
            candidates.append({"action_type": "kill_weak_lane", "action_key": f"kill:{name}", "target_id": aid, "expected_upside": 15, "confidence": 0.8})
        if state in ("monetizing", "scaling"):
            candidates.append({"action_type": "activate_monetization", "action_key": f"monetize:{name}", "target_id": aid, "expected_upside": 50, "confidence": 0.6})

    for winner in system_state.get("experiment_winners", []):
        candidates.append({"action_type": "promote_winner", "action_key": f"promote:{winner.get('name', 'exp')}", "target_id": winner.get("id"), "expected_upside": 40, "confidence": float(winner.get("confidence", 0.8))})

    for blocker in system_state.get("blockers", []):
        candidates.append({"action_type": "fix_blocker", "action_key": f"fix:{blocker.get('name', 'blocker')}", "target_id": blocker.get("id"), "expected_upside": 25, "confidence": 0.9})

    for exp in system_state.get("pending_experiments", []):
        candidates.append({"action_type": "run_experiment", "action_key": f"exp:{exp.get('name', 'test')}", "target_id": exp.get("id"), "expected_upside": 15, "confidence": 0.4})

    for asset in system_state.get("ready_assets", []):
        candidates.append({"action_type": "publish_asset", "action_key": f"publish:{asset.get('title', 'asset')[:30]}", "target_id": asset.get("id"), "expected_upside": 20, "confidence": 0.7})
...
def rank_actions(candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for c in candidates:
        ...
        composite = round(
            0.30 * upside
            + 0.30 * min(1.0, delay["daily_cost"] / 15)
            + 0.25 * urgency
            + 0.15 * confidence,
            4,
        )
        ...
        explanation_parts = [f"{atype}:{akey}", f"upside={upside:.2f}", f"delay=${delay['daily_cost']:.0f}/day", f"urgency={urgency:.2f}", f"confidence={confidence:.2f}"]
        if safe:
