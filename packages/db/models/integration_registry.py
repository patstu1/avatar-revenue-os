...
class IntegrationProvider(Base):
    ...
    extra_config: Mapped[dict | None] = mapped_column(
        JSONB, default=dict, comment="Additional config: webhook_secret, account_id, region, model_name, etc."
    )
    ...
    priority_order: Mapped[int] = mapped_column(
        Integer, default=10, comment="Lower = higher priority. Primary=1, secondary=5, fallback=10"
    )

    # Health
    health_status: Mapped[str] = mapped_column(
        String(20), default="unknown", comment="healthy, degraded, down, unknown, unconfigured"
    )
    ...
    quality_tier: Mapped[str] = mapped_column(
        String(20), default="standard", comment="hero, standard, bulk — determines when this provider is used"
    )
    cost_per_unit: Mapped[float] = mapped_column(
        Float, default=0.0, comment="Cost per token/image/second depending on provider type"
    )
...
class CreatorPlatformAccount(Base):
    ...
    creator_account_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), nullable=True, index=True, comment="FK-less ref to creator_accounts.id"
    )
    ...
    connection_status: Mapped[str] = mapped_column(
        String(30), default="disconnected", comment="connected, disconnected, expired, error"
    )
    connected_via: Mapped[str | None] = mapped_column(String(40), comment="buffer, publer, ayrshare, direct_oauth")
    ...
    account_role: Mapped[str] = mapped_column(
        String(30), default="content", comment="content, sponsor, engagement, growth, test"
    )
    warmup_state: Mapped[str] = mapped_column(
        String(30), default="active", comment="warmup, active, scaling, paused, reduced"
    )
    ...
    __table_args__ = (Index("ix_creator_platform_org_brand", "organization_id", "brand_id"),)
