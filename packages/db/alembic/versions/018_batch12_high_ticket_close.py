...
def upgrade():
    ...
    if not _table_exists("client_high_ticket_profiles"):
        op.create_table(
            ...
            sa.Column("status", sa.String(30), nullable=False, server_default="discovery_pending"),
    ...
    if _table_exists("client_retention_events") and not _column_exists("client_retention_events", "amount_cents"):
        op.add_column(
...
def downgrade():
    if _table_exists("client_retention_events") and _column_exists("client_retention_events", "amount_cents"):
