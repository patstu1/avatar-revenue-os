...
def upgrade():
    ...
    if not _table_exists("invoices"):
        ...
        op.create_index("ix_invoices_avenue_slug", "invoices", ["avenue_slug"])
    ...
    if not _table_exists("invoice_milestones"):
        ...
        op.create_index("ix_invoice_milestones_status", "invoice_milestones", ["status"])
    ...
    if not _table_exists("sponsor_campaigns"):
        op.create_table(
            "sponsor_campaigns",
            sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
            sa.Column("client_id", UUID(as_uuid=True), sa.ForeignKey("clients.id"), nullable=False, unique=True),
            sa.Column("org_id", UUID(as_uuid=True), sa.ForeignKey("organizations.id"), nullable=False),
            sa.Column("brand_id", UUID(as_uuid=True), sa.ForeignKey("brands.id"), nullable=True),
            sa.Column("sponsor_opportunity_id", UUID(as_uuid=True), nullable=True),
            sa.Column("avenue_slug", sa.String(60), nullable=False, server_default="sponsor_deals"),
            # status: pre_contract / contract_signed / brief_received /
            #         campaign_live / campaign_complete / cancelled
            sa.Column("status", sa.String(30), nullable=False, server_default="pre_contract"),
            ...
            sa.Column("brief_received_at", sa.DateTime(timezone=True), nullable=True),
            sa.Column("campaign_start_at", sa.DateTime(timezone=True), nullable=True),
            sa.Column("campaign_end_at", sa.DateTime(timezone=True), nullable=True),
        ...
        op.create_index("ix_sponsor_campaigns_org_status", "sponsor_campaigns", ["org_id", "status"])
        op.create_index("ix_sponsor_campaigns_start_at", "sponsor_campaigns", ["campaign_start_at"])
    ...
    if not _table_exists("sponsor_placements"):
        op.create_table(
            "sponsor_placements",
            sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
            sa.Column("campaign_id", UUID(as_uuid=True), sa.ForeignKey("sponsor_campaigns.id"), nullable=False),
            sa.Column("org_id", UUID(as_uuid=True), sa.ForeignKey("organizations.id"), nullable=False),
            sa.Column("position", sa.Integer(), nullable=False, server_default="0"),
            ...
            sa.Column("make_good_of_placement_id", UUID(as_uuid=True), nullable=True),
        ...
        op.create_index("ix_sponsor_placements_campaign_id", "sponsor_placements", ["campaign_id"])
        op.create_index("ix_sponsor_placements_status", "sponsor_placements", ["status"])
        op.create_index("ix_sponsor_placements_scheduled_at", "sponsor_placements", ["scheduled_at"])
        op.create_index("ix_sponsor_placements_makegood", "sponsor_placements", ["make_good_of_placement_id"])
    ...
    if not _table_exists("sponsor_reports"):
        op.create_table(
            ...
            sa.Column("report_type", sa.String(30), nullable=False, server_default="monthly"),
        ...
        op.create_index("ix_sponsor_reports_campaign_id", "sponsor_reports", ["campaign_id"])
        op.create_index("ix_sponsor_reports_period", "sponsor_reports", ["period_start", "period_end"])
