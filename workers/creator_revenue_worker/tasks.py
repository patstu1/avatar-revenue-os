...
@shared_task(name="workers.creator_revenue_worker.tasks.recompute_creator_revenue")
def recompute_creator_revenue() -> str:
    asyncio.run(_run_all_brands(_recompute_opps))
    return "creator-revenue-opps-done"


@shared_task(name="workers.creator_revenue_worker.tasks.recompute_ugc_services")
def recompute_ugc_services() -> str:
    asyncio.run(_run_all_brands(_recompute_ugc))
    return "ugc-services-done"


@shared_task(name="workers.creator_revenue_worker.tasks.recompute_service_consulting")
def recompute_service_consulting() -> str:
    asyncio.run(_run_all_brands(_recompute_consulting))
    return "service-consulting-done"


@shared_task(name="workers.creator_revenue_worker.tasks.recompute_premium_access")
def recompute_premium_access() -> str:
    asyncio.run(_run_all_brands(_recompute_premium))
    return "premium-access-done"


@shared_task(name="workers.creator_revenue_worker.tasks.recompute_licensing")
def recompute_licensing_task() -> str:
    asyncio.run(_run_all_brands(_recompute_licensing))
    return "licensing-done"


@shared_task(name="workers.creator_revenue_worker.tasks.recompute_syndication")
def recompute_syndication_task() -> str:
    asyncio.run(_run_all_brands(_recompute_syndication))
    return "syndication-done"


@shared_task(name="workers.creator_revenue_worker.tasks.recompute_data_products")
def recompute_data_products_task() -> str:
    asyncio.run(_run_all_brands(_recompute_data_products))
    return "data-products-done"


@shared_task(name="workers.creator_revenue_worker.tasks.recompute_merch")
def recompute_merch_task() -> str:
    asyncio.run(_run_all_brands(_recompute_merch))
    return "merch-done"


@shared_task(name="workers.creator_revenue_worker.tasks.recompute_live_events")
def recompute_live_events_task() -> str:
    asyncio.run(_run_all_brands(_recompute_live_events))
    return "live-events-done"


@shared_task(name="workers.creator_revenue_worker.tasks.recompute_affiliate_program")
def recompute_affiliate_program_task() -> str:
    asyncio.run(_run_all_brands(_recompute_affiliate_program))
    return "affiliate-program-done"


@shared_task(name="workers.creator_revenue_worker.tasks.recompute_creator_revenue_hub")
def recompute_creator_revenue_hub() -> str:
    asyncio.run(_run_all_brands(_recompute_hub))
    return "creator-revenue-hub-done"


@shared_task(name="workers.creator_revenue_worker.tasks.recompute_creator_revenue_blockers")
