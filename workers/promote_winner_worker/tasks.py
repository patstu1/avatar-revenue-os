...
async def _ingest_experiment_observations():
    ...
    async with get_async_session_factory()() as db:
        experiments = list(
            (await db.execute(select(ActiveExperiment).where(ActiveExperiment.status == "active"))).scalars().all()
        )

        observations_created = 0
        for exp in experiments:
            variants = list(
                (await db.execute(select(PWExperimentVariant).where(PWExperimentVariant.experiment_id == exp.id)))
                .scalars()
                .all()
            )
                ...
                for cid_str in content_ids:
                    try:
                        ...
                        for m in metrics:
                            existing = (
                                await db.execute(
                                    select(PWExperimentObservation)
                                    .where(
                            ...
                            if not existing:
                                db.add(
                                    PWExperimentObservation(
                                        ...
                                        metric_value=float(m.engagement_rate or 0),
                                    )
                                )
                                db.add(
                                    PWExperimentObservation(
                                        ...
                                        metric_value=float(m.impressions or 0),
                                    )
                                )
...
async def _evaluate_active():
    ...
    async with get_async_session_factory()() as db:
        brand_ids = list((await db.execute(select(ActiveExperiment.brand_id).distinct())).scalars().all())
