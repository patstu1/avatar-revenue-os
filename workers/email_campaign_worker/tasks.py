        ...
        try:
            ...
            if direction == "inbound":
                ...
                dr = await create_reply_draft(
                    ...
                    classification_id=ec.id,
                )
                if dr.get("draft_id"):
                    drafts += 1

                # Side effect: mark unsubscribe contacts as unsubscribed even
                # though the policy engine handled the reply (the reply will
                # still auto-send if confidence + allowlist pass).
                if cls_result.intent == "unsubscribe" and contact:
