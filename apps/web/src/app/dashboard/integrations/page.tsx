...
export default function IntegrationsPage() {
  ...
  useEffect(() => { if (!orgId) return; Promise.all([apiFetch<any>(`/api/v1/orgs/${orgId}/integrations/connectors`), apiFetch<any>(`/api/v1/orgs/${orgId}/integrations/clusters`), apiFetch<any>(`/api/v1/orgs/${orgId}/integrations/blockers`)]).then(([c, cl, b]) => { setConnectors(c); setClusters(cl); setBlockers(b); }).catch(() => {}).finally(() => setLoading(false)); }, [orgId]);
