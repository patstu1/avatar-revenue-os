...
export default function AffiliateIntelPage() {
  ...
  useEffect(() => { Promise.all([apiFetch(`/api/v1/brands/${brandId}/affiliate-offers`), apiFetch(`/api/v1/brands/${brandId}/affiliate-leaks`), apiFetch(`/api/v1/brands/${brandId}/affiliate-blockers`)]).then(([o, l, b]) => { setOffers(o); setLeaks(l); setBlockers(b); }).catch(() => {}).finally(() => setLoading(false)); }, []);
