...
async def get_ledger_entries(
    ...
    source_type: str = Query(
        None, description="affiliate_commission, sponsor_payment, service_fee, product_sale, ad_revenue"
    ),
...
async def record_manual_revenue(
    ...
    revenue_source_type: str = Query(
        ..., description="sponsor_payment, service_fee, consulting_fee, product_sale, etc."
    ),
...
async def mark_ledger_paid(
    ...
    entry = (
        await db.execute(select(RevenueLedgerEntry).where(RevenueLedgerEntry.id == ledger_id))
    ).scalar_one_or_none()
    if not entry:
