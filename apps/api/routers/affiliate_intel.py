...
async def recompute(brand_id: uuid.UUID, current_user: OperatorUser, db: DBSession, _rl=Depends(recompute_rate_limit)):
    await _rb(brand_id, current_user, db); r = await svc.recompute_ranking(db, brand_id); await db.commit(); return RecomputeSummaryOut(**r)
...
async def list_links(brand_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _rb(brand_id, current_user, db); return await svc.list_links(db, brand_id)
...
async def list_leaks(brand_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _rb(brand_id, current_user, db); return await svc.list_leaks(db, brand_id)
...
async def ranking(brand_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _rb(brand_id, current_user, db); return await svc.get_best_offer_for_content(db, brand_id)

@router.get("/{brand_id}/affiliate-commissions")
async def commissions(brand_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _rb(brand_id, current_user, db); return await svc.list_commissions(db, brand_id)

@router.get("/{brand_id}/affiliate-payouts")
async def payouts(brand_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _rb(brand_id, current_user, db); return await svc.list_payouts(db, brand_id)

@router.post("/{brand_id}/affiliate-sync")
async def sync_networks(brand_id: uuid.UUID, current_user: OperatorUser, db: DBSession, _rl=Depends(recompute_rate_limit)):
    from apps.api.services.affiliate_sync_service import sync_network_data
    await _rb(brand_id, current_user, db); result = await sync_network_data(db, brand_id); await db.commit(); return result
