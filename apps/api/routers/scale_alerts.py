...
async def recompute_scale_intel_all(
    brand_id: uuid.UUID, current_user: OperatorUser, db: DBSession, _rl=Depends(recompute_rate_limit)
):
    ...
    await log_action(
        db,
        "scale_intel.recomputed_all",
        organization_id=current_user.organization_id,
        brand_id=brand_id,
        user_id=current_user.id,
        actor_type="human",
        entity_type="scale_intel",
        details={"launch_candidates": lc, "alerts": al, "blockers": sb, "readiness": lr},
    )
...
async def recompute_launch_candidates(
    brand_id: uuid.UUID, current_user: OperatorUser, db: DBSession, _rl=Depends(recompute_rate_limit)
):
    await _require_brand(brand_id, current_user, db)
    ...
    await log_action(
        db,
        "launch_candidates.recomputed",
        organization_id=current_user.organization_id,
        brand_id=brand_id,
        user_id=current_user.id,
        actor_type="human",
        entity_type="launch_candidate",
        details=result,
    )
    return result
...
async def recompute_scale_blockers(
    brand_id: uuid.UUID, current_user: OperatorUser, db: DBSession, _rl=Depends(recompute_rate_limit)
):
    await _require_brand(brand_id, current_user, db)
    ...
    await log_action(
        db,
        "scale_blockers.recomputed",
        organization_id=current_user.organization_id,
        brand_id=brand_id,
        user_id=current_user.id,
        actor_type="human",
        entity_type="scale_blocker",
        details=result,
    )
    return result
...
async def recompute_launch_readiness(
    brand_id: uuid.UUID, current_user: OperatorUser, db: DBSession, _rl=Depends(recompute_rate_limit)
):
    await _require_brand(brand_id, current_user, db)
    ...
    await log_action(
        db,
        "launch_readiness.recomputed",
        organization_id=current_user.organization_id,
        brand_id=brand_id,
        user_id=current_user.id,
        actor_type="human",
        entity_type="launch_readiness",
        details=result,
    )
    return result
