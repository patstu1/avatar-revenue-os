...
async def draft_sponsor_outreach(
    current_user: OperatorUser, db: DBSession, brand_id: uuid.UUID = Query(...), sponsor_id: uuid.UUID = Query(...)
):
...
async def draft_proposal(
    current_user: OperatorUser, db: DBSession, brand_id: uuid.UUID = Query(...), deal_id: uuid.UUID = Query(...)
):
...
async def queue_outreach(
    ...
    if "error" in draft:
        return draft
    result = await queue_outreach_sequence(
        db, current_user.organization_id, brand_id, target_type="sponsor_profile", target_id=sponsor_id, draft=draft
    )
    await db.commit()
...
async def auto_create_brief(
    ...
    result = await auto_create_brief_from_decision(
        db, brand_id, decision_class=decision_class, objective=objective, target_platform=target_platform
    )
    await db.commit()
    return result
...
async def ingest_email_reply(
    ...
    result = await ingest_reply(
        db, current_user.organization_id, sender_email=sender_email, subject=subject, body=body, brand_id=brand_id
    )
    await db.commit()
    return result
...
async def get_outreach_brief(
    current_user: CurrentUser, db: DBSession, brand_id: uuid.UUID = Query(...), sponsor_id: uuid.UUID = Query(...)
):
