...
async def roles(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _ro(org_id, current_user, db); return await svc.list_roles(db, org_id)
...
async def seed(org_id: uuid.UUID, current_user: OperatorUser, db: DBSession):
    await _ro(org_id, current_user, db); r = await svc.seed_system_roles(db, org_id); await db.commit(); return RecomputeSummaryOut(rows_processed=r["roles_created"], status=r["status"])
...
async def audit(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _ro(org_id, current_user, db); return await svc.list_audit_trail(db, org_id)
...
async def data_policies(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _ro(org_id, current_user, db); return await svc.list_data_policies(db, org_id)
...
async def compliance(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _ro(org_id, current_user, db); return await svc.list_compliance_controls(db, org_id)

@router.post("/orgs/{org_id}/security/compliance/recompute", response_model=RecomputeSummaryOut)
async def recompute_compliance(org_id: uuid.UUID, current_user: OperatorUser, db: DBSession, _rl=Depends(recompute_rate_limit)):
    await _ro(org_id, current_user, db); r = await svc.recompute_compliance(db, org_id); await db.commit(); return RecomputeSummaryOut(**r)
...
async def model_isolation(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _ro(org_id, current_user, db); return await svc.list_model_isolation(db, org_id)
...
async def risk_overrides(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _ro(org_id, current_user, db); return await svc.list_risk_overrides(db, org_id)
