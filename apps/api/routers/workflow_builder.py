...
async def list_defs(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    ...
    return await svc.list_definitions(db, org_id)
...
async def list_instances(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession, status: str = None):
    await _ro(org_id, current_user, db)
    return await svc.list_instances(db, org_id, status)


@router.post("/orgs/{org_id}/workflows/from-template")
async def from_template(
    org_id: uuid.UUID, template_key: str, current_user: OperatorUser, db: DBSession, brand_id: uuid.UUID = None
):
    await _ro(org_id, current_user, db)
    wf = await svc.create_from_template(db, org_id, template_key, brand_id)
    await db.commit()
...
async def approve(instance_id: uuid.UUID, current_user: OperatorUser, db: DBSession, notes: str = ""):
    r = await svc.approve_step(db, instance_id, current_user.id, getattr(current_user, "role", "org_admin"), notes)
    await db.commit()
    return r
...
async def reject(instance_id: uuid.UUID, reason: str, current_user: OperatorUser, db: DBSession):
    r = await svc.reject_step(db, instance_id, current_user.id, reason)
    await db.commit()
    return r
...
async def override(instance_id: uuid.UUID, reason: str, current_user: OperatorUser, db: DBSession):
    r = await svc.override_workflow(db, instance_id, current_user.id, reason)
    await db.commit()
    return r
