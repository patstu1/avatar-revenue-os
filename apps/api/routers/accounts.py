...
async def update_credentials(account_id: uuid.UUID, body: CredentialUpdate, current_user: OperatorUser, db: DBSession):
    ...
    if body.platform_token_expires_at:
        try:
            ...
            logger.warning(
                "Invalid token expiry date %r for account %s: %s", body.platform_token_expires_at, account_id, exc
            )
    ...
    await log_action(
        db,
        "creator_account.credentials_updated",
        organization_id=current_user.organization_id,
        brand_id=acct.brand_id,
        user_id=current_user.id,
        actor_type="human",
...
async def trigger_sync(account_id: uuid.UUID, current_user: OperatorUser, db: DBSession):
    ...
    await log_action(
        db,
        "creator_account.sync_triggered",
        organization_id=current_user.organization_id,
        brand_id=acct.brand_id,
        user_id=current_user.id,
        actor_type="human",
