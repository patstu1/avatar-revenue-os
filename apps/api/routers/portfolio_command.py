...
async def portfolio_overview(current_user: CurrentUser, db: DBSession):
    ...
    # Brands
    brands_result = (
        (await db.execute(select(Brand).where(Brand.organization_id == org_id, Brand.is_active))).scalars().all()
    )
    ...
    rev_total = (
        await db.execute(
            ...
            .where(Brand.organization_id == org_id, RevenueLedgerEntry.occurred_at >= day_30)
        )
    ).scalar() or 0
    ...
    pending_actions = (
        await db.execute(
            select(func.count(OperatorAction.id)).where(
                OperatorAction.organization_id == org_id, OperatorAction.status == "pending"
            )
        )
    ).scalar() or 0
    ...
    for brand in brands_result:
        b_rev = (
            await db.execute(
                select(func.coalesce(func.sum(RevenueLedgerEntry.gross_amount), 0)).where(
                    RevenueLedgerEntry.brand_id == brand.id, RevenueLedgerEntry.occurred_at >= day_30
                )
            )
        ).scalar() or 0

        b_content = (
            await db.execute(select(func.count(ContentItem.id)).where(ContentItem.brand_id == brand.id))
        ).scalar() or 0
...
async def portfolio_command_center(current_user: CurrentUser, db: DBSession):
    ...
    brands_q = await db.execute(
        select(Brand.id, Brand.name, func.coalesce(func.sum(RevenueLedgerEntry.gross_amount), 0.0))
        .outerjoin(
            RevenueLedgerEntry,
            and_(
    ...
    brand_rankings = [{"brand_id": str(r[0]), "name": r[1], "revenue_30d": float(r[2] or 0)} for r in brands_q.all()]
    ...
    stalled = (
        await db.execute(
            ...
            .where(
                SponsorOpportunity.brand_id.in_(select(Brand.id).where(Brand.organization_id == org_id)),
    ...
    # ── System health ──
    jobs_running = (
        await db.execute(select(func.count()).select_from(SystemJob).where(SystemJob.status == "running"))
    ).scalar() or 0
    jobs_failed_24h = (
        await db.execute(
            select(func.count())
            .select_from(SystemJob)
            .where(SystemJob.status == "failed", SystemJob.completed_at >= now - timedelta(hours=24))
        )
    ).scalar() or 0
