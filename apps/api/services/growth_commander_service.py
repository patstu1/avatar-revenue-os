...
async def recompute_growth_commands(
    ...
    try:
        ...
        for cmd in commands:
            ...
            db.add(
                GrowthCommand(
                    ...
                    title=cmd["title"][:500],
                    exact_instruction=cmd["exact_instruction"],
                    rationale=cmd.get("rationale"),
                    comparison=cmd.get("comparison"),
                    platform_fit=cmd.get("platform_fit"),
                    niche_fit=cmd.get("niche_fit"),
                    monetization_path=cmd.get("monetization_path"),
                    cannibalization_analysis=cmd.get("cannibalization_analysis"),
                    success_threshold=cmd.get("success_threshold"),
                    failure_threshold=cmd.get("failure_threshold"),
                    expected_upside=cmd["expected_upside"],
                    expected_cost=cmd["expected_cost"],
                    ...
                    linked_launch_candidate_id=uuid.UUID(cmd["linked_launch_candidate_id"])
                    if cmd.get("linked_launch_candidate_id")
                    else None,
                    linked_scale_recommendation_id=uuid.UUID(cmd["linked_scale_recommendation_id"])
                    if cmd.get("linked_scale_recommendation_id")
                    else None,
        ...
        await audit_log_action(
            db,
            "growth_commander.recompute_failed",
            organization_id=brand.organization_id,
            brand_id=brand_id,
            user_id=user_id,
            actor_type="system",
            entity_type="growth_command_run",
            entity_id=run.id,
...
async def _sync_growth_operator_alerts(
    ...
    for cmd in sorted_cmds[:5]:
        ...
        ls = (
            uuid.UUID(cmd["linked_scale_recommendation_id"])
            if cmd.get("linked_scale_recommendation_id")
            else scale_recommendation_id
        )
        db.add(
            OperatorAlert(
                brand_id=brand_id,
...
def _ser(r: GrowthCommand) -> dict:
    return {
        ...
        "priority": r.priority,
        "title": r.title,
        "exact_instruction": r.exact_instruction,
        ...
        "blocking_factors": r.blocking_factors,
        "first_week_plan": r.first_week_plan,
        "linked_launch_candidate_id": str(r.linked_launch_candidate_id) if r.linked_launch_candidate_id else None,
        "linked_scale_recommendation_id": str(r.linked_scale_recommendation_id)
        if r.linked_scale_recommendation_id
        else None,
        "evidence": r.evidence,
