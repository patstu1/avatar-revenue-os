        ...
        (
            await db.execute(
                select(EnterpriseRole).where(
                    EnterpriseRole.organization_id == org_id, EnterpriseRole.is_system.is_(True)
                )
            )
        )
        .scalars()
...
async def check_permission(db: AsyncSession, user_id: uuid.UUID, action: str) -> dict[str, Any]:
    scopes = list(
        (
            await db.execute(
                select(EnterpriseAccessScope).where(
                    EnterpriseAccessScope.user_id == user_id, EnterpriseAccessScope.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def recompute_compliance(db: AsyncSession, org_id: uuid.UUID) -> dict[str, Any]:
    roles = list(
        (await db.execute(select(EnterpriseRole).where(EnterpriseRole.organization_id == org_id))).scalars().all()
    )
    scopes = list((await db.execute(select(EnterpriseAccessScope))).scalars().all())
    policies = list(
        (await db.execute(select(SensitiveDataPolicy).where(SensitiveDataPolicy.organization_id == org_id)))
        .scalars()
        .all()
    )
    isolation = list(
        (await db.execute(select(ModelIsolationPolicy).where(ModelIsolationPolicy.organization_id == org_id)))
        .scalars()
        .all()
    )
    audits = list(
        (await db.execute(select(AuditTrailEvent).where(AuditTrailEvent.organization_id == org_id).limit(1)))
        .scalars()
        .all()
    )
...
async def list_roles(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(EnterpriseRole).where(
                    EnterpriseRole.organization_id == org_id, EnterpriseRole.is_active.is_(True)
                )
            )
        )
        .scalars()
        ...
        (
            await db.execute(
                select(EnterprisePermission).where(
                    EnterprisePermission.role_id == role_id, EnterprisePermission.is_active.is_(True)
                )
            )
        )
        .scalars()
    ...
    return list(
        (
            await db.execute(
                ...
                .where(AuditTrailEvent.organization_id == org_id)
...
async def list_data_policies(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(SensitiveDataPolicy).where(
                    SensitiveDataPolicy.organization_id == org_id, SensitiveDataPolicy.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def list_compliance_controls(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(ComplianceControlReport).where(
                    ComplianceControlReport.organization_id == org_id, ComplianceControlReport.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def list_model_isolation(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(ModelIsolationPolicy).where(
                    ModelIsolationPolicy.organization_id == org_id, ModelIsolationPolicy.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def list_risk_overrides(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(RiskOverrideEvent).where(
                    RiskOverrideEvent.organization_id == org_id, RiskOverrideEvent.is_active.is_(True)
                )
            )
        )
        .scalars()
