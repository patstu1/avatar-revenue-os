...
async def recompute_offer_lab(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    await db.execute(delete(OfferLabPositioningTest).where(OfferLabPositioningTest.offer_id.in_(select(OfferLabOffer.id).where(OfferLabOffer.brand_id == brand_id))))
    await db.execute(delete(OfferLabPricingTest).where(OfferLabPricingTest.offer_id.in_(select(OfferLabOffer.id).where(OfferLabOffer.brand_id == brand_id))))
    await db.execute(delete(OfferLabVariant).where(OfferLabVariant.offer_id.in_(select(OfferLabOffer.id).where(OfferLabOffer.brand_id == brand_id))))
    ...
    lab_offers = []
    for o in offers:
        source = {"name": o.name, "monetization_method": o.monetization_method, "payout_amount": float(o.payout_amount or 0), "epc": float(o.epc or 0), "conversion_rate": float(o.conversion_rate or 0)}
        spec = generate_offer(source, brand_dict)
        lo = OfferLabOffer(brand_id=brand_id, source_offer_id=o.id, **spec)
        db.add(lo); await db.flush()
    ...
    lo_dicts = [{"id": str(lo.id), "offer_name": lo.offer_name, "price_point": float(lo.price_point)} for lo in lab_offers]
    for b in generate_bundles(lo_dicts):
        db.add(OfferLabBundle(brand_id=brand_id, bundle_name=b["bundle_name"], offer_ids=b["offer_ids"], combined_price=b["combined_price"], savings_pct=b["savings_pct"], expected_uplift=b["expected_uplift"]))
...
async def list_offers(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list((await db.execute(select(OfferLabOffer).where(OfferLabOffer.brand_id == brand_id, OfferLabOffer.is_active.is_(True)).order_by(OfferLabOffer.rank_score.desc()))).scalars().all())

async def list_variants(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list((await db.execute(select(OfferLabVariant).join(OfferLabOffer).where(OfferLabOffer.brand_id == brand_id, OfferLabVariant.is_active.is_(True)))).scalars().all())

async def list_bundles(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list((await db.execute(select(OfferLabBundle).where(OfferLabBundle.brand_id == brand_id, OfferLabBundle.is_active.is_(True)))).scalars().all())

async def list_blockers(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list((await db.execute(select(OfferLabBlocker).where(OfferLabBlocker.brand_id == brand_id, OfferLabBlocker.is_active.is_(True)))).scalars().all())

async def list_learning(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list((await db.execute(select(OfferLabLearning).where(OfferLabLearning.brand_id == brand_id, OfferLabLearning.is_active.is_(True)))).scalars().all())

async def get_best_offer(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    o = (await db.execute(select(OfferLabOffer).where(OfferLabOffer.brand_id == brand_id, OfferLabOffer.is_active.is_(True)).order_by(OfferLabOffer.rank_score.desc()).limit(1))).scalar_one_or_none()
    if not o:
        return {"offer_id": None}
    return {"offer_id": str(o.id), "name": o.offer_name, "rank_score": o.rank_score, "angle": o.primary_angle, "truth_label": o.truth_label}
