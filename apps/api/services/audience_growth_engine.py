...
async def discover_audience_expansion_opportunities(
    ...
    perf_q = await db.execute(
        select(PerformanceMetric.platform, func.sum(PerformanceMetric.impressions),
               func.sum(PerformanceMetric.revenue), func.count())
    ...
    for row in perf_q.all():
        if row[0]:
            platform_perf[str(row[0])] = {"impressions": int(row[1] or 0), "revenue": float(row[2] or 0), "count": row[3]}
    ...
    all_platforms = ["youtube", "tiktok", "instagram", "x", "linkedin", "reddit",
                     "pinterest", "blog", "email_newsletter", "substack", "threads"]
    platform_expansions = []
    ...
    niche_clusters = (await db.execute(
        select(NicheCluster).where(NicheCluster.brand_id == brand_id)
        .order_by(NicheCluster.monetization_potential.desc().nullslast()).limit(10)
    )).scalars().all()
    ...
    trending = (await db.execute(
        select(TrendSignal).order_by(TrendSignal.velocity.desc().nullslast()).limit(10)
    )).scalars().all()

    trend_opportunities = [
        {"keyword": t.keyword, "velocity": t.velocity, "volume": t.volume,
         "strength": t.strength.value if hasattr(t.strength, 'value') else str(t.strength),
         "action": "create_content_on_trend"}
        for t in trending if t.velocity and t.velocity > 0.3
    ]
    ...
    underserved = [
        {"content_type": ct, "current_count": content_types.get(ct, 0),
         "action": "create_more_content_type"}
        for ct in all_types if content_types.get(ct, 0) < 3
    ]

    # ── 5. High-performing segments to double down on ──
    top_content = (await db.execute(
        select(ContentItem.platform, ContentItem.content_type, func.count(),
               func.sum(PerformanceMetric.impressions), func.sum(PerformanceMetric.revenue))
    ...
    double_down = [
        {"platform": str(r[0] or "unknown"), "content_type": str(r[1].value if hasattr(r[1], 'value') else r[1]),
         "content_count": r[2], "impressions": int(r[3] or 0), "revenue": float(r[4] or 0),
         "action": "scale_winning_segment"}
        for r in top_content if (r[4] or 0) > 0
    ]
    ...
    return {
        ...
        "total_opportunities": (len(platform_expansions) + len(adjacent_niches) +
                                len(trend_opportunities) + len(underserved) + len(double_down)),
    }
...
async def surface_audience_growth_actions(
    ...
    for exp in data.get("platform_expansions", [])[:3]:
        a = await emit_action(
            db, org_id=org_id, action_type="expand_to_platform",
            title=f"Expand to {exp['platform']} (fit: {exp['fit_score']:.0%})",
            description=exp.get("rationale", ""),
            category="opportunity", priority="high" if exp["fit_score"] > 0.6 else "medium",
            brand_id=brand_id, source_module="audience_growth_engine",
        )
        created.append({"type": "platform_expansion", "action_id": str(a.id)})

    for trend in data.get("trending_opportunities", [])[:2]:
        a = await emit_action(
            db, org_id=org_id, action_type="create_content_on_trend",
...
def _score_niche_platform_fit(niche: str, platform: str) -> float:
    ...
    fit_matrix = {
        "business": {"linkedin": 0.9, "youtube": 0.8, "x": 0.7, "substack": 0.7, "blog": 0.6, "instagram": 0.4, "tiktok": 0.5},
