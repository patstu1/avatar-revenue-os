...
async def sync_network_data(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    networks = list((await db.execute(
        select(AffiliateNetworkAccount).where(AffiliateNetworkAccount.brand_id == brand_id, AffiliateNetworkAccount.is_active.is_(True))
    )).scalars().all())
...
async def _import_impact_conversions(db: AsyncSession, brand_id: uuid.UUID, network_id: uuid.UUID, actions: list) -> int:
    count = 0
    for a in actions:
        ...
        if not link:
            ...
            if offer_id_str:
                link = (await db.execute(
                    select(AffiliateLink).where(AffiliateLink.brand_id == brand_id).order_by(AffiliateLink.created_at.desc()).limit(1)
                )).scalar_one_or_none()
            else:
                link = (await db.execute(
                    select(AffiliateLink).where(AffiliateLink.brand_id == brand_id).order_by(AffiliateLink.created_at.desc()).limit(1)
        ...
        if not link:
            continue
        db.add(AffiliateConversionEvent(
            brand_id=brand_id, link_id=link.id, offer_id=link.offer_id,
...
async def _import_cj_commissions(db: AsyncSession, brand_id: uuid.UUID, network_id: uuid.UUID, data: Any) -> int:
    ...
    count = 0
    for item in data:
        link = (await db.execute(select(AffiliateLink).where(AffiliateLink.brand_id == brand_id).limit(1))).scalar_one_or_none()
        if not link:
            continue
        db.add(AffiliateConversionEvent(
            brand_id=brand_id, link_id=link.id, offer_id=link.offer_id,
