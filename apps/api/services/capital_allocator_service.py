...
async def recompute_allocation(
    ...
    constraints = list(
        (
            await db.execute(
                select(CAAllocationConstraint).where(
                    CAAllocationConstraint.brand_id == brand_id, CAAllocationConstraint.is_active.is_(True)
                )
            )
        )
        .scalars()
    ...
    for dec in result["decisions"]:
        ...
        at = AllocationTarget(
            brand_id=brand_id,
            report_id=report.id,
            target_type=dec["target_type"],
            target_key=dec["target_key"],
            target_id=tid,
            expected_return=dec.get("return_score", 0),
            expected_cost=0,
...
async def list_decisions(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(CAAllocationDecision)
                .where(CAAllocationDecision.brand_id == brand_id, CAAllocationDecision.is_active.is_(True))
                .order_by(CAAllocationDecision.allocation_pct.desc())
            )
        )
        .scalars()
        .all()
...
async def list_rebalances(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(CAAllocationRebalance)
                .where(CAAllocationRebalance.brand_id == brand_id)
                .order_by(CAAllocationRebalance.created_at.desc())
                .limit(20)
            )
        )
        .scalars()
...
async def get_allocation_for_target(
    db: AsyncSession, brand_id: uuid.UUID, target_type: str, target_key: str
) -> dict[str, Any]:
    ...
    at = (
        await db.execute(
            ...
            .order_by(AllocationTarget.created_at.desc())
            .limit(1)
        )
    ).scalar_one_or_none()
    if not at:
    ...
    dec = (
        await db.execute(
            select(CAAllocationDecision).where(
                CAAllocationDecision.target_id == at.id, CAAllocationDecision.is_active.is_(True)
            )
        )
    ).scalar_one_or_none()
    if not dec:
