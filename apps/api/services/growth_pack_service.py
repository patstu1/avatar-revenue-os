...
async def _load_generation_inputs(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    cand_dicts = [
        {
            ...
            "secondary_platform": c.secondary_platform,
            "niche": c.niche,
            ...
            "posting_strategy": c.posting_strategy,
    ...
    blockers = list(
        (
            await db.execute(
                select(ScaleBlockerReport).where(
                    ScaleBlockerReport.brand_id == brand_id, ScaleBlockerReport.is_resolved.is_(False)
                )
            )
        )
        .scalars()
    ...
    readiness = (
        (
            await db.execute(
                select(LaunchReadinessReport)
                .where(LaunchReadinessReport.brand_id == brand_id, LaunchReadinessReport.is_active.is_(True))
                .order_by(LaunchReadinessReport.created_at.desc())
                .limit(1)
            )
        )
        .scalars()
    ...
    readiness_dict = (
        {"launch_readiness_score": readiness.launch_readiness_score, "recommended_action": readiness.recommended_action}
        if readiness
        else None
    )
    trust_rows = list(
...
async def recompute_portfolio_launch_plan(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    order = [
        {"rank": i + 1, "command_type": c.get("command_type"), "title": c.get("title")}
        for i, c in enumerate(ctx["commands"][:5])
    ]
    ...
    plan = build_portfolio_launch_plan(
        ...
        explanation={
            "sources": ["scale_recommendation", "growth_commander_engine"],
            "comparison": launch_cmds[0].get("comparison") if launch_cmds else {},
        },
    )
...
async def recompute_account_blueprints(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ctx = await _load_generation_inputs(db, brand_id)
    launch_cmds = [
        c for c in ctx["commands"] if c.get("command_type") in ("launch_account", "shift_platform", "shift_niche")
    ]
...
async def recompute_capital_deployment(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    plat_mix = {
        p: round(total / max(1, len(ctx["accounts_by_platform"])) * 0.15, 2) for p in ctx["accounts_by_platform"]
    } or {"youtube": round(total * 0.3, 2)}
...
async def recompute_cross_account_cannibalization(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    pairs = build_cannibalization_pairs(ctx["acc_dicts"])
    await db.execute(
        delete(CrossAccountCannibalizationReport).where(CrossAccountCannibalizationReport.brand_id == brand_id)
    )
    for p in pairs:
        db.add(
            CrossAccountCannibalizationReport(
...
async def list_portfolio_launch_plans(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list(
        (
            await db.execute(
                select(PortfolioLaunchPlan)
                .where(PortfolioLaunchPlan.brand_id == brand_id)
                .order_by(PortfolioLaunchPlan.created_at.desc())
                .limit(5)
            )
        )
        .scalars()
...
async def list_account_blueprints(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list(
        (
            await db.execute(
                select(AccountLaunchBlueprint)
                .where(AccountLaunchBlueprint.brand_id == brand_id)
                .order_by(AccountLaunchBlueprint.created_at.desc())
            )
        )
        .scalars()
        .all()
...
async def get_account_blueprint(db: AsyncSession, blueprint_id: uuid.UUID) -> dict | None:
    b = (
        await db.execute(select(AccountLaunchBlueprint).where(AccountLaunchBlueprint.id == blueprint_id))
    ).scalar_one_or_none()
...
async def list_platform_allocation(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list(
        (
            await db.execute(
                select(PlatformAllocationReport)
                .where(PlatformAllocationReport.brand_id == brand_id)
                .order_by(PlatformAllocationReport.expansion_priority.desc())
            )
        )
        .scalars()
        .all()
...
async def list_niche_deployment(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list(
        (
            await db.execute(
                select(NicheDeploymentReport)
                .where(NicheDeploymentReport.brand_id == brand_id)
                .order_by(NicheDeploymentReport.expected_upside.desc())
            )
        )
        .scalars()
        .all()
...
async def list_growth_blockers_pack(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list(
        (
            await db.execute(
                select(GrowthPackBlockerReport)
                .where(GrowthPackBlockerReport.brand_id == brand_id)
                .order_by(GrowthPackBlockerReport.urgency_score.desc())
            )
        )
        .scalars()
        .all()
...
async def list_capital_deployment(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list(
        (
            await db.execute(
                select(CapitalDeploymentPlan)
                .where(CapitalDeploymentPlan.brand_id == brand_id)
                .order_by(CapitalDeploymentPlan.created_at.desc())
                .limit(3)
            )
        )
        .scalars()
...
async def list_cross_cannibalization(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list(
        (
            await db.execute(
                select(CrossAccountCannibalizationReport)
                .where(CrossAccountCannibalizationReport.brand_id == brand_id)
                .order_by(CrossAccountCannibalizationReport.overlap_score.desc())
            )
        )
        .scalars()
        .all()
...
async def list_portfolio_output(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list(
        (
            await db.execute(
                select(PortfolioOutputReport)
                .where(PortfolioOutputReport.brand_id == brand_id)
                .order_by(PortfolioOutputReport.created_at.desc())
                .limit(3)
            )
        )
        .scalars()
