...
async def recompute_offer_ladders(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    offer_dicts = [{
        "id": str(o.id), "name": o.name, "epc": float(o.epc or 0), "conversion_rate": float(o.conversion_rate or 0),
...
async def recompute_owned_audience(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    if not brand:
        raise ValueError("Brand not found")
    items = list((await db.execute(select(ContentItem).where(ContentItem.brand_id == brand_id).limit(100))).scalars().all())
...
async def generate_message_sequences(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    await db.execute(
        delete(MessageSequenceStep).where(
            MessageSequenceStep.sequence_id.in_(
                select(MessageSequence.id).where(MessageSequence.brand_id == brand_id)
            )
        )
    )
...
async def get_offer_ladders(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list((await db.execute(select(OfferLadder).where(OfferLadder.brand_id == brand_id, OfferLadder.is_active.is_(True)))).scalars().all())
    return [_ladder_dict(r) for r in rows]


async def get_owned_audience_bundle(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    assets = list((await db.execute(select(OwnedAudienceAsset).where(OwnedAudienceAsset.brand_id == brand_id, OwnedAudienceAsset.is_active.is_(True)))).scalars().all())
    events = list((await db.execute(select(OwnedAudienceEvent).where(OwnedAudienceEvent.brand_id == brand_id).order_by(OwnedAudienceEvent.created_at.desc()).limit(200))).scalars().all())
    return {
        "assets": [{
            "id": str(a.id),
            "asset_type": a.asset_type,
...
async def get_message_sequences(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    seqs = list((await db.execute(select(MessageSequence).where(MessageSequence.brand_id == brand_id, MessageSequence.is_active.is_(True)))).scalars().all())
    out: list[dict] = []
    for s in seqs:
        steps = list((await db.execute(
            select(MessageSequenceStep).where(MessageSequenceStep.sequence_id == s.id).order_by(MessageSequenceStep.step_order)
        )).scalars().all())
        out.append({
            "id": str(s.id),
...
async def get_funnel_stage_metrics(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list((await db.execute(select(FunnelStageMetric).where(FunnelStageMetric.brand_id == brand_id))).scalars().all())
    return [{
        "id": str(r.id),
...
async def get_funnel_leaks(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list((await db.execute(select(FunnelLeakFix).where(FunnelLeakFix.brand_id == brand_id, FunnelLeakFix.is_active.is_(True)))).scalars().all())
    return [{
        "id": str(r.id),
