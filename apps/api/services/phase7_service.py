...
async def recompute_phase7(
    ...
    winner_dicts = [
        {
            ...
            "title": w.title,
            "win_score": w.win_score,
            "platform": next((c.platform for c in cp_items if c.content_id == w.content_id), "youtube"),
            "revenue": next((c.revenue for c in cp_items if c.content_id == w.content_id), 0),
            "rpm": next((c.rpm for c in cp_items if c.content_id == w.content_id), 0),
        }
        for w in winner_signals
        if w.is_winner
    ]
    ...
    leak_dicts = [
        {
            ...
            "estimated_leaked_revenue": l.estimated_leaked_revenue,
            "estimated_recoverable": l.estimated_recoverable,
            "recommended_fix": l.recommended_fix,
    ...
    scale_rec = (
        (
            await db.execute(
                ...
                .order_by(ScaleRecommendation.created_at.desc())
                .limit(1)
            )
        )
        .scalars()
    ...
    trust_rows = list(
        (await db.execute(select(TrustSignalReport).where(TrustSignalReport.brand_id == brand_id))).scalars().all()
    )
    ...
    paid_candidates = (
        await db.execute(
            select(func.count())
            .select_from(PaidAmplificationJob)
            .where(PaidAmplificationJob.brand_id == brand_id, PaidAmplificationJob.is_candidate.is_(True))
        )
    ).scalar() or 0
    ...
    await db.execute(
        delete(CapitalAllocationRecommendation).where(CapitalAllocationRecommendation.brand_id == brand_id)
    )
    ...
    comments = list(
        (
            await db.execute(
                select(CommentIngestion)
                .where(CommentIngestion.brand_id == brand_id, CommentIngestion.is_processed.is_(True))
                .limit(500)
            )
        )
        .scalars()
    ...
    comment_dicts = [
        {
            ...
            "is_purchase_intent": c.is_purchase_intent,
    ...
    # —— 3. Knowledge graph ——
    kg_nodes, kg_edges = build_knowledge_graph_entries(
        brand.niche, acc_dicts, offer_dicts, winner_dicts, seg_dicts, leak_dicts
    )
    ...
    roadmap = generate_roadmap(
        brand.niche, acc_dicts, offer_dicts, winner_dicts, leak_dicts, seg_dicts, geo_dicts, scale_key, trust_avg
    )
    for rm in roadmap:
        conf = (
            ConfidenceLevel.HIGH
            if rm["priority_score"] >= 75
            else (ConfidenceLevel.MEDIUM if rm["priority_score"] >= 50 else ConfidenceLevel.LOW)
        )
        db.add(
            RoadmapRecommendation(
                brand_id=brand_id,
                ...
                priority_score=rm["priority_score"],
    ...
    prod_rec_count = (
        await db.execute(
            select(func.count())
            .select_from(MonetizationRecommendation)
            .where(
    ...
    cap_allocs = compute_capital_allocation(
        ...
        paid_candidate_count=paid_candidates,
        geo_rec_count=len(geo_recs),
        trust_avg=trust_avg,
        scale_rec_key=scale_key,
        productization_rec_count=prod_rec_count,
        owned_audience_size=owned_size,
    )
...
async def get_sponsor_opportunities(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    packages = recommend_sponsor_packages(
        brand.niche if brand else None, _accounts_as_dicts(accounts), total_rev, int(total_imps)
    )
    ...
    opps = list(
        (
            await db.execute(
                ...
                .order_by(SponsorOpportunity.deal_value.desc())
            )
        )
        .scalars()
        .all()
...
async def get_comment_cash_signals(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    rows = list(
        (
            await db.execute(
                select(CommentCashSignal)
                .where(CommentCashSignal.brand_id == brand_id)
                .order_by(CommentCashSignal.signal_strength.desc())
            )
        )
        .scalars()
        .all()
    ...
    rows = list(
        (
            await db.execute(
                ...
                .where(RoadmapRecommendation.brand_id == brand_id)
...
async def get_capital_allocation(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    rows = list(
        (
            await db.execute(
                select(CapitalAllocationRecommendation)
                .where(CapitalAllocationRecommendation.brand_id == brand_id)
                .order_by(CapitalAllocationRecommendation.recommended_allocation_pct.desc())
            )
        )
        .scalars()
        .all()
    ...
    return {
        "allocations": [
            {
                "id": str(r.id),
                "allocation_target_type": r.allocation_target_type,
...
async def get_knowledge_graph(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
        ...
        (
            await db.execute(
                select(KnowledgeGraphNode).where(
                    KnowledgeGraphNode.brand_id == brand_id, KnowledgeGraphNode.is_active.is_(True)
                )
            )
        )
        .scalars()
        ...
        (
            await db.execute(
                select(KnowledgeGraphEdge).where(
                    KnowledgeGraphEdge.brand_id == brand_id, KnowledgeGraphEdge.is_active.is_(True)
                )
            )
        )
        .scalars()
    ...
    return {
        "nodes": [
            {"id": str(n.id), "node_type": n.node_type, "label": n.label, "properties": n.properties} for n in nodes
        ],
        "edges": [
            {
                "id": str(e.id),
...
async def get_operator_cockpit(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    scale_action = (
        ...
        if scale_rec
        else {"recommendation_key": "none", "explanation": "No scale data yet.", "scale_readiness_score": 0}
    )
    ...
    return {
        ...
        "expansion_targets": [
            {
                "geography": g.target_geography,
                "language": g.target_language,
                "revenue_potential": g.estimated_revenue_potential,
            }
            for g in geo_recs
        ],
