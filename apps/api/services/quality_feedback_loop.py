...
async def run_quality_feedback(db: AsyncSession, brand_id: uuid.UUID) -> dict:
    ...
    if not rows:
        return {"processed": 0, "winners_updated": 0, "losers_created": 0,
    ...
    for content, script, perf in rows:
        ...
        if performance_score >= 0.6:
            ...
            for pattern_type, pattern_name in [
                ("hook", hook_type), ("cta", cta_type),
                ("content_form", content_type), ("offer_angle", offer_angle),
            ]:
            ...
            for pattern_type, pattern_name in [
                ("hook", hook_type), ("cta", cta_type), ("content_form", content_type),
            ]:
                if pattern_name and pattern_name != "unknown":
                    existing = (await db.execute(
    ...
    await emit_event(
        db, domain="intelligence", event_type="quality_feedback.completed",
        ...
        details={"winners_updated": winners_updated, "losers_created": losers_created,
                 "memory_entries": memory_entries, "content_evaluated": len(rows)},
    )
