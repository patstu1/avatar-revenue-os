...
async def recompute_ranking(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    offers = list(
        (
            await db.execute(
                select(AffiliateOffer).where(AffiliateOffer.brand_id == brand_id, AffiliateOffer.is_active.is_(True))
            )
        )
        .scalars()
        .all()
    ...
    offer_dicts = [
        {
            ...
            "epc": float(o.epc),
            "conversion_rate": float(o.conversion_rate),
            "commission_rate": float(o.commission_rate),
            "refund_rate": float(o.refund_rate),
            "trust_score": float(o.trust_score),
            "content_fit_score": float(o.content_fit_score),
            "platform_fit_score": float(o.platform_fit_score),
            "audience_fit_score": float(o.audience_fit_score),
            "affiliate_url": o.affiliate_url,
            "destination_url": o.destination_url,
            "blocker_state": o.blocker_state,
    ...
    links = list(
        (
            await db.execute(
                select(AffiliateLink).where(AffiliateLink.brand_id == brand_id, AffiliateLink.is_active.is_(True))
            )
        )
        .scalars()
        .all()
    ...
    link_dicts = [
        {
            "id": str(l.id),
            "offer_id": str(l.offer_id),
            "click_count": l.click_count,
            "conversion_count": l.conversion_count,
        }
        for l in links
    ]
    ...
    for lk in leaks:
        ...
        if lk.get("offer_id"):
            try:
                oid = uuid.UUID(str(lk["offer_id"]))
            except (ValueError, AttributeError):
                pass
        if lk.get("link_id"):
            try:
                lid = uuid.UUID(str(lk["link_id"]))
            except (ValueError, AttributeError):
                pass
        db.add(
            AffiliateLeak(
                ...
                revenue_loss_estimate=lk["revenue_loss_estimate"],
                recommendation=lk["recommendation"],
            )
        )
    ...
    return list(
        (
            await db.execute(
                ...
                .where(AffiliateOffer.brand_id == brand_id, AffiliateOffer.is_active.is_(True))
                .order_by(AffiliateOffer.rank_score.desc())
            )
        )
        .scalars()
        .all()
...
async def list_leaks(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(AffiliateLeak)
                .where(AffiliateLeak.brand_id == brand_id, AffiliateLeak.is_active.is_(True))
                .order_by(AffiliateLeak.severity)
            )
        )
        .scalars()
        .all()
...
async def list_blockers(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(AffiliateBlocker).where(
                    AffiliateBlocker.brand_id == brand_id, AffiliateBlocker.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def list_commissions(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (await db.execute(select(AffiliateCommissionEvent).where(AffiliateCommissionEvent.brand_id == brand_id)))
        .scalars()
        .all()
    )
...
async def list_payouts(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (await db.execute(select(AffiliatePayoutEvent).where(AffiliatePayoutEvent.brand_id == brand_id)))
        .scalars()
        .all()
    )
