...
async def recompute_leaks(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    report = RevenueLeakReport(
        brand_id=brand_id,
        total_leaks=len(leaks),
        total_estimated_loss=loss["total_estimated_loss"],
        critical_count=critical,
        top_leak_type=top_type,
        summary=summary,
    )
    db.add(report)
    await db.flush()
    ...
    for l in leaks:
        ...
        if l.get("affected_id"):
            try:
                aid = uuid.UUID(str(l["affected_id"]))
        ...
        ev = RevenueLeakEvent(
            ...
            severity=l["severity"],
            affected_scope=l["affected_scope"],
            affected_id=aid,
            estimated_revenue_loss=l["estimated_revenue_loss"],
            confidence=l["confidence"],
            evidence_json=l.get("evidence_json", {}),
            next_best_action=l["next_best_action"],
            truth_label=l.get("truth_label", "measured"),
        )
        db.add(ev)
        await db.flush()
    ...
    for i, corr in enumerate(corrections):
        leak_ev_id = (
            list(event_map.values())[i] if i < len(event_map) else list(event_map.values())[0] if event_map else None
        )
        if leak_ev_id:
            db.add(
                LeakCorrectionAction(
                    brand_id=brand_id,
                    leak_event_id=leak_ev_id,
                    action_type=corr["action_type"],
                    action_detail=corr["action_detail"],
    ...
    db.add(
        RevenueLossEstimate(
            ...
            total_estimated_loss=loss["total_estimated_loss"],
            by_leak_type=loss["by_leak_type"],
            by_scope=loss["by_scope"],
        )
    )

    await db.flush()
    return {
        "rows_processed": len(leaks),
        "clusters": len(clusters),
        "total_loss": loss["total_estimated_loss"],
        "status": "completed",
    }
...
async def _gather_system_data(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    perfs = list(
        (await db.execute(select(PerformanceMetric).where(PerformanceMetric.brand_id == brand_id).limit(200)))
        .scalars()
        .all()
    )
    ...
    for p in perfs:
        ci_perf.setdefault(
            p.content_item_id, {"impressions": 0, "clicks": 0, "revenue": 0, "engagement_rate": 0, "n": 0}
        )
    ...
    offers = list(
        (
            await db.execute(
                select(OfferLabOffer).where(OfferLabOffer.brand_id == brand_id, OfferLabOffer.is_active.is_(True))
            )
        )
        .scalars()
        .all()
    ...
    return list(
        (
            await db.execute(
                ...
                .where(RevenueLeakReport.brand_id == brand_id)
                .order_by(RevenueLeakReport.created_at.desc())
                .limit(10)
            )
        )
        .scalars()
...
async def list_events(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(RevenueLeakEvent)
                .where(RevenueLeakEvent.brand_id == brand_id, RevenueLeakEvent.is_active.is_(True))
                .order_by(RevenueLeakEvent.estimated_revenue_loss.desc())
                .limit(50)
            )
        )
        .scalars()
...
async def list_clusters(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(LeakCluster)
                .where(LeakCluster.brand_id == brand_id, LeakCluster.is_active.is_(True))
                .order_by(LeakCluster.priority_score.desc())
            )
        )
        .scalars()
        .all()
...
async def list_corrections(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(LeakCorrectionAction)
                .where(LeakCorrectionAction.brand_id == brand_id, LeakCorrectionAction.is_active.is_(True))
                .order_by(LeakCorrectionAction.priority)
            )
        )
        .scalars()
        .all()
...
async def get_leak_summary(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    return {
        ...
        "total_loss": report.total_estimated_loss,
        "critical": report.critical_count,
        "top_type": report.top_leak_type,
