        ...
        if delay:
            db.add(
                CostOfDelayModel(
                    ...
                    decay_rate=delay.get("decay_rate", 0),
                    time_sensitivity=delay.get("time_sensitivity", "normal"),
...
async def _gather_system_state(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
        ...
        (
            await db.execute(
                select(PWExperimentWinner).where(
                    PWExperimentWinner.brand_id == brand_id, PWExperimentWinner.promoted.is_(False)
                )
            )
        )
        .scalars()
    ...
    for w in winners:
        state["experiment_winners"].append(
            {"id": str(w.id), "name": str(w.variant_id)[:8], "confidence": float(w.confidence)}
        )
    ...
    ready = list(
        (
            await db.execute(
                select(ContentItem)
                .where(ContentItem.brand_id == brand_id, ContentItem.status.in_(("approved", "media_complete")))
                .limit(10)
            )
        )
        .scalars()
...
async def list_reports(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(OpportunityCostReport)
                .where(OpportunityCostReport.brand_id == brand_id)
                .order_by(OpportunityCostReport.created_at.desc())
                .limit(10)
            )
        )
        .scalars()
    ...
    return [
        {
            ...
            "type": r.action_type,
            "key": r.action_key,
            "upside": r.expected_upside,
            "delay_cost": r.cost_of_delay,
            "urgency": r.urgency,
            "safe_to_wait": r.safe_to_wait,
            "explanation": r.explanation,
        }
