...
async def recompute_advisory(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    scale_result_obj = run_scale_engine(
        ...
        brand.niche,
        funnel_weak=False,
        weak_offer_diversity=len(offers) < 2,
    )
    scale_dict = {
    ...
    account_dicts = [
        {"id": str(a.id), "platform": s.platform, "username": s.username} for a, s in zip(accounts_raw, snapshots)
    ]
    ...
    avg_health = (
        ...
        if any(h == "warning" for h in healths)
        else "healthy"
    )
    ...
    if advisory["should_add_account_now"]:
        ...
        db.add(
            OperatorAlert(
                ...
                recommended_action=f"Fix: {blocker_summary[:300]}"
                if blocker_summary
                else f"Wait until: {hold_reason[:300]}",
                confidence=float(advisory.get("confidence", 0)),
                urgency=30.0,
                supporting_metrics=advisory.get("evidence", {}),
