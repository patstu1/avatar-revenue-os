...
async def compute_creator_monetization_fit(
    ...
    for acct in accounts:
        ...
        health = (
            acct.health_status.value
            if hasattr(acct, "health_status") and hasattr(acct.health_status, "value")
            else "healthy"
        )
        ...
        base = max(
            0,
            follower_ratio * 0.3
            + min(1.0, engagement * 10) * 0.3
            + (0.4 if health == "healthy" else 0.2)
            - scale_penalty,
        )
        ...
        scores["sponsor"] = min(
            1.0,
            base
            * (1.3 if followers > 0 else 0.4)
            * (1.1 if engagement > 0 else 0.7)
            * (1.2 if sponsor_count > 0 else 0.8),
        )
        scores["dtc"] = min(
            1.0, base * (1.2 if followers > 0 else 0.5) * (0.9 if platform in ("instagram", "tiktok") else 0.6)
        )
        scores["product"] = min(
            1.0,
            base
            * (1.3 if float(ledger_revenue) > 0 else 0.4)
            * (1.1 if niche in ("education", "tech", "business", "finance") else 0.7),
        )
        scores["subscription"] = min(
            1.0, base * (1.4 if follower_ratio > 0.5 else 0.3) * (1.2 if engagement > 0.05 else 0.5)
        )
        scores["services"] = min(
            1.0,
            base
            * (1.5 if niche in ("business", "consulting", "tech", "marketing") else 0.6)
            * (1.1 if float(ledger_revenue) > 0 else 0.7),
        )
        scores["licensing"] = min(
            1.0, base * (1.2 if is_video_platform else 0.4) * (0.8 if follower_ratio > 0.8 else 0.3)
        )
        scores["lead_gen"] = min(
            1.0,
            base
            * (1.3 if niche in ("business", "finance", "saas", "consulting") else 0.5)
            * (1.1 if engagement > 0 else 0.6),
        )
        scores["long_form"] = min(1.0, base * (1.4 if is_long_form else 0.3) * (1.2 if platform == "youtube" else 0.8))
        scores["short_form"] = min(
            1.0, base * (1.3 if is_short_form else 0.3) * (1.2 if platform in ("tiktok", "instagram") else 0.7)
        )
...
async def detect_revenue_opportunities(
    ...
    for opp in top_opps.scalars().all():
        ...
        calibrated_action = (
            override_recommendation_action(opp.recommended_action, cal_ctx) if opp.recommended_action else None
        )
        ...
        opportunities.append(
            {
                ...
                "expected_upside": opp.composite_score
                * max(total_rev_for_upside, 100),  # Scaled to actual portfolio revenue
    ...
    orphan_q = await db.execute(select(Offer).where(Offer.brand_id == brand_id, Offer.is_active.is_(True)).limit(20))
    ...
    active_sponsors = (
        await db.execute(
            ...
            .where(SponsorOpportunity.brand_id == brand_id, SponsorOpportunity.status == "active")
        )
    ).scalar() or 0
            ...
            opportunities.append(
                {
                    ...
                    "expected_upside": followers
                    * (rev_per_follower if rev_per_follower > 0 else 0.02),  # Derived from actual yield
            ...
            opportunities.append(
                {
                    ...
                    "expected_upside": pat.win_score
                    * float(total_rev_for_upside)
                    * 0.1,  # Winning pattern worth 10% of portfolio
...
async def get_revenue_memory(
    ...
    by_source_q = await db.execute(
        ...
        .where(
            ...
            RevenueLedgerEntry.is_active.is_(True),
            RevenueLedgerEntry.is_refund.is_(False),
            RevenueLedgerEntry.gross_amount > 0,
        )
        .group_by(RevenueLedgerEntry.revenue_source_type)
    )
    revenue_by_source = {
        str(r[0]): {"total": float(r[1] or 0), "avg_margin": round(float(r[2] or 0), 3)} for r in by_source_q.all()
    }
...
async def compute_monetization_mix(
    ...
    for a in alloc["allocations"]:
        recommended_mix[a["source_type"]] = (
            round(a["allocation_score"] / total_score * 100, 1) if total_score > 0 else 0
        )
