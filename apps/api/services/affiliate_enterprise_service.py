...
async def recompute_governance(db: AsyncSession, org_id: uuid.UUID) -> dict[str, Any]:
        ...
        (
            await db.execute(
                select(AffiliateOffer).where(
                    AffiliateOffer.brand_id.in_(
                        select(AffiliateOffer.brand_id).where(AffiliateOffer.is_active.is_(True))
                    )
                )
            )
        )
    ...
    rules = list(
        (
            await db.execute(
                select(AffiliateGovernanceRule).where(
                    AffiliateGovernanceRule.organization_id == org_id, AffiliateGovernanceRule.is_active.is_(True)
                )
            )
        )
        .scalars()
    ...
    banned = list(
        (
            await db.execute(
                select(AffiliateBannedEntity).where(
                    AffiliateBannedEntity.organization_id == org_id, AffiliateBannedEntity.is_active.is_(True)
                )
            )
        )
        .scalars()
    ...
    [
        {"rule_type": r.rule_type, "rule_key": r.rule_key, "rule_value": r.rule_value, "severity": r.severity}
        for r in rules
    ]
    ...
    for o in offers:
        offer_dict = {
            ...
            "refund_rate": float(o.refund_rate),
            "epc": float(o.epc),
            "approved": True,
        }
...
async def recompute_partner_scores(db: AsyncSession, org_id: uuid.UUID) -> dict[str, Any]:
    partners = list(
        (
            await db.execute(
                select(OwnedAffiliatePartner).where(
                    OwnedAffiliatePartner.organization_id == org_id, OwnedAffiliatePartner.is_active.is_(True)
                )
            )
        )
        .scalars()
    ...
    for p in partners:
        convs = list(
            (await db.execute(select(OwnedPartnerConversion).where(OwnedPartnerConversion.partner_id == p.id)))
            .scalars()
            .all()
        )
        pdict = {
            "total_conversions": p.total_conversions,
            "conversion_quality": float(p.conversion_quality),
            "fraud_risk": float(p.fraud_risk),
            "total_revenue_generated": float(p.total_revenue_generated),
        }
...
async def list_approvals(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(AffiliateApproval).where(
                    AffiliateApproval.organization_id == org_id, AffiliateApproval.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def list_risk_flags(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(AffiliateRiskFlag).where(
                    AffiliateRiskFlag.organization_id == org_id, AffiliateRiskFlag.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def list_partners(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(OwnedAffiliatePartner)
                .where(OwnedAffiliatePartner.organization_id == org_id, OwnedAffiliatePartner.is_active.is_(True))
                .order_by(OwnedAffiliatePartner.partner_score.desc())
            )
        )
        .scalars()
        .all()
...
async def list_audit(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(AffiliateAuditEvent)
                .where(AffiliateAuditEvent.organization_id == org_id)
                .order_by(AffiliateAuditEvent.created_at.desc())
                .limit(50)
            )
        )
        .scalars()
