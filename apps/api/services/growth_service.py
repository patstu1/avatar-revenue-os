...
async def _content_performance_rollups(db: AsyncSession, brand_id: uuid.UUID) -> list[ContentPerfRollup]:
    ...
    for item in items:
        ...
        out.append(
            ContentPerfRollup(
                content_id=str(item.id), title=item.title, brand_id=str(brand_id),
                ...
                impressions=imps, clicks=clicks, views=views, revenue=rev,
                profit=profit, cost=float(item.total_cost or 0),
                ctr=ctr, rpm=rpm, engagement_rate=avg_er, avg_watch_pct=avg_watch,
                conversions=int(conv), conversion_rate=cvr,
            )
        )
    return out
...
async def recompute_growth_intel(
    ...
    offer_by_id = {
        str(o.id): {
            "id": str(o.id), "epc": o.epc, "conversion_rate": o.conversion_rate,
            "payout_amount": o.payout_amount, "average_order_value": o.average_order_value,
            "recurring_commission": o.recurring_commission, "priority": o.priority,
        }
        for o in offers
    }
    ...
    for o in offers:
        ...
        for plat in plats:
            row = estimate_ltv_rules(offer_d, plat, geo_d, lang_d, seg_n, seg_n, "organic", float(o.conversion_rate or 0.02))
            db.add(LtvModel(
                brand_id=brand_id,
    ...
    for gr in geo_rows:
        db.add(GeoLanguageExpansionRecommendation(
            brand_id=brand_id,
    ...
    cp_items = [
        ContentPerformance(
            ...
            impressions=r.impressions, revenue=r.revenue, profit=r.profit,
            rpm=r.rpm, ctr=r.ctr, engagement_rate=r.engagement_rate,
            conversion_rate=r.conversion_rate, platform=r.platform,
    ...
    for row in (await db.execute(select(PaidAmplificationJob.content_item_id).where(PaidAmplificationJob.brand_id == brand_id))).all():
    ...
    for c in candidates:
        ...
        for r in rollups:
            if r.content_id == c["content_item_id"]:
                ...
                break
        db.add(PaidAmplificationJob(
            brand_id=brand_id, content_item_id=cid, creator_account_id=acct_id,
    ...
    for a in accounts:
        ...
        db.add(TrustSignalReport(
            brand_id=brand_id, creator_account_id=a.id,
            trust_score=tr["trust_score"], components=tr["components"],
            recommendations=tr["recommendations"], evidence=tr["evidence"],
...
async def get_ltv_models(db: AsyncSession, brand_id: uuid.UUID) -> list[LtvModel]:
    q = await db.execute(
        select(LtvModel).where(LtvModel.brand_id == brand_id, LtvModel.is_active.is_(True)).limit(200)
    )
    return list(q.scalars().all())
