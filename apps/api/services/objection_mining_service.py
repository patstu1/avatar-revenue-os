...
async def recompute_objections(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    comments = list(
        (await db.execute(select(CommentIngestion).where(CommentIngestion.brand_id == brand_id).limit(500)))
        .scalars()
        .all()
    )
...
async def list_signals(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(ObjectionSignal)
                .where(ObjectionSignal.brand_id == brand_id, ObjectionSignal.is_active.is_(True))
                .order_by(ObjectionSignal.monetization_impact.desc())
                .limit(100)
            )
        )
        .scalars()
...
async def list_responses(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(ObjectionResponse)
                .where(ObjectionResponse.brand_id == brand_id, ObjectionResponse.is_active.is_(True))
                .order_by(ObjectionResponse.priority)
            )
        )
        .scalars()
        .all()
