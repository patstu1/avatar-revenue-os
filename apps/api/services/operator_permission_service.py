        ...
        (
            await db.execute(
                select(OperatorPermissionMatrix).where(
                    OperatorPermissionMatrix.organization_id == org_id, OperatorPermissionMatrix.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def check_action(db: AsyncSession, org_id: uuid.UUID, action_class: str) -> dict[str, Any]:
    ...
    policies = list(
        (
            await db.execute(
                select(AutonomyActionPolicy).where(
                    AutonomyActionPolicy.organization_id == org_id, AutonomyActionPolicy.is_active.is_(True)
                )
            )
        )
        .scalars()
    ...
    m_dicts = [
        {
            ...
            "override_role": m.override_role,
            "is_active": m.is_active,
        }
        for m in matrix
    ]
    p_dicts = [
        {"action_class": p.action_class, "default_mode": p.default_mode, "is_active": p.is_active} for p in policies
    ]
...
async def list_matrix(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(OperatorPermissionMatrix)
                .where(OperatorPermissionMatrix.organization_id == org_id, OperatorPermissionMatrix.is_active.is_(True))
                .order_by(OperatorPermissionMatrix.action_class)
            )
        )
        .scalars()
        .all()
...
async def list_execution_modes(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(ActionExecutionMode).where(
                    ActionExecutionMode.organization_id == org_id, ActionExecutionMode.is_active.is_(True)
                )
            )
        )
        .scalars()
