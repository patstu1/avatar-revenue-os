...
async def _revenue_section(db: AsyncSession, brand_id: uuid.UUID, now: datetime) -> dict[str, Any]:
    ...
    today_rev = float(
        (
            await db.execute(
                select(func.coalesce(func.sum(CreatorRevenueEvent.revenue), 0)).where(
                    CreatorRevenueEvent.brand_id == brand_id, CreatorRevenueEvent.created_at >= today_start
                )
            )
        ).scalar()
        or 0
    ...
    week_rev = float(
        (
            await db.execute(
                select(func.coalesce(func.sum(CreatorRevenueEvent.revenue), 0)).where(
                    CreatorRevenueEvent.brand_id == brand_id, CreatorRevenueEvent.created_at >= day7
                )
            )
        ).scalar()
        or 0
    ...
    month_rev = float(
        (
            await db.execute(
                select(func.coalesce(func.sum(CreatorRevenueEvent.revenue), 0)).where(
                    CreatorRevenueEvent.brand_id == brand_id, CreatorRevenueEvent.created_at >= day30
                )
            )
        ).scalar()
        or 0
    ...
    mtd_rev = float(
        (
            await db.execute(
                select(func.coalesce(func.sum(CreatorRevenueEvent.revenue), 0)).where(
                    CreatorRevenueEvent.brand_id == brand_id, CreatorRevenueEvent.created_at >= month_start
                )
            )
        ).scalar()
        or 0
    ...
    mtd_perf_rev = float(
        (
            await db.execute(
                select(func.coalesce(func.sum(PerformanceMetric.revenue), 0)).where(
    ...
    try:
        ...
        if daily_revs and len(daily_revs) >= 7:
            ...
            forecast_data = {
                "forecast_30d": fc["forecast_revenue_30d"],
                "trend": fc["trend_direction"],
                "confidence": fc["confidence"],
                "summary": generate_forecast_summary(fc),
            }
    except Exception:
...
async def _provider_health(db: AsyncSession, brand_id: uuid.UUID) -> list[dict[str, Any]]:
    ...
    readiness = list(
        (await db.execute(select(ProviderReadinessReport).where(ProviderReadinessReport.brand_id == brand_id)))
        .scalars()
        .all()
    )
    blockers = list(
        ...
        providers.append(
            {
                ...
                "blockers": [
                    {"type": b.blocker_type, "severity": b.severity, "action": b.operator_action_needed} for b in bs
                ],
            }
        )
    ...
    return sorted(
        providers,
        key=lambda p: {"blocked": 0, "needs_attention": 1, "degraded": 2, "healthy": 3, "inactive": 4}.get(
            p["status"], 5
        ),
    )
...
async def _platform_health(db: AsyncSession, brand_id: uuid.UUID) -> list[dict[str, Any]]:
    ...
    return sorted(
        platforms,
        key=lambda p: {"blocked": 0, "weak": 1, "saturated": 2, "warming": 3, "healthy": 4}.get(p["status"], 5),
    )
...
async def _alert_layer(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    critical_alerts = list(
        (
            await db.execute(
                ...
                .where(
                    OperatorAlert.brand_id == brand_id, OperatorAlert.urgency >= 0.8, OperatorAlert.is_active.is_(True)
                )
    ...
    suppressions = list(
        (
            await db.execute(
                ...
                .where(SuppressionRule.brand_id == brand_id, SuppressionRule.is_active.is_(True))
                .limit(5)
            )
        )
        .scalars()
    ...
    top_actions = list(
        (
            await db.execute(
                select(RankedAction)
                .where(RankedAction.brand_id == brand_id, RankedAction.is_active.is_(True))
                .order_by(RankedAction.rank_position)
                .limit(5)
            )
        )
        .scalars()
    ...
    return {
        "critical_alerts": [
            {"title": a.title, "urgency": a.urgency, "action": a.recommended_action} for a in critical_alerts
        ],
        "quality_blocks": [{"reason": b.block_reason, "severity": b.severity} for b in quality_blocks],
        "active_suppressions": [
            {"type": s.family_type, "key": s.family_key, "mode": s.suppression_mode} for s in suppressions
        ],
        "top_actions": [
            {
