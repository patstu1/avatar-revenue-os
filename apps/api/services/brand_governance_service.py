...
async def evaluate_content(db: AsyncSession, brand_id: uuid.UUID, content_item_id: uuid.UUID) -> dict[str, Any]:
    ...
    profile = (
        await db.execute(
            select(BrandGovernanceProfile)
            .where(BrandGovernanceProfile.brand_id == brand_id, BrandGovernanceProfile.is_active.is_(True))
            .limit(1)
        )
    ).scalar_one_or_none()
    voice_rules = list(
        (
            await db.execute(
                select(BrandVoiceRule).where(BrandVoiceRule.brand_id == brand_id, BrandVoiceRule.is_active.is_(True))
            )
        )
        .scalars()
        .all()
    ...
    editorial_rules = list(
        (
            await db.execute(
                select(BrandEditorialRule).where(
                    BrandEditorialRule.brand_id == brand_id, BrandEditorialRule.is_active.is_(True)
                )
            )
        )
        .scalars()
    ...
    rule_dicts = [
        ...
        for r in voice_rules
    ]
    violations = evaluate_voice_rules(text, rule_dicts)

    profile_dict = {"tone_profile": profile.tone_profile if profile else ""}
    ed_dicts = [
        {"rule_category": r.rule_category, "check_type": r.check_type, "check_value": r.check_value}
        for r in editorial_rules
    ]
    ct = ci.content_type
    content_dict = {
        "body_text": ci.description or "",
        "hook_text": ci.title or "",
        "content_form": ct.value if hasattr(ct, "value") else str(ct),
    ...
    await db.execute(
        delete(BrandGovernanceViolation).where(BrandGovernanceViolation.content_item_id == content_item_id)
    )
    for v in violations:
        rid = None
        if v.get("rule_id"):
            try:
                rid = uuid.UUID(str(v["rule_id"]))
    ...
    if editorial["verdict"] == "fail":
        db.add(
            BrandGovernanceViolation(
                brand_id=brand_id,
                content_item_id=content_item_id,
                violation_type="editorial_fail",
                severity="hard",
                detail=f"Editorial score {editorial['total_score']:.2f} below threshold",
            )
        )
...
async def list_profiles(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(BrandGovernanceProfile).where(
                    BrandGovernanceProfile.brand_id == brand_id, BrandGovernanceProfile.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def list_knowledge_bases(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(BrandKnowledgeBase).where(
                    BrandKnowledgeBase.brand_id == brand_id, BrandKnowledgeBase.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def list_audiences(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(BrandAudienceProfile).where(
                    BrandAudienceProfile.brand_id == brand_id, BrandAudienceProfile.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def list_assets(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(BrandAssetLibrary).where(
                    BrandAssetLibrary.brand_id == brand_id, BrandAssetLibrary.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def list_violations(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(BrandGovernanceViolation)
                .where(BrandGovernanceViolation.brand_id == brand_id, BrandGovernanceViolation.is_active.is_(True))
                .order_by(BrandGovernanceViolation.created_at.desc())
                .limit(50)
            )
        )
        .scalars()
...
async def list_approvals(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(BrandGovernanceApproval).where(
                    BrandGovernanceApproval.brand_id == brand_id, BrandGovernanceApproval.is_active.is_(True)
                )
            )
        )
        .scalars()
