...
async def assign_offer_to_content(
    ...
    *, org_id: uuid.UUID | None = None, actor_id: str | None = None,
    ...
    item.monetization_method = offer.monetization_method.value if hasattr(offer.monetization_method, 'value') else str(offer.monetization_method) if offer.monetization_method else None
...
async def record_affiliate_commission_to_ledger(
    ...
    entry = RevenueLedgerEntry(
        ...
        brand_id=brand_id, offer_id=offer_id,
        content_item_id=content_item_id, creator_account_id=creator_account_id,
        affiliate_link_id=affiliate_link_id,
        gross_amount=gross_amount, net_amount=net_amount if net_amount is not None else gross_amount - platform_fee,
        platform_fee=platform_fee, currency="USD",
    ...
    await emit_event(
        db, domain="monetization", event_type="ledger.affiliate_commission",
...
async def record_sponsor_payment_to_ledger(
    ...
    entry = RevenueLedgerEntry(
        ...
        brand_id=brand_id, sponsor_id=sponsor_id,
        content_item_id=content_item_id, creator_account_id=creator_account_id,
    ...
    org_id = (await db.execute(select(Brand.organization_id).where(Brand.id == brand_id))).scalar()
    await emit_event(
        db, domain="monetization", event_type="ledger.sponsor_payment",
...
async def record_service_payment_to_ledger(
    ...
    entry = RevenueLedgerEntry(
        ...
        brand_id=brand_id, content_item_id=content_item_id,
    ...
    org_id = (await db.execute(select(Brand.organization_id).where(Brand.id == brand_id))).scalar()
    await emit_event(
        db, domain="monetization", event_type="ledger.service_payment",
...
async def record_product_sale_to_ledger(
    ...
    entry = RevenueLedgerEntry(
        ...
        source_object_id=source_object_id, source_object_table="product_launches" if source_object_id else None,
        brand_id=brand_id, offer_id=offer_id, content_item_id=content_item_id,
        gross_amount=gross_amount, net_amount=net_amount if net_amount is not None else gross_amount - platform_fee,
        platform_fee=platform_fee, currency="USD",
        payment_state="confirmed",
    ...
    org_id = (await db.execute(select(Brand.organization_id).where(Brand.id == brand_id))).scalar()
    await emit_event(
        db, domain="monetization", event_type="ledger.product_sale",
...
async def record_ad_revenue_to_ledger(
    ...
    entry = RevenueLedgerEntry(
        ...
        brand_id=brand_id, content_item_id=content_item_id,
        creator_account_id=creator_account_id,
        gross_amount=gross_amount, net_amount=gross_amount - platform_fee,
        platform_fee=platform_fee, currency="USD",
        payment_state="confirmed",
...
async def record_refund_to_ledger(
    ...
    original = (await db.execute(
        select(RevenueLedgerEntry).where(RevenueLedgerEntry.id == refund_of_id)
    )).scalar_one_or_none()

    entry = RevenueLedgerEntry(
        revenue_source_type="refund",
        source_object_id=refund_of_id, source_object_table="revenue_ledger",
        brand_id=brand_id,
        offer_id=original.offer_id if original else None,
        content_item_id=original.content_item_id if original else None,
        gross_amount=-abs(refund_amount), net_amount=-abs(refund_amount),
        currency="USD", payment_state="confirmed",
        attribution_state=original.attribution_state if original else "unattributed",
        is_refund=True, refund_of_id=refund_of_id,
        dispute_reason=reason, webhook_ref=webhook_ref,
...
async def get_ledger_summary(
    db: AsyncSession, brand_id: uuid.UUID, *, days: int = 30,
) -> dict:
    ...
    by_source_q = await db.execute(
        ...
        .where(RevenueLedgerEntry.brand_id == brand_id, RevenueLedgerEntry.occurred_at >= cutoff, RevenueLedgerEntry.is_active.is_(True))
    ...
    entry_count = (await db.execute(
        select(func.count()).select_from(RevenueLedgerEntry).where(
            RevenueLedgerEntry.brand_id == brand_id, RevenueLedgerEntry.occurred_at >= cutoff
        )
    )).scalar() or 0
    ...
    unattributed = (await db.execute(
        select(func.count()).select_from(RevenueLedgerEntry).where(
            RevenueLedgerEntry.brand_id == brand_id, RevenueLedgerEntry.attribution_state == "unattributed",
            RevenueLedgerEntry.occurred_at >= cutoff, RevenueLedgerEntry.is_refund.is_(False),
        )
    )).scalar() or 0
...
async def get_brand_revenue_state(db: AsyncSession, brand_id: uuid.UUID) -> dict:
    ...
    ledger_30d = (await db.execute(
        select(func.coalesce(func.sum(RevenueLedgerEntry.gross_amount), 0.0)).where(
            RevenueLedgerEntry.brand_id == brand_id, RevenueLedgerEntry.occurred_at >= day_30, RevenueLedgerEntry.is_active.is_(True)
        )
    )).scalar() or 0.0

    ledger_7d = (await db.execute(
        select(func.coalesce(func.sum(RevenueLedgerEntry.gross_amount), 0.0)).where(
            RevenueLedgerEntry.brand_id == brand_id, RevenueLedgerEntry.occurred_at >= day_7, RevenueLedgerEntry.is_active.is_(True)
        )
    )).scalar() or 0.0
    ...
    by_source_q = await db.execute(
        select(RevenueLedgerEntry.revenue_source_type, func.sum(RevenueLedgerEntry.gross_amount))
        .where(RevenueLedgerEntry.brand_id == brand_id, RevenueLedgerEntry.occurred_at >= day_30, RevenueLedgerEntry.is_active.is_(True))
        .group_by(RevenueLedgerEntry.revenue_source_type)
    )
    ...
    pending = (await db.execute(
        select(func.coalesce(func.sum(RevenueLedgerEntry.gross_amount), 0.0)).where(
            RevenueLedgerEntry.brand_id == brand_id, RevenueLedgerEntry.payment_state == "pending",
            RevenueLedgerEntry.is_active.is_(True)
        )
    )).scalar() or 0.0

    # Legacy sources (backward compat — included in totals if ledger is empty)
    legacy_revenue = (await db.execute(
    ...
    monetized_content = (await db.execute(
        select(func.count()).select_from(ContentItem).where(
            ContentItem.brand_id == brand_id, ContentItem.offer_id.isnot(None)
        )
    )).scalar() or 0
...
async def attribute_revenue_event(
    ...
    attr = AttributionEvent(
        brand_id=brand_id, content_item_id=content_item_id, offer_id=offer_id,
        event_type=event_type, event_value=revenue, attribution_model="last_click",
    ...
    source_map = {"affiliate": "affiliate_commission", "sponsor": "sponsor_payment",
                  "service": "service_fee", "product": "product_sale", "webhook": "service_fee"}
    ledger_source = source_map.get(source, "service_fee")

    ledger = RevenueLedgerEntry(
        revenue_source_type=ledger_source, brand_id=brand_id,
        offer_id=offer_id, content_item_id=content_item_id,
        gross_amount=revenue, net_amount=revenue, currency="USD",
        payment_state="confirmed", attribution_state="auto_attributed" if offer_id else "manually_attributed",
    ...
    await emit_event(
        ...
        org_id=org_id, brand_id=brand_id, entity_type="revenue_ledger", entity_id=ledger.id,
...
async def surface_monetization_actions(
    ...
    for item in unmonetized.scalars().all():
        action = await emit_action(
            db, org_id=org_id, action_type="assign_offer",
            title=f"Monetize: {item.title[:50]}",
            description="Published content with no offer. Assign an offer to earn.",
            category="monetization", priority="medium", brand_id=brand_id,
            entity_type="content_item", entity_id=item.id, source_module="monetization_bridge",
        )
    ...
    unattributed = await db.execute(
        select(RevenueLedgerEntry).where(
            ...
            RevenueLedgerEntry.is_refund.is_(False), RevenueLedgerEntry.is_active.is_(True),
        ).limit(5)
    )
    for entry in unattributed.scalars().all():
        action = await emit_action(
            db, org_id=org_id, action_type="attribute_revenue",
            ...
            category="monetization", priority="high", brand_id=brand_id,
            entity_type="revenue_ledger", entity_id=entry.id, source_module="monetization_bridge",
        )
    ...
    for offer in orphan_offers.scalars().all():
        ...
        if content_count == 0:
            action = await emit_action(
                db, org_id=org_id, action_type="create_content_for_offer",
                title=f"No content for: {offer.name[:50]}",
                description=f"Active offer (${offer.payout_amount or 0:.2f} payout) has no content.",
                category="monetization", priority="medium", brand_id=brand_id,
                entity_type="offer", entity_id=offer.id, source_module="monetization_bridge",
            )
    ...
    for b in blockers.scalars().all():
        action = await emit_action(
            db, org_id=org_id, action_type="resolve_revenue_blocker",
            ...
            category="monetization", priority="high" if b.severity == "critical" else "medium",
            brand_id=brand_id, entity_type="revenue_blocker", entity_id=b.id,
...
async def get_content_with_offers(db: AsyncSession, brand_id: uuid.UUID, *, limit: int = 50) -> list[dict]:
    ...
    q = await db.execute(
        select(ContentItem, Offer).outerjoin(Offer, ContentItem.offer_id == Offer.id)
        .where(ContentItem.brand_id == brand_id)
        .order_by(ContentItem.created_at.desc()).limit(limit)
    )
    return [
        {
            "content_id": str(row[0].id), "title": row[0].title, "status": row[0].status,
            "offer": {"id": str(row[1].id), "name": row[1].name, "payout_amount": float(row[1].payout_amount) if row[1].payout_amount else None} if row[1] else None,
        }
        for row in q.all()
    ]
