...
async def recompute_landing_pages(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    await db.execute(
        delete(LandingPageBlock).where(
            LandingPageBlock.page_id.in_(select(LandingPage.id).where(LandingPage.brand_id == brand_id))
        )
    )
    await db.execute(
        delete(LandingPageVariant).where(
            LandingPageVariant.page_id.in_(select(LandingPage.id).where(LandingPage.brand_id == brand_id))
        )
    )
        ...
        (
            await db.execute(
                select(ObjectionCluster).where(
                    ObjectionCluster.brand_id == brand_id, ObjectionCluster.is_active.is_(True)
                )
            )
        )
        .scalars()
    ...
    for offer in offers:
        for pt in ("product", "review", "presell"):
            ...
            page = LandingPage(
                brand_id=brand_id,
                offer_id=offer.id,
                **{k: v for k, v in spec.items() if k not in ("status", "publish_status", "truth_label") or True},
            )
            db.add(page)
            await db.flush()
...
async def list_pages(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(LandingPage)
                .where(LandingPage.brand_id == brand_id, LandingPage.is_active.is_(True))
...
async def list_variants(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(LandingPageVariant)
                .join(LandingPage)
                .where(LandingPage.brand_id == brand_id, LandingPageVariant.is_active.is_(True))
            )
        )
        .scalars()
        .all()
...
async def list_quality(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(LandingPageQualityReport).where(
                    LandingPageQualityReport.brand_id == brand_id, LandingPageQualityReport.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def get_best_page_for_offer(db: AsyncSession, brand_id: uuid.UUID, offer_id: uuid.UUID) -> dict[str, Any]:
    page = (
        await db.execute(
            select(LandingPage)
            .where(LandingPage.brand_id == brand_id, LandingPage.offer_id == offer_id, LandingPage.is_active.is_(True))
            .order_by(LandingPage.created_at.desc())
            .limit(1)
        )
    ).scalar_one_or_none()
...
async def publish_page(
    db: AsyncSession, page_id: uuid.UUID, publish_method: str = "manual", destination_url: str = ""
) -> dict[str, Any]:
    ...
    quality = (
        await db.execute(
            select(LandingPageQualityReport)
            .where(LandingPageQualityReport.page_id == page_id, LandingPageQualityReport.is_active.is_(True))
            .order_by(LandingPageQualityReport.created_at.desc())
            .limit(1)
        )
    ).scalar_one_or_none()
