...
async def recompute_alerts(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    (
        ...
        cann_risk,
        aud_sep,
    ) = await _brand_context(db, brand_id)
    ...
    for ad in alerts_data:
        alert = OperatorAlert(
            ...
            urgency=ad["urgency"],
            expected_upside=ad["expected_upside"],
            expected_cost=ad["expected_cost"],
    ...
    top_c = (
        (
            await db.execute(
                ...
                .where(
                    LaunchCandidate.brand_id == brand_id,
                    LaunchCandidate.is_active.is_(True),
                )
...
async def recompute_launch_candidates(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    for cd in candidates:
        lc = LaunchCandidate(
            brand_id=brand_id,
            candidate_type=cd["candidate_type"],
            primary_platform=cd["primary_platform"],
            secondary_platform=cd.get("secondary_platform"),
            niche=cd["niche"],
            sub_niche=cd.get("sub_niche"),
            ...
            content_style=cd.get("content_style"),
            posting_strategy=cd.get("posting_strategy"),
...
async def recompute_scale_blockers(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    await db.execute(
        delete(ScaleBlockerReport).where(
...
async def recompute_launch_readiness(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    db.add(
        LaunchReadinessReport(
            brand_id=brand_id,
            launch_readiness_score=result["launch_readiness_score"],
            explanation=result["explanation"],
            recommended_action=result["recommended_action"],
            gating_factors=result["gating_factors"],
...
async def get_launch_candidates(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list(
        (
            await db.execute(
                select(LaunchCandidate)
                .where(LaunchCandidate.brand_id == brand_id, LaunchCandidate.is_active.is_(True))
                .order_by(LaunchCandidate.urgency.desc())
            )
        )
        .scalars()
    ...
    return [
        {
            ...
            "recommended_fix": b.recommended_fix,
            "current_value": b.current_value,
            "threshold_value": b.threshold_value,
        }
        for b in rows
...
async def get_launch_readiness(db: AsyncSession, brand_id: uuid.UUID) -> dict | None:
    ...
    return {
        "id": str(r.id),
        "launch_readiness_score": r.launch_readiness_score,
        "explanation": r.explanation,
        "recommended_action": r.recommended_action,
        "gating_factors": r.gating_factors,
    ...
    rows = list(
        (
            await db.execute(
                ...
                .where(NotificationDelivery.brand_id == brand_id)
...
def _ser_alert(a: OperatorAlert) -> dict:
    ...
    return {
        ...
        "urgency": a.urgency,
        "expected_upside": a.expected_upside,
        ...
        "linked_scale_recommendation_id": str(a.linked_scale_recommendation_id)
        if a.linked_scale_recommendation_id
        else None,
...
def _ser_candidate(c: LaunchCandidate) -> dict:
    return {
        ...
        "linked_scale_recommendation_id": str(c.linked_scale_recommendation_id)
        if c.linked_scale_recommendation_id
        else None,
    }
