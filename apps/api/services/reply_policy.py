...
class ReplyPolicySettings:
    ...
    avatar_allowed_cta_verbs: frozenset[str] = frozenset({
        "secure", "finish", "complete", "review", "proceed",
    })
...
FORCED_ESCALATION_PATTERNS: list[tuple[str, str]] = [
    (r"(?i)\b(lawyer|attorney|legal\s+action|legal\s+counsel|legal\s+team|counsel\b|sue\b|suing|lawsuit|litigation|cease\s+and\s+desist)\b", "legal_threat"),
    (r"(?i)\b(refund|chargeback|dispute\s+(the\s+)?charge|money\s+back|return\s+my\s+money|reverse\s+(the\s+)?payment)\b", "refund_dispute"),
    (r"(?i)\b(fraud(?:ulent)?|scam|scammed|deceptive|deceived|misled|mislead|ripped\s+off|con\s+artist)\b", "fraud_accusation"),
    (r"(?i)\b(furious|outraged|livid|appalled|disgusted|unacceptable|unprofessional|incompetent|disgraceful|shameful)\b", "hostile_tone"),
    (r"(?i)\b(harass(?:ment|ing)?|threat(?:en|ening)?|intimidat(?:e|ing|ion)|retaliat(?:e|ion)|stalking)\b", "harassment"),
    (r"(?i)\b(fuck|f\*+ck|shit|bullshit|asshole|a\*+hole|bastard|piss\s+off|screw\s+you|go\s+to\s+hell)\b", "profanity"),
    (r"(?i)\b(report\s+you|file\s+(a\s+)?complaint|better\s+business\s+bureau|\bbbb\b|consumer\s+protection|ftc\b)\b", "complaint_threat"),
]
...
FORCED_DRAFT_PATTERNS: list[tuple[str, str]] = [
    ...
    (r"(?i)\b(over\s+budget|out\s+of\s+budget|no\s+budget|tight\s+budget|limited\s+budget|shoestring|strapped\s+for\s+cash)\b", "budget_pushback"),
    (r"(?i)\b(too\s+expensive|too\s+much|too\s+pricey|too\s+costly|cost\s+too\s+much|can'?t\s+afford|out\s+of\s+(our\s+)?price\s+range)\b", "pricing_objection"),
    # Custom scope / tailoring asks
    (r"(?i)\b(customi[sz]e|customi[sz]ation|custom\s+(scope|package|quote|plan|bundle|offer|proposal|project)|tailor(ed)?|bespoke)\b", "custom_scope"),
    (r"(?i)\b(something\s+(smaller|simpler|different|else|custom|more\s+basic)|scaled\s+down|mini\s+version|stripped\s+down|light\s+version)\b", "custom_scope"),
    (r"(?i)\bcan\s+you\s+(do|make|offer|throw\s+in|include|add)\s+(something|a\s+smaller|a\s+custom|more|less|less\s+expensive)\b", "custom_request"),
    (r"(?i)\b(what\s+if\s+we|would\s+you\s+(be\s+)?willing\s+to|any\s+room\s+to|flexible\s+on|wiggle\s+room|negotiate\b)\b", "negotiation_signal"),
    (r"(?i)\b(discount|reduce(d)?\s+(price|rate|cost)|lower\s+(the\s+)?(price|rate|cost)|price\s+break|price\s+cut|deal\s+on\s+the\s+price)\b", "discount_ask"),
    # Phone / call / meeting asks — operator handles these by hand
    (r"(?i)\b(hop\s+on\s+(a\s+)?(quick\s+)?(call|chat|zoom)|jump\s+on\s+(a\s+)?(call|chat|zoom)|quick\s+(phone\s+)?call|phone\s+call|zoom\s+(link|call|meeting)|schedule\s+a\s+call|book\s+a\s+call|get\s+on\s+(a\s+)?call)\b", "call_request"),
    # Enterprise / partnership / revenue share
    (r"(?i)\b(enterprise\s+(deal|contract|agreement|plan|pricing|inquiry|account)|corporate\s+agreement|master\s+service\s+agreement|\bmsa\b|legal\s+review|procurement)\b", "enterprise_inquiry"),
    (r"(?i)\b(partnership|partner\s+(with|up)|strategic\s+partner|joint\s+venture|co.?brand(?:ing)?|affiliate\s+program|referral\s+program|white.?label)\b", "partnership_inquiry"),
    (r"(?i)\b(revenue\s+share|rev\s+share|revshare|profit\s+share|equity\s+(split|share|stake)|commission\s+structure|royalty)\b", "revenue_share_inquiry"),
    # High-value / unusual commercial
    (r"(?i)\b(six.?figure|seven.?figure|\$\s*\d{1,3}[,.]?\d{3}[,.]?\d{3}|large\s+(contract|deal|project|engagement)|big\s+(project|budget))\b", "high_value_inquiry"),
]
...
_AUTOMATION_SUBJECT_PATTERNS = [
    (r"(?i)^(re:\s*)?(undeliverable|delivery\s+status|mail\s+delivery|returned\s+mail|delivery\s+(failure|has\s+failed))", "bounce_subject"),
    (r"(?i)\b(auto[._-]?reply|automatic\s+reply|auto\s+response|out\s+of\s+(the\s+)?office|currently\s+(out|away)|away\s+from\s+(the\s+)?office|maternity|paternity|annual\s+leave)\b", "ooo_subject"),
    (r"(?i)^(re:\s*)?(welcome\s+to|verify\s+your|confirm\s+your|activate\s+your|your\s+.+\s+(receipt|invoice|statement))", "transactional_subject"),
]
    ...
    *,
    ...
    if confidence < settings.auto_send_min_confidence:
        ...
        trace.rules_evaluated.append(
            f"confidence:MISS:{confidence:.2f}<{settings.auto_send_min_confidence:.2f}"
        )
    ...
    trace.rules_evaluated.append(
        f"confidence:PASS:{confidence:.2f}>={settings.auto_send_min_confidence:.2f}"
    )
    ...
    if recent_auto_reply_in_thread:
        ...
        trace.rationale = (
            f"thread already has an auto-reply in last {settings.thread_cooldown_hours}h"
        )
        return trace
    ...
    trace.rationale = (
        f"intent={intent} conf={confidence:.2f} standard template — all safety rails passed"
    )
    return trace
