...
async def recompute_recovery(db: AsyncSession, org_id: uuid.UUID) -> dict[str, Any]:
    await db.execute(
        delete(RecoveryOutcome).where(
            RecoveryOutcome.incident_id.in_(
                select(RecoveryIncidentV2.id).where(RecoveryIncidentV2.organization_id == org_id)
            )
        )
    )
    await db.execute(
        delete(ThrottlingAction).where(
            ThrottlingAction.incident_id.in_(
                select(RecoveryIncidentV2.id).where(RecoveryIncidentV2.organization_id == org_id)
            )
        )
    )
    await db.execute(
        delete(RerouteAction).where(
            RerouteAction.incident_id.in_(
                select(RecoveryIncidentV2.id).where(RecoveryIncidentV2.organization_id == org_id)
            )
        )
    )
    await db.execute(
        delete(RollbackAction).where(
            RollbackAction.incident_id.in_(
                select(RecoveryIncidentV2.id).where(RecoveryIncidentV2.organization_id == org_id)
            )
        )
    )
    ...
    for inc in incidents:
        ...
        if inc.get("affected_id"):
            try:
                aid = uuid.UUID(str(inc["affected_id"]))
            except (ValueError, AttributeError):
                pass
        rec_inc = RecoveryIncidentV2(
            ...
            affected_id=aid,
            detail=inc["detail"],
            auto_recoverable=inc["auto_recoverable"],
        )
        db.add(rec_inc)
        await db.flush()
        ...
        rec_inc.recovery_status = (
            "escalated"
            if escalate
            else "auto_recovering"
            if recovery["decision"] == "auto_recover"
            else "pending_review"
        )
        ...
        for action in recovery.get("actions", []):
            ...
            if atype == "rollback":
                db.add(
                    RollbackAction(
                        incident_id=rec_inc.id,
                        rollback_type=atype,
                        rollback_target=action.get("target", ""),
                ...
                db.add(
                    ThrottlingAction(
                        incident_id=rec_inc.id,
                        throttle_target=action.get("target", ""),
                        throttle_level=action.get("level", "50pct"),
                        reason=action.get("detail", ""),
                        execution_status="pending",
                    )
                )
...
async def _gather_state(db: AsyncSession, org_id: uuid.UUID) -> dict[str, Any]:
    ...
    prov_blockers = list(
        (await db.execute(select(ProviderBlocker).where(ProviderBlocker.is_active.is_(True)).limit(10))).scalars().all()
    )
    ...
    deg_events = list(
        (
            await db.execute(
                ...
                .where(
                    DegradationEvent.organization_id == org_id,
                    DegradationEvent.recovered.is_(False),
                    DegradationEvent.is_active.is_(True),
                )
                .limit(5)
            )
        )
...
async def list_incidents(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(RecoveryIncidentV2)
                .where(RecoveryIncidentV2.organization_id == org_id, RecoveryIncidentV2.is_active.is_(True))
                .order_by(RecoveryIncidentV2.created_at.desc())
                .limit(50)
            )
        )
        .scalars()
    ...
    return list(
        (
            await db.execute(
                ...
                .join(RecoveryIncidentV2)
                .where(RecoveryIncidentV2.organization_id == org_id, RollbackAction.is_active.is_(True))
            )
        )
        .scalars()
        .all()
...
async def list_reroutes(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(RerouteAction)
                .join(RecoveryIncidentV2)
                .where(RecoveryIncidentV2.organization_id == org_id, RerouteAction.is_active.is_(True))
            )
        )
        .scalars()
        .all()
...
async def list_throttles(db: AsyncSession, org_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(ThrottlingAction)
                .join(RecoveryIncidentV2)
                .where(RecoveryIncidentV2.organization_id == org_id, ThrottlingAction.is_active.is_(True))
            )
        )
        .scalars()
        .all()
    ...
    return list(
        (
            await db.execute(
                ...
                .join(RecoveryIncidentV2)
                .where(RecoveryIncidentV2.organization_id == org_id, RecoveryOutcome.is_active.is_(True))
            )
        )
        .scalars()
        .all()
...
async def get_recovery_summary(db: AsyncSession, org_id: uuid.UUID) -> dict[str, Any]:
        ...
        (
            await db.execute(
                ...
                .where(
                    ...
                    RecoveryIncidentV2.recovery_status != "resolved",
    ...
    return {
        ...
        "incidents": [
            {"type": i.incident_type, "severity": i.severity, "status": i.recovery_status} for i in incidents[:3]
        ],
    }
...
async def execute_pending_recovery_actions(db: AsyncSession, org_id: uuid.UUID) -> dict[str, Any]:
    ...
    for incident in auto_incidents:
        rollbacks = list(
            (
                await db.execute(
                    select(RollbackAction).where(
                        RollbackAction.incident_id == incident.id, RollbackAction.execution_status == "pending"
                    )
                )
            )
            .scalars()
        ...
        reroutes = list(
            (
                await db.execute(
                    select(RerouteAction).where(
                        RerouteAction.incident_id == incident.id, RerouteAction.execution_status == "pending"
                    )
                )
            )
            .scalars()
        ...
        throttles = list(
            (
                await db.execute(
                    select(ThrottlingAction).where(
                        ThrottlingAction.incident_id == incident.id, ThrottlingAction.execution_status == "pending"
                    )
                )
            )
            .scalars()
        ...
        all_done = all(
            a.execution_status in ("executed", "awaiting_approval") for a in rollbacks + reroutes + throttles
        )
