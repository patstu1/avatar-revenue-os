...
def _probe_layers(module_name: str) -> dict[str, bool]:
    ...
    return {
        ...
        if _dir_exists("packages", "db", "alembic", "versions")
        else False,
        "has_engine": (
        ...
        "has_api": (_file_exists("apps", "api", "routers", f"{m}.py")),
        "has_frontend": (
        ...
        "has_docs": any(
            m.replace("_", "") in f.replace("_", "").replace("-", "") for f in os.listdir(os.path.join(_BASE, "docs"))
        )
        if _dir_exists("docs")
        else False,
        "has_worker": (_dir_exists("workers", f"{m}_worker") or _file_exists("workers", f"{m}_worker", "tasks.py")),
    }
...
async def recompute_completion(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    for mod in SYSTEM_MODULES:
        ...
        db.add(
            GatekeeperCompletionReport(
                brand_id=brand_id, **{k: v for k, v in r.items() if k != "module_name"}, module_name=r["module_name"]
            )
        )
        reports.append(r)
    await db.flush()
    return {
        "rows_processed": len(reports),
        "alerts_generated": sum(1 for r in reports if not r["gate_passed"]),
        "status": "completed",
    }
    ...
    for mod in SYSTEM_MODULES:
        ...
        db.add(
            GatekeeperTruthReport(
                ...
                truth_mismatch=r["truth_mismatch"],
                mislabeled_as_live=r["mislabeled_as_live"],
...
async def recompute_execution_closure(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    await db.execute(
        delete(GatekeeperExecutionClosureReport).where(GatekeeperExecutionClosureReport.brand_id == brand_id)
    )
    reports: list[dict] = []
    for mod in SYSTEM_MODULES:
        layers = _probe_layers(mod)
        ...
        db.add(
            GatekeeperExecutionClosureReport(
                brand_id=brand_id,
                module_name=r["module_name"],
                has_execution_path=r["has_execution_path"],
                has_downstream_action=r["has_downstream_action"],
                has_blocker_handling=r["has_blocker_handling"],
                dead_end_detected=r["dead_end_detected"],
                stale_blocker_detected=r["stale_blocker_detected"],
                orphaned_recommendation=r["orphaned_recommendation"],
                gate_passed=r["gate_passed"],
                severity=r["severity"],
                explanation=r.get("explanation"),
            )
...
async def recompute_tests(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    for mod in SYSTEM_MODULES:
        m = mod.replace("-", "_")
        unit_count = (
            sum(1 for f in os.listdir(os.path.join(test_dir, "unit")) if m in f.replace("-", "_"))
            if _dir_exists("tests", "unit")
            else 0
        )
        integ_count = (
            sum(1 for f in os.listdir(os.path.join(test_dir, "integration")) if m in f.replace("-", "_"))
            if _dir_exists("tests", "integration")
            else 0
        )
        ...
        db.add(
            GatekeeperTestReport(
                brand_id=brand_id,
                module_name=r["module_name"],
                unit_test_count=r["unit_test_count"],
                integration_test_count=r["integration_test_count"],
                critical_paths_covered=r["critical_paths_covered"],
                high_risk_flows_tested=r["high_risk_flows_tested"],
                gate_passed=r["gate_passed"],
                severity=r["severity"],
                explanation=r.get("explanation"),
            )
...
async def recompute_dependencies(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    reports: list[dict] = []
    dep_map = {
        "buffer_distribution": "buffer",
        "live_execution_phase2": "stripe",
        "creator_revenue": "stripe",
        "live_execution_phase1": "smtp",
    }
        ...
        if prov:
            ...
            db.add(
                GatekeeperDependencyReport(
                    ...
                    dependency_met=r["dependency_met"],
                    credential_present=r["credential_present"],
                    integration_live=r["integration_live"],
        ...
        states.append(
            {
                "module": mod,
                "status": actual,
                "claims": {"capability": mod, "role": "primary" if layers.get("has_api") else "support"},
                "depends_on": None,
            }
        )
...
async def recompute_operator_commands(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    await db.execute(
        delete(GatekeeperOperatorCommandReport).where(GatekeeperOperatorCommandReport.brand_id == brand_id)
    )
    sample_commands = [
    ...
    for src, text, target, metric, deadline in sample_commands:
        ...
        db.add(
            GatekeeperOperatorCommandReport(
                ...
                command_summary=r["command_summary"],
                is_actionable=r["is_actionable"],
                is_specific=r["is_specific"],
                has_measurable_outcome=r["has_measurable_outcome"],
...
async def recompute_expansion_permissions(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    completion_rows = list(
        (await db.execute(select(GatekeeperCompletionReport).where(GatekeeperCompletionReport.brand_id == brand_id)))
        .scalars()
        .all()
    )
    test_rows = list(
        (await db.execute(select(GatekeeperTestReport).where(GatekeeperTestReport.brand_id == brand_id)))
        .scalars()
        .all()
    )
    dep_rows = list(
        (await db.execute(select(GatekeeperDependencyReport).where(GatekeeperDependencyReport.brand_id == brand_id)))
        .scalars()
        .all()
    )
    ...
    for t in targets:
        ...
        db.add(
            GatekeeperExpansionPermission(
                ...
                blockers_resolved=r["blockers_resolved"],
                test_coverage_sufficient=r["test_coverage_sufficient"],
    ...
    return {
        "rows_processed": len(reports),
        "alerts_generated": sum(1 for r in reports if not r["permission_granted"]),
        "status": "completed",
    }
...
async def recompute_alerts(db: AsyncSession, brand_id: uuid.UUID) -> int:
    ...
    truth = [
        ...
        for r in (await db.execute(select(GatekeeperTruthReport).where(GatekeeperTruthReport.brand_id == brand_id)))
        .scalars()
        .all()
    ]
    closure = [
        ...
        for r in (
            await db.execute(
                select(GatekeeperExecutionClosureReport).where(GatekeeperExecutionClosureReport.brand_id == brand_id)
            )
        )
        .scalars()
        .all()
    ...
    tests = [
        ...
        for r in (await db.execute(select(GatekeeperTestReport).where(GatekeeperTestReport.brand_id == brand_id)))
        .scalars()
        .all()
    ]
    deps = [
    ...
    contras = [
        {"module_a": r.module_a, "module_b": r.module_b, "description": r.description, "severity": r.severity}
        for r in (
            await db.execute(
                select(GatekeeperContradictionReport).where(GatekeeperContradictionReport.brand_id == brand_id)
            )
        )
        .scalars()
        .all()
...
async def list_completion(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(GatekeeperCompletionReport)
                .where(GatekeeperCompletionReport.brand_id == brand_id)
                .order_by(GatekeeperCompletionReport.created_at.desc())
            )
        )
        .scalars()
        .all()
...
async def list_truth(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(GatekeeperTruthReport)
                .where(GatekeeperTruthReport.brand_id == brand_id)
                .order_by(GatekeeperTruthReport.created_at.desc())
            )
        )
        .scalars()
        .all()
...
async def list_execution_closure(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(GatekeeperExecutionClosureReport)
                .where(GatekeeperExecutionClosureReport.brand_id == brand_id)
                .order_by(GatekeeperExecutionClosureReport.created_at.desc())
            )
        )
        .scalars()
        .all()
...
async def list_tests(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(GatekeeperTestReport)
                .where(GatekeeperTestReport.brand_id == brand_id)
                .order_by(GatekeeperTestReport.created_at.desc())
            )
        )
        .scalars()
        .all()
...
async def list_dependencies(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(GatekeeperDependencyReport)
                .where(GatekeeperDependencyReport.brand_id == brand_id)
                .order_by(GatekeeperDependencyReport.created_at.desc())
            )
        )
        .scalars()
        .all()
    ...
    return list(
        (
            await db.execute(
                ...
                .where(GatekeeperContradictionReport.brand_id == brand_id)
...
async def list_operator_commands(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(GatekeeperOperatorCommandReport)
                .where(GatekeeperOperatorCommandReport.brand_id == brand_id)
                .order_by(GatekeeperOperatorCommandReport.created_at.desc())
            )
        )
        .scalars()
        .all()
...
async def list_expansion_permissions(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(GatekeeperExpansionPermission)
                .where(GatekeeperExpansionPermission.brand_id == brand_id)
                .order_by(GatekeeperExpansionPermission.created_at.desc())
            )
        )
        .scalars()
        .all()
    ...
    return list(
        (
            await db.execute(
                ...
                .where(GatekeeperAlert.brand_id == brand_id, GatekeeperAlert.is_active.is_(True))
                .order_by(GatekeeperAlert.created_at.desc())
            )
        )
        .scalars()
        .all()
...
async def list_audit_ledger(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(GatekeeperAuditLedger)
                .where(GatekeeperAuditLedger.brand_id == brand_id)
                .order_by(GatekeeperAuditLedger.created_at.desc())
                .limit(200)
            )
        )
        .scalars()
