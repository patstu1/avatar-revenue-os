...
async def run_simulation(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    await db.execute(
        delete(SimulationOutcome).where(
            SimulationOutcome.scenario_id.in_(
    ...
    await db.execute(
        delete(SimulationAssumption).where(
            SimulationAssumption.scenario_id.in_(
                select(SimulationScenario.id).join(SimulationRun).where(SimulationRun.brand_id == brand_id)
            )
        )
    )
    await db.execute(
        delete(SimulationScenario).where(
            SimulationScenario.run_id.in_(select(SimulationRun.id).where(SimulationRun.brand_id == brand_id))
        )
    )
    ...
    for s in raw_scenarios:
        ...
        for opt_key, opt_data, outcome_data, is_winner in [
            ...
            sc = SimulationScenario(
                ...
                compared_to=s["option_b"]["label"] if opt_key == "a" else s["option_a"]["label"],
                expected_upside=float(opt_data["upside"]),
                expected_cost=float(opt_data["cost"]),
                expected_risk=float(opt_data["risk"]),
            ...
            db.add(
                SimulationOutcome(
                    scenario_id=sc.id,
                    metric="risk_adjusted_profit",
                    predicted_value=outcome_data["risk_adjusted_profit"],
                    confidence=outcome_data["confidence"],
                    risk_adjusted_value=outcome_data["risk_adjusted_profit"],
                )
            )
            db.add(
                SimulationAssumption(
                    scenario_id=sc.id,
                    assumption_key="base_upside",
                    assumption_value=str(opt_data["upside"]),
                    confidence=outcome_data["confidence"],
                )
            )
...
async def _gather_state(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    for s in acct_states:
        if s.current_state in ("scaling", "monetizing"):
            state["scaling_accounts"].append(
                {"id": str(s.account_id), "name": str(s.account_id)[:8], "state": s.current_state}
            )

    winners = list(
        (await db.execute(select(PWExperimentWinner).where(PWExperimentWinner.brand_id == brand_id).limit(5)))
        .scalars()
        .all()
    )
    for w in winners:
    ...
    offers = list(
        (
            await db.execute(
                ...
                .order_by(OfferLabOffer.rank_score)
                .limit(5)
            )
        )
        .scalars()
    ...
    camps = list(
        (
            await db.execute(
                select(Campaign)
                .where(Campaign.brand_id == brand_id, Campaign.is_active.is_(True), Campaign.confidence < 0.4)
                .limit(3)
            )
        )
        .scalars()
    ...
    return list(
        (
            await db.execute(
                ...
                .where(SimulationRun.brand_id == brand_id)
    ...
    return list(
        (
            await db.execute(
                ...
                .join(SimulationRun)
                .where(SimulationRun.brand_id == brand_id, SimulationScenario.is_active.is_(True))
                .order_by(SimulationScenario.expected_upside.desc())
            )
        )
        .scalars()
        .all()
...
async def list_recommendations(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list(
        (
            await db.execute(
                select(SimulationRecommendation)
                .where(SimulationRecommendation.brand_id == brand_id, SimulationRecommendation.is_active.is_(True))
                .order_by(SimulationRecommendation.expected_profit_delta.desc())
            )
        )
        .scalars()
        .all()
