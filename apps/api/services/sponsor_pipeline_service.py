...
async def get_sponsor_pipeline(db: AsyncSession, brand_id: uuid.UUID) -> dict:
    ...
    active_deals = sum(
        d["count"] for s, d in by_stage.items() if s in ("negotiation", "proposal", "active", "delivering")
    )
    ...
    return {
        ...
        "stalled_deals": [
            {"id": str(d.id), "title": d.title, "status": d.status, "deal_value": float(d.deal_value or 0)}
            for d in stalled
        ],
...
async def score_sponsor_fit(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    ...
    for acct in accounts:
        ...
        platform_fit = (
            0.8 if platform in ("youtube", "instagram", "tiktok") else 0.5 if platform in ("linkedin", "x") else 0.3
        )
        ...
        content_count = (
            await db.execute(
                ...
                .where(ContentItem.brand_id == brand_id, ContentItem.creator_account_id == acct.id)
            )
        ).scalar() or 0
        content_score = min(1.0, content_count / 20)

        sponsor_score = 0.30 * audience_score + 0.25 * engagement_score + 0.25 * platform_fit + 0.20 * content_score
...
async def surface_sponsor_actions(
    ...
    for deal in pipeline.get("stalled_deals", [])[:3]:
        a = await emit_action(
            ...
            priority="high",
            brand_id=brand_id,
            source_module="sponsor_pipeline",
            entity_type="sponsor_opportunity",
            entity_id=uuid.UUID(deal["id"]),
        )
