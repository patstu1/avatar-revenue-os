...
async def create_workflow(db: AsyncSession, org_id: uuid.UUID, data: dict[str, Any]) -> WorkflowDefinition:
    wf = WorkflowDefinition(
        ...
        scope_type=data.get("scope_type", "org"),
    ...
    for s in data.get("steps", []):
        db.add(
            WorkflowStep(
                ...
                required_action=s.get("required_action", "approve"),
...
async def create_from_template(
    db: AsyncSession, org_id: uuid.UUID, template_key: str, brand_id: uuid.UUID = None
) -> WorkflowDefinition:
    ...
    return await create_workflow(
        ...
        org_id,
        {
            "workflow_name": f"{template_key} workflow",
...
async def start_instance(
    ...
    steps = list(
        (
            await db.execute(
                select(WorkflowStep)
                .where(WorkflowStep.definition_id == definition_id, WorkflowStep.is_active.is_(True))
    ...
    for s in steps:
        db.add(
            WorkflowInstanceStep(
                ...
                step_id=s.id,
                step_order=s.step_order,
                status="pending" if s.step_order > 1 else "awaiting_action",
            )
        )
    await db.flush()
    return inst
...
async def approve_step(
    db: AsyncSession, instance_id: uuid.UUID, user_id: uuid.UUID, user_role: str, notes: str = ""
) -> dict[str, Any]:
    ...
    steps = list(
        (
            await db.execute(
                select(WorkflowStep)
                .where(WorkflowStep.definition_id == inst.definition_id, WorkflowStep.is_active.is_(True))
                .order_by(WorkflowStep.step_order)
            )
        )
        .scalars()
    ...
    if result["action"] == "advance":
        ...
        nist = (
            await db.execute(
                select(WorkflowInstanceStep).where(
                    WorkflowInstanceStep.instance_id == instance_id,
                    WorkflowInstanceStep.step_order == result["next_step_order"],
                )
            )
        ).scalar_one_or_none()
        if nist:
    ...
    current_step = (
        await db.execute(
            select(WorkflowStep).where(
                WorkflowStep.definition_id == inst.definition_id, WorkflowStep.step_order == inst.current_step_order
            )
        )
    ).scalar_one_or_none()
    if current_step:
...
async def override_workflow(
    db: AsyncSession, instance_id: uuid.UUID, user_id: uuid.UUID, reason: str
) -> dict[str, Any]:
    inst = (await db.execute(select(WorkflowInstance).where(WorkflowInstance.id == instance_id))).scalar_one_or_none()
    if not inst:
        ...
        (
            await db.execute(
                select(WorkflowDefinition).where(
                    WorkflowDefinition.organization_id == org_id, WorkflowDefinition.is_active.is_(True)
                )
            )
        )
        .scalars()
...
async def list_instances(db: AsyncSession, org_id: uuid.UUID, status: str = None) -> list:
    q = (
        ...
        .where(WorkflowDefinition.organization_id == org_id, WorkflowInstance.is_active.is_(True))
    )
    if status:
...
async def list_approvals(db: AsyncSession, instance_id: uuid.UUID) -> list:
    return list(
        (await db.execute(select(WorkflowApproval).where(WorkflowApproval.instance_id == instance_id))).scalars().all()
    )


async def list_rejections(db: AsyncSession, instance_id: uuid.UUID) -> list:
    return list(
        (await db.execute(select(WorkflowRejection).where(WorkflowRejection.instance_id == instance_id)))
        .scalars()
        .all()
    )
...
async def get_workflow_status(db: AsyncSession, resource_type: str, resource_id: uuid.UUID) -> dict[str, Any]:
    ...
    inst = (
        await db.execute(
            select(WorkflowInstance)
            .where(
                WorkflowInstance.resource_type == resource_type,
                WorkflowInstance.resource_id == resource_id,
    ...
    return {
        "has_workflow": True,
        "status": inst.status,
        "current_step": inst.current_step_order,
