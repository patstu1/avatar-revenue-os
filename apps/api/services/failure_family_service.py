...
async def recompute_failure_families(
    ...
    await db.execute(
        delete(FailureFamilyMember).where(
            FailureFamilyMember.report_id.in_(
                select(FailureFamilyReport.id).where(FailureFamilyReport.brand_id == brand_id)
            )
        )
    )
    ...
    for f in families:
        report = FailureFamilyReport(
            ...
            recommended_alternative=next(
                (r.get("recommended_alternative") for r in rules if r["family_key"] == f["family_key"]), None
            ),
        ...
        for m in f.get("members", [])[:20]:
            ...
            db.add(
                FailureFamilyMember(
                    ...
                    fail_score=float(m.get("fail_score", 0)),
                    detail=m.get("detail"),
                )
            )
        ...
        for r in matching_rules:
            db.add(
                SuppressionRule(
                    ...
                    suppression_mode=r["suppression_mode"],
                    retest_after_days=r["retest_after_days"],
        ...
        items.append(
            {
                ...
                "pattern_id": str(lp.id),
                "fail_score": float(lp.fail_score),
                "family_type_val": lp.pattern_type,
                lp.pattern_type: lp.pattern_name,
            ...
            items.append(
                {
                    ...
                    "fail_score": 1.0 - qg.total_score,
                    "created_at": qg.created_at,
                    "detail": f"Quality fail: {qg.total_score:.2f}",
...
async def run_decay_check(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    rule_dicts = [
        {"family_type": r.family_type, "family_key": r.family_key, "expires_at": r.expires_at, "is_active": r.is_active}
    ...
    return list(
        (
            await db.execute(
                ...
                .where(SuppressionEvent.brand_id == brand_id, SuppressionEvent.is_active.is_(True))
                .limit(50)
            )
        )
        .scalars()
...
async def get_active_suppressions_for_downstream(db: AsyncSession, brand_id: uuid.UUID) -> list[dict[str, Any]]:
    ...
    return [
        {"family_type": r.family_type, "family_key": r.family_key, "mode": r.suppression_mode, "reason": r.reason}
        for r in rules
    ]
