...
async def update_all_autonomy_grants(db: AsyncSession) -> dict:
    ...
    brands = (await db.execute(select(Brand.id).where(Brand.is_active.is_(True)))).scalars().all()
        ...
        stats = (
            await db.execute(
                select(
                    ...
                    func.count(OperatorAction.id).filter(OperatorAction.outcome_score > 0).label("succeeded"),
                )
                .where(
