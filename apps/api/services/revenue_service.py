...
async def recompute_revenue_intel(
    ...
    offer_dicts = [
        {
            ...
            "monetization_method": o.monetization_method.value
    ...
    seg_dicts = [
        {"name": s.name, "estimated_size": s.estimated_size, "segment_criteria": s.segment_criteria} for s in segments
    ]
    ...
    await db.execute(
        delete(MonetizationDecision).where(
        ...
        if stacks:
            ...
            db.add(
                MonetizationRecommendation(
                    ...
                    expected_cost=0.0,
                    confidence=min(0.9, best["segment_fit_multiplier"] * 0.6),
            ...
            db.add(
                MonetizationDecision(
                    brand_id=brand_id,
                    decision_type=DecisionType.MONETIZATION,
                    ...
                    conversion_rate_estimate=float(offer_dicts[0].get("conversion_rate", 0.02))
                    if offer_dicts
                    else 0.02,
                    ...
                    if best["combined_expected_revenue"] > 5
                    else RecommendedAction.EXPERIMENT,
    ...
    for cr in content_rollups:
        ...
        avg_val_row = (
            await db.execute(
                select(func.coalesce(func.avg(AttributionEvent.event_value), 10.0)).where(
        ...
        paths.append(
            {
                ...
                "stages": cr["event_types"],
                "total_clicks": cr["clicks"],
        ...
        if fr["expected_recoverable_revenue"] > 0:
            db.add(
                MonetizationRecommendation(
                    ...
                    expected_cost=0.0,
                    confidence=min(0.8, fr["efficiency_vs_brand_avg"]),
    ...
    opt_in_count = (
        await db.execute(
            ...
            .where(AttributionEvent.brand_id == brand_id, AttributionEvent.event_type == "opt_in")
        )
    ).scalar() or 0
    membership_count = (
        await db.execute(
            ...
            .where(
                ...
                AttributionEvent.event_type.in_(["purchase", "affiliate_conversion"]),
            )
        )
    ).scalar() or 0
    repeat_count = max(
        ...
        - len(
            set(
                ...
                for r in (
                    await db.execute(
                        select(AttributionEvent.tracking_id)
                        .where(AttributionEvent.brand_id == brand_id, AttributionEvent.event_type == "purchase")
                        .distinct()
                    )
                ).all()
            )
    ...
    for action in owned.get("recommended_actions", []):
        db.add(
            MonetizationRecommendation(
                brand_id=brand_id,
                recommendation_type="owned_audience",
    ...
    winner_dicts = [
        {"title": w.title, "win_score": w.win_score, "content_id": w.content_id} for w in winner_signals if w.is_winner
    ]

    purchase_signal_count = (
        await db.execute(
            select(func.count())
            .select_from(CommentCashSignal)
            .where(CommentCashSignal.brand_id == brand_id, CommentCashSignal.signal_type == "purchase_intent_cluster")
        )
    ).scalar() or 0

    prod_recs = recommend_productization(
        winner_dicts, seg_dicts, offer_dicts, purchase_signal_count, total_rev, subscriber_count
    )
    for pr in prod_recs:
        db.add(
            MonetizationRecommendation(
                ...
                title=pr["title"][:500],
                description=f"Type: {pr['product_type']}, price: ${pr['price_point']:.0f}",
                expected_revenue_uplift=pr["expected_revenue"],
                expected_cost=pr["expected_cost"],
        ...
        if density["recommended_additions"]:
            db.add(
                MonetizationRecommendation(
                    ...
                    expected_revenue_uplift=sum(
                        a["expected_revenue_uplift_pct"] for a in density["recommended_additions"]
                    )
                    * cr["revenue"]
                    / 100
                    if cr["revenue"] > 0
...
async def get_offer_stacks(db: AsyncSession, brand_id: uuid.UUID) -> list[dict]:
    rows = list(
        (
            await db.execute(
                select(MonetizationRecommendation)
                .where(
