...
async def get_service_pipeline(db: AsyncSession, brand_id: uuid.UUID) -> dict:
    ...
    for row in stage_q.all():
        by_stage[str(row[0])] = {"count": row[1], "value": float(row[2] or 0), "weighted_value": float(row[3] or 0)}
    ...
    win_rate = (
        won.get("count", 0) / (won.get("count", 0) + lost.get("count", 0))
        if (won.get("count", 0) + lost.get("count", 0)) > 0
        else 0
    )
    ...
    return {
        ...
        "stalled_deals": [
            {"id": str(d.id), "customer": d.customer_name, "stage": d.stage, "value": float(d.deal_value or 0)}
            for d in stalled
        ],
    ...
    for lead in leads:
        if lead["qualification"] == "hot":
            a = await emit_action(
                ...
                priority="high",
                brand_id=brand_id,
                source_module="service_sales_engine",
                entity_type="high_ticket_deal",
                entity_id=uuid.UUID(lead["deal_id"]),
            )
