...
def rank_offer(offer: dict[str, Any]) -> float:
    ...
    score = (0.25 * epc + 0.15 * cvr + 0.10 * commission + 0.10 * refund_risk
             + 0.10 * trust + 0.10 * content_fit + 0.10 * platform_fit + 0.10 * audience_fit)
...
def build_affiliate_link(offer: dict[str, Any], content_item_id: str = "", campaign_id: str = "", platform: str = "") -> dict[str, Any]:
...
def detect_leaks(offers: list[dict[str, Any]], links: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for link in links:
        ...
        if clicks > 50 and conversions == 0:
            leaks.append({"leak_type": "high_clicks_zero_conversions", "severity": "critical", "revenue_loss_estimate": clicks * 0.5, "recommendation": "Check landing page, offer status, and tracking integrity", "link_id": link.get("id"), "offer_id": link.get("offer_id")})
        elif clicks > 20 and conversions > 0 and (conversions / clicks) < 0.005:
            leaks.append({"leak_type": "very_low_conversion", "severity": "high", "revenue_loss_estimate": clicks * 0.3, "recommendation": "Test different landing page or offer angle", "link_id": link.get("id"), "offer_id": link.get("offer_id")})

    for offer in offers:
        if offer.get("is_active") and offer.get("blocker_state"):
            leaks.append({"leak_type": "blocked_offer_still_active", "severity": "high", "revenue_loss_estimate": 10, "recommendation": "Deactivate or fix blocker", "offer_id": offer.get("id")})
        if float(offer.get("refund_rate", 0) or 0) > 0.15:
            leaks.append({"leak_type": "high_refund_rate", "severity": "medium", "revenue_loss_estimate": float(offer.get("epc", 0) or 0) * 20, "recommendation": "Switch to lower-refund merchant or product", "offer_id": offer.get("id")})
...
def detect_blockers(offers: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for o in offers:
        if not o.get("affiliate_url") and not o.get("destination_url"):
            blockers.append({"offer_id": o.get("id"), "blocker_type": "no_destination_url", "description": f"Offer '{o.get('product_name', '')}' has no affiliate or destination URL", "severity": "critical"})
        if float(o.get("epc", 0) or 0) == 0 and float(o.get("commission_rate", 0) or 0) == 0:
            blockers.append({"offer_id": o.get("id"), "blocker_type": "no_commission_data", "description": f"Offer '{o.get('product_name', '')}' has no EPC or commission rate", "severity": "high"})
    return blockers
