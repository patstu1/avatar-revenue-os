...
async def org_with_signals(db_session: AsyncSession):
    ...
    db_session.add(EnterpriseConnector(organization_id=org.id, connector_name="Salesforce CRM", connector_type="crm", endpoint_url="https://sf.example.com", credential_env_key="SF_KEY"))
    db_session.add(EnterpriseConnector(organization_id=org.id, connector_name="Broken Connector", connector_type="erp"))

    db_session.add(SocialListeningEvent(organization_id=org.id, brand_id=brand.id, signal_type="brand_mention", raw_text="Love this product so much!", sentiment=0.8, relevance_score=0.9))
    db_session.add(SocialListeningEvent(organization_id=org.id, brand_id=brand.id, signal_type="demand_signal", raw_text="Need a feature for bulk scheduling", sentiment=0.3, relevance_score=0.8))
    db_session.add(SocialListeningEvent(organization_id=org.id, brand_id=brand.id, signal_type="demand_signal", raw_text="When will you add analytics?", sentiment=0.2, relevance_score=0.7))
    db_session.add(CompetitorSignalEvent(organization_id=org.id, brand_id=brand.id, competitor_name="Rival Co", signal_type="competitor_mention", raw_text="Disappointed with Rival, looking for alternative", sentiment=-0.6))
...
async def test_clusters_created(db_session, org_with_signals):
    org, _ = org_with_signals
    await recompute_listening(db_session, org.id); await db_session.commit()
    clusters = (await db_session.execute(select(ListeningCluster).where(ListeningCluster.organization_id == org.id))).scalars().all()
...
async def test_blockers_detected(db_session, org_with_signals):
    org, _ = org_with_signals
    await recompute_listening(db_session, org.id); await db_session.commit()
    blockers = (await db_session.execute(select(IntegrationBlocker).where(IntegrationBlocker.organization_id == org.id))).scalars().all()
...
async def test_response_recs_created(db_session, org_with_signals):
    org, _ = org_with_signals
    await recompute_listening(db_session, org.id); await db_session.commit()
    recs = (await db_session.execute(select(SignalResponseRecommendation).where(SignalResponseRecommendation.organization_id == org.id))).scalars().all()
    assert len(recs) >= 2
...
async def test_idempotent(db_session, org_with_signals):
    org, _ = org_with_signals
    r1 = await recompute_listening(db_session, org.id); await db_session.commit()
    r2 = await recompute_listening(db_session, org.id); await db_session.commit()
