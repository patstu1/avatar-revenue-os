...
@shared_task(base=TrackedTask, name="workers.health_monitor_worker.tasks.check_dependencies")
def check_dependencies() -> dict:
    """Hourly dependency truth check — surfaces missing credentials and unhealthy workers.

    Emits critical events for any missing dependencies so the alerting system
    and operator dashboard can surface them.
    """
    import asyncio

    async def _run():
        from packages.db.session import async_session_factory
        from apps.api.services.dependency_checker import check_runtime_dependencies
        from apps.api.services.event_bus import emit_event

        async with async_session_factory() as db:
            result = await check_runtime_dependencies(db)

            if result["blockers"]:
                await emit_event(
                    db, domain="health", event_type="dependency.blockers_found",
                    summary=f"Dependency check: {result['status'].upper()} — {len(result['blockers'])} blockers: {', '.join(result['blockers'][:5])}",
