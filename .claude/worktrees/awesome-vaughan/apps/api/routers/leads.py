...
async def capture_lead(body: LeadCaptureRequest, request: Request, db: DBSession):
    ...
    # ── Route lead into outreach pipeline (non-fatal) ──
    routing_result = None
    try:
        from apps.api.services.lead_routing_service import route_lead_to_outreach
        routing_result = await route_lead_to_outreach(
            db, lead=lead, brand=brand,
            lead_email=body.email, lead_name=body.name,
            offer_slug=body.offer_slug,
        )
        await db.commit()
    except Exception:
        pass  # Lead captured successfully; outreach routing failure is non-fatal
