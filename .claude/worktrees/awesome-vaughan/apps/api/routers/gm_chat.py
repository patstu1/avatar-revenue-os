...
async def approve_blueprint(current_user: CurrentUser, db: DBSession):
    ...
    # ── Dependency check — block if critical prerequisites missing ──
    from apps.api.services.dependency_checker import check_blueprint_readiness
    readiness = await check_blueprint_readiness(db)
    if not readiness["ready"]:
        raise HTTPException(
            status_code=422,
            detail=f"Cannot execute blueprint — missing dependencies: {'; '.join(readiness['blockers'])}",
        )
    ...
    try:
        ...
        await emit_event(
            db, domain="orchestration", event_type="blueprint.approved",
            summary=f"Blueprint approved, dispatching auto-execution",
            org_id=current_user.organization_id,
            entity_type="gm_blueprint", entity_id=bp.id,
            previous_state="proposed", new_state="approved",
        )
    except Exception:
        pass
    ...
    # ── Dispatch auto-execution worker ──
    try:
        from workers.blueprint_executor_worker.tasks import execute_approved_blueprint
        execute_approved_blueprint.delay(str(bp.id))
        logger.info("blueprint.execution_dispatched", blueprint_id=str(bp.id))
    except Exception as e:
        logger.warning("blueprint.dispatch_failed", error=str(e))
