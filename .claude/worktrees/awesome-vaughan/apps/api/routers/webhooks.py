    ...
    *,
    ...
    if event_type == "checkout.session.completed" and brand_id and meta.get("source") == "outreach_proposal":
        ...
        if amount > 0:
            ...
            # ── Bridge: Payment → Fulfillment ──
            try:
                from apps.api.services.fulfillment_service import initiate_fulfillment
                customer_email = obj.get("customer_email", "") or meta.get("customer_email", "")
                customer_name = meta.get("customer_name", "")
                await initiate_fulfillment(db, entry, customer_email=customer_email, customer_name=customer_name)
                await db.flush()
            except Exception as ful_err:
                logger.warning("stripe.fulfillment_failed", error=str(ful_err))
