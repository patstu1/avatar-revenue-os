...
async def _setup_brand(api_client, sample_org_data) -> tuple[dict, str]:
    ...
    brand_resp = await api_client.post("/api/v1/brands/", json={
        "name": "Avatar Test Brand", "slug": "avatar-test",
    }, headers=headers)
...
async def test_create_avatar(api_client, sample_org_data):
    ...
    response = await api_client.post("/api/v1/avatars/", json={
        "brand_id": brand_id, "name": "Test Avatar",
        "persona_description": "A friendly educator",
        "voice_style": "warm", "visual_style": "professional",
    }, headers=headers)
...
async def test_list_avatars(api_client, sample_org_data):
    headers, brand_id = await _setup_brand(api_client, sample_org_data)
    await api_client.post("/api/v1/avatars/", json={
        "brand_id": brand_id, "name": "Avatar 1",
    }, headers=headers)
    await api_client.post("/api/v1/avatars/", json={
        "brand_id": brand_id, "name": "Avatar 2",
    }, headers=headers)
...
async def test_delete_avatar(api_client, sample_org_data):
    headers, brand_id = await _setup_brand(api_client, sample_org_data)
    create_resp = await api_client.post("/api/v1/avatars/", json={
        "brand_id": brand_id, "name": "To Delete",
    }, headers=headers)
...
async def test_avatar_provider_profile_crud(api_client, sample_org_data):
    headers, brand_id = await _setup_brand(api_client, sample_org_data)
    avatar_resp = await api_client.post("/api/v1/avatars/", json={
        "brand_id": brand_id, "name": "Provider Test Avatar",
    }, headers=headers)
    avatar_id = avatar_resp.json()["id"]

    # Create avatar provider profile (Tavus)
    response = await api_client.post("/api/v1/providers/avatar", json={
        "avatar_id": avatar_id, "provider": "tavus",
        "is_primary": True, "cost_per_minute": 0.50,
    ...
    # Update provider
    update_resp = await api_client.patch(f"/api/v1/providers/avatar/{profile['id']}", json={
...
async def test_voice_provider_profile_crud(api_client, sample_org_data):
    headers, brand_id = await _setup_brand(api_client, sample_org_data)
    avatar_resp = await api_client.post("/api/v1/avatars/", json={
        "brand_id": brand_id, "name": "Voice Test Avatar",
    }, headers=headers)
    avatar_id = avatar_resp.json()["id"]

    # Create voice provider profile (ElevenLabs)
    response = await api_client.post("/api/v1/providers/voice", json={
        "avatar_id": avatar_id, "provider": "elevenlabs",
        "is_primary": True, "cost_per_minute": 0.30,
    ...
    fallback_resp = await api_client.post("/api/v1/providers/voice", json={
        "avatar_id": avatar_id, "provider": "openai_realtime",
        "is_fallback": True, "cost_per_minute": 0.12,
...
async def test_unsupported_provider_rejected(api_client, sample_org_data):
    headers, brand_id = await _setup_brand(api_client, sample_org_data)
    avatar_resp = await api_client.post("/api/v1/avatars/", json={
        "brand_id": brand_id, "name": "Bad Provider Avatar",
    }, headers=headers)
    avatar_id = avatar_resp.json()["id"]

    response = await api_client.post("/api/v1/providers/avatar", json={
        "avatar_id": avatar_id, "provider": "nonexistent_provider",
    }, headers=headers)
...
async def test_avatar_creation_creates_audit_log(api_client, sample_org_data):
    headers, brand_id = await _setup_brand(api_client, sample_org_data)
    await api_client.post("/api/v1/avatars/", json={
        "brand_id": brand_id, "name": "Audit Avatar",
    }, headers=headers)
