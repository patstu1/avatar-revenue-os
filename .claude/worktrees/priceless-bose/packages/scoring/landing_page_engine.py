...
def generate_page(offer: dict[str, Any], audience: str = "", platform: str = "", page_type: str = "product") -> dict[str, Any]:
    ...
    proof = [{"type": "testimonial", "text": f"Real results with {name}"}, {"type": "data", "text": f"Proven conversion data for {name}"}]
    ...
    disclosure = [{"type": "affiliate_disclosure", "text": "This page contains affiliate links. We may earn a commission."}] if method == "affiliate" else []
...
def score_page_quality(page: dict[str, Any], objection_count: int = 0, offer_cvr: float = 0) -> dict[str, Any]:
    ...
    trust = 0.3
    if has_disclosure: trust += 0.3
    if has_proof: trust += 0.2
    if has_objection_blocks: trust += 0.2

    conversion_fit = 0.3
    if has_cta: conversion_fit += 0.3
    if has_headline: conversion_fit += 0.2
    if offer_cvr > 0.03: conversion_fit += 0.2
    ...
    return {"total_score": total, "trust_score": round(trust, 3), "conversion_fit": round(conversion_fit, 3), "objection_coverage": round(obj_cov, 3), "verdict": verdict}


def _headline_for_type(pt: str, name: str) -> str:
    m = {"product": f"The Complete Guide to {name}", "review": f"Honest {name} Review — Is It Worth It?", "comparison": f"{name} vs The Competition", "advertorial": f"How {name} Changed Everything", "presell": f"Before You Buy {name}, Read This", "optin": f"Get Free Access to {name}", "lead_magnet": f"Download the {name} Playbook", "quiz_funnel": f"Find Your Perfect {name} Match", "authority": f"The Definitive {name} Resource", "creator_revenue": f"Work With Us — {name}", "sponsor": f"Partner With {name}"}
...
def _hook_angle(pt: str, method: str) -> str:
    m = {"product": "value_demonstration", "review": "honest_assessment", "comparison": "side_by_side", "advertorial": "story_driven", "presell": "objection_preempt", "optin": "free_value", "lead_magnet": "lead_capture", "quiz_funnel": "personalization", "authority": "expertise", "creator_revenue": "partnership", "sponsor": "brand_fit"}
