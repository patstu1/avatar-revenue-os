...
async def recompute_campaigns(db: AsyncSession, brand_id: uuid.UUID) -> dict[str, Any]:
    ...
    await db.execute(delete(CampaignDestination).where(CampaignDestination.campaign_id.in_(select(Campaign.id).where(Campaign.brand_id == brand_id))))
    await db.execute(delete(CampaignAsset).where(CampaignAsset.campaign_id.in_(select(Campaign.id).where(Campaign.brand_id == brand_id))))
    await db.execute(delete(CampaignVariant).where(CampaignVariant.campaign_id.in_(select(Campaign.id).where(Campaign.brand_id == brand_id))))
    ...
    suppressed = list((await db.execute(select(SuppressionRule.family_key).where(SuppressionRule.brand_id == brand_id, SuppressionRule.is_active.is_(True), SuppressionRule.family_type == "hook_type"))).scalars().all())
    prov_blockers = list((await db.execute(select(ProviderBlocker).where(ProviderBlocker.brand_id == brand_id, ProviderBlocker.is_active.is_(True)))).scalars().all())
    system_state = {"suppressed_families": suppressed, "provider_blockers": [{"key": b.provider_key} for b in prov_blockers]}

    brand_dict = {"niche": brand.niche if brand else "general"}
    acct_dicts = [{"id": str(a.id), "platform": a.platform.value if hasattr(a.platform, "value") else str(a.platform)} for a in accounts]
    ...
    for offer in offers:
        ...
        lp = (await db.execute(select(LandingPage).where(LandingPage.brand_id == brand_id, LandingPage.offer_id == offer.id, LandingPage.is_active.is_(True)).limit(1))).scalar_one_or_none()
        ...
        for ct in ("affiliate", "product_conversion"):
            ...
            camp = Campaign(brand_id=brand_id, offer_id=offer.id, landing_page_id=lp.id if lp else None, **{k: v for k, v in spec.items() if k not in ("landing_page_id",)})
            db.add(camp)
            await db.flush()
            ...
            if lp:
                db.add(CampaignDestination(campaign_id=camp.id, landing_page_id=lp.id, destination_url=lp.destination_url, destination_type="landing_page"))
...
async def list_campaigns(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list((await db.execute(select(Campaign).where(Campaign.brand_id == brand_id, Campaign.is_active.is_(True)).order_by(Campaign.created_at.desc()))).scalars().all())

async def list_campaign_variants(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list((await db.execute(select(CampaignVariant).join(Campaign).where(Campaign.brand_id == brand_id, CampaignVariant.is_active.is_(True)))).scalars().all())

async def list_campaign_blockers(db: AsyncSession, brand_id: uuid.UUID) -> list:
    return list((await db.execute(select(CampaignBlocker).where(CampaignBlocker.brand_id == brand_id, CampaignBlocker.is_active.is_(True)))).scalars().all())
...
async def get_campaign_for_content(db: AsyncSession, brand_id: uuid.UUID, offer_id: uuid.UUID = None) -> dict[str, Any]:
    ...
    return {"campaign_id": str(camp.id), "campaign_type": camp.campaign_type, "name": camp.campaign_name, "truth_label": camp.truth_label, "launch_status": camp.launch_status}
