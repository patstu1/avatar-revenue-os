...
# ── Inactive Shell Detection ──────────────────────────────────────────


@shared_task(base=TrackedTask, name="workers.health_monitor_worker.tasks.detect_inactive_shells")
def detect_inactive_shells() -> dict:
    """Detect brands and accounts that appear active but can't publish or monetize.

    Flags:
    - Brands with is_active=True but zero publishable accounts
    - Brands with is_active=True but zero active offers
    - Accounts with is_active=True but expired/missing credentials or suspended health
    - Creates operator actions for each shell detected
    - Excludes shells from runtime targeting by marking them
    """
    from sqlalchemy.orm import Session
    ...
    with Session(engine) as session:
        ...
        for org in orgs:
            brands = session.execute(
                select(Brand).where(Brand.organization_id == org.id, Brand.is_active.is_(True))
            ).scalars().all()

            for brand in brands:
                # Check 1: Does this brand have any publishable accounts?
                publishable_accounts = session.execute(
                    select(func.count()).select_from(CreatorAccount).where(
                        CreatorAccount.brand_id == brand.id,
                        CreatorAccount.is_active.is_(True),
                        CreatorAccount.credential_status != "missing",
                        CreatorAccount.credential_status != "expired",
                        CreatorAccount.account_health != HealthStatus.SUSPENDED,
                    )
                ).scalar() or 0

                # Check 2: Does this brand have any active offers?
                ...
                if publishable_accounts == 0:
                    is_shell = True
                    reasons.append("no_publishable_accounts")
                ...
                if is_shell:
                    shell_brands.append({
                        ...
                        "reasons": reasons,
                        "publishable_accounts": publishable_accounts,
                    ...
                    session.add(SystemEvent(
                        ...
                        brand_id=brand.id,
                        event_type="shell.brand_detected",
                        entity_type="brand",
                        entity_id=brand.id,
                        severity="warning",
                        domain="health",
                        requires_action=True,
                        summary=f"Inactive shell brand: {brand.name} — {', '.join(reasons)}",
                        details_json={
                            "reasons": reasons,
                            "publishable_accounts": publishable_accounts,
                            "active_offers": active_offers,
                            "fix_actions": [
                                "Add platform accounts with valid credentials" if "no_publishable_accounts" in reasons else None,
                                "Create at least one active offer" if "no_active_offers" in reasons else None,
                            ],
                        },
                    ))
                    brands_deactivated += 1

                # Check 3: Individual dead accounts
                ...
                for acct in all_accounts:
                    ...
                    if acct.credential_status in ("missing", "expired"):
                        dead_reasons.append(f"credentials_{acct.credential_status}")
                    if acct.account_health == HealthStatus.SUSPENDED:
                        dead_reasons.append("suspended")
                    if acct.posting_capacity_per_day == 0:
                        dead_reasons.append("zero_posting_capacity")
                    ...
                    if dead_reasons:
                        ...
                        # Mark account health as degraded if not already worse
                        if acct.account_health not in (HealthStatus.CRITICAL, HealthStatus.SUSPENDED):
                            acct.account_health = HealthStatus.DEGRADED

                        session.add(SystemEvent(
                            organization_id=org.id,
                            brand_id=brand.id,
                            event_type="shell.account_detected",
                            entity_type="creator_account",
                            entity_id=acct.id,
                            severity="warning",
                            domain="health",
                            requires_action=True,
                            summary=f"Dead account: {acct.platform_username} ({platform}) — {', '.join(dead_reasons)}",
                            details_json={
                                "reasons": dead_reasons,
                                "platform": platform,
                                "fix_actions": [
                                    "Reconnect platform credentials" if "credentials" in str(dead_reasons) else None,
                                    "Unsuspend or replace account" if "suspended" in dead_reasons else None,
                                    "Restore posting capacity" if "zero_posting_capacity" in dead_reasons else None,
                                ],
                            },
                        ))
                        accounts_flagged += 1
    ...
    logger.info("shell_detection.complete",
                shell_brands=len(shell_brands), shell_accounts=len(shell_accounts))

    return {
        "status": "completed",
        "shell_brands": len(shell_brands),
        "shell_accounts": len(shell_accounts),
        "brands_flagged": brands_deactivated,
        "accounts_flagged": accounts_flagged,
        "details": {
            "brands": shell_brands[:20],
            "accounts": shell_accounts[:50],
        },
    }
