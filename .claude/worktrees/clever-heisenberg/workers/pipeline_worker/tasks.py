...
def _compose_final_video(session, content_item, assets: list, manifest: dict, storage) -> object | None:
    """Compose a final deliverable video from individual assets.

    Strategy:
    1. If avatar video exists: it's the primary visual track (already has synced voice)
    2. If only voice narration exists: create audio-only deliverable
    3. If b-roll clips exist with avatar: concatenate using ffmpeg (if available)
       or reference as ordered playlist in metadata
    4. Persist composed output to storage and create a composed Asset record

    Falls back gracefully: if ffmpeg is unavailable, the avatar video alone
    becomes the final deliverable (most common case for short-form content).
    """
    ...
    for a in assets:
        at = a.asset_type or ""
        if at in ("avatar", "avatar_video"):
            avatar_asset = a
        elif at in ("voice", "voice_narration"):
            voice_asset = a
        elif "broll" in at or "b_roll" in at:
            broll_assets.append(a)

    if not avatar_asset and not voice_asset:
        return None

    # Primary deliverable: avatar video (already has voice synced via lip-sync)
    primary_url = avatar_asset.file_path if avatar_asset else (voice_asset.file_path if voice_asset else None)
    if not primary_url:
        return None

    # If we have b-roll and avatar, attempt ffmpeg composition
    composed_url = primary_url  # Default: avatar video IS the deliverable
    composition_method = "avatar_primary"

    if broll_assets and avatar_asset:
        try:
            import subprocess
            import tempfile
            import os

            # Download assets to temp files for ffmpeg
            concat_list = tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False)
            # Avatar is the main video, b-roll clips intercut
            concat_list.write(f"file '{avatar_asset.file_path}'\n")
            for br in broll_assets[:3]:  # Max 3 b-roll clips
                concat_list.write(f"file '{br.file_path}'\n")
            concat_list.close()
            ...
            if result.returncode == 0 and os.path.exists(output_path):
                storage_key = storage.generate_key(prefix="pipeline/composed", extension="mp4")
                composed_url = storage.upload_file(output_path, key=storage_key, content_type="video/mp4")
                composition_method = "ffmpeg_concat"
                logger.info("pipeline.ffmpeg_composed", assets=len(broll_assets) + 1)
            ...
            # ffmpeg not available or timed out — use avatar as-is
            composition_method = "avatar_primary_no_ffmpeg"
            logger.info("pipeline.ffmpeg_unavailable, using avatar as final deliverable")
        except Exception:
            composition_method = "avatar_primary_fallback"
            logger.debug("pipeline.compose_failed", exc_info=True)

    # Create composed asset record
    composed = Asset(
        brand_id=content_item.brand_id,
        content_item_id=content_item.id,
        asset_type="composed_video",
        file_path=composed_url,
        mime_type="video/mp4" if avatar_asset else "audio/mpeg",
        storage_provider=avatar_asset.storage_provider if avatar_asset else "local",
        metadata_blob={
            "composition_method": composition_method,
            "source_assets": [str(a.id) for a in ([avatar_asset] if avatar_asset else []) + ([voice_asset] if voice_asset else []) + broll_assets],
            "broll_count": len(broll_assets),
        },
    )
    session.add(composed)
    session.flush()
...
def assemble_and_finalize(self, content_item_id: str) -> dict:
    ...
    with _get_session() as session:
        ...
        # ── 5. Compose final deliverable from assets ────────────────
        composed_asset = _compose_final_video(session, content_item, assets, manifest, storage)
        if composed_asset:
            manifest["composed_asset"] = {
                "id": str(composed_asset.id),
                "file_path": composed_asset.file_path,
                "asset_type": "composed_video",
            }
            content_item.video_asset_id = composed_asset.id
            logger.info("pipeline.composed", content_item_id=content_item_id,
                        composed_url=composed_asset.file_path)

        # ── 6. Update content item to ready_to_publish ───────────────
        ...
        # ── 7. Dispatch to publishing queue ──────────────────────────
        ...
        # ── 8. Emit pipeline.content_ready event ─────────────────────
        emit_event_sync(
            session,
            domain="pipeline",
