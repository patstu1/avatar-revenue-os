...
class RLDReportOut(BaseModel):
    ...
    id: uuid.UUID; total_leaks: int; total_estimated_loss: float; critical_count: int; top_leak_type: Optional[str] = None; summary: Optional[str] = None
...
class RLDClusterOut(BaseModel):
    model_config = ConfigDict(from_attributes=True)
    id: uuid.UUID; cluster_type: str; event_count: int; total_loss: float; priority_score: float; recommended_action: Optional[str] = None
