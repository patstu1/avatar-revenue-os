...
def _build_generation_prompt(brief: ContentBrief, brand: Optional[Brand], metadata: dict) -> str:
    ...
    if wins:
        parts.append(f"\n=== MANDATORY PATTERN CONSTRAINTS (ENFORCED) ===")
        parts.append(f"REQUIRED: You MUST incorporate at least one of these proven winning patterns into the content.")
        parts.append(f"These patterns have been statistically validated to drive engagement and revenue:")
        for w in wins[:5]:
            parts.append(f"  [REQUIRED] {w.get('pattern_name', '')} (type: {w.get('pattern_type', '')}, win score: {w.get('win_score', 0):.2f})")
        parts.append(f"The output MUST clearly demonstrate use of at least one required pattern above.")

    losses = metadata.get("losing_patterns", [])
    if losses:
        parts.append(f"\n=== HARD EXCLUSIONS (DO NOT USE) ===")
        parts.append(f"BANNED: The following patterns have been proven to underperform. Do NOT use any of these.")
        parts.append(f"If any of these patterns appear in the output, the content will be rejected by QA:")
        for lp in losses[:3]:
            parts.append(f"  [BANNED] {lp.get('pattern_name', '')} (type: {lp.get('pattern_type', '')})")
        parts.append(f"Violation of any exclusion above will result in content rejection.")

    niche_wins = metadata.get("niche_winning_patterns", [])
    if niche_wins:
        parts.append(f"\nCROSS-PLATFORM NICHE INTELLIGENCE (validated across multiple accounts):")
        parts.append(f"STRONGLY PREFERRED: These patterns work across this entire niche:")
        for np in niche_wins[:3]:
            parts.append(f"  [PREFERRED] {np.get('pattern_name', '')} (type: {np.get('pattern_type', '')}, avg score: {np.get('avg_win_score', 0):.2f}, validated by {np.get('brand_count', 1)} accounts)")

    niche_losses = metadata.get("niche_losing_patterns", [])
    if niche_losses:
        parts.append(f"\nNICHE-WIDE FAILURES (BANNED across all accounts):")
        for nl in niche_losses[:2]:
            parts.append(f"  [BANNED] {nl.get('pattern_name', '')} (type: {nl.get('pattern_type', '')})")
...
async def generate_content_from_brief(db: AsyncSession, brief_id: uuid.UUID) -> dict[str, Any]:
    ...
    # --- Dispatch media generation via canonical MediaJob path (not inline) ---
    # All media generation now goes through MediaJob records for tracking,
    # crash recovery, and observability. The old inline produce_full_video()
    # path is replaced with a tracked async dispatch.
    ...
    if ct_val in ("SHORT_VIDEO", "LONG_VIDEO", "REEL", "SHORT", "STORY"):
        try:
            ...
            # Create tracked MediaJob for voice generation (first pipeline step)
            media_job = MediaJob(
                ...
                input_payload={
                    "script_text": script.full_script[:500] if script.full_script else "",
            ...
            content_item.status = "media_generating"
            logger.info("media.job_dispatched",
                        content_item_id=str(content_item.id),
                        media_job_id=media_job_id,
                        job_type="voice", tier=tier)
        except Exception:
            logger.exception("MediaJob dispatch failed for content %s", content_item.id)
    ...
    return {
        ...
        "video_produced": False,  # Now async — tracked via MediaJob
