...
async def publish_page(db: AsyncSession, page_id: uuid.UUID, publish_method: str = "manual", destination_url: str = "") -> dict[str, Any]:
    ...
    if publish_method == "manual" and not destination_url:
        # Auto-generate the public render URL instead of failing
        import os
        domain = os.environ.get("DOMAIN", os.environ.get("API_BASE_URL", ""))
        destination_url = f"{domain}/api/v1/lp/{page_id}/render" if domain else f"/api/v1/lp/{page_id}/render"
