...
async def surface_intelligence_actions(
    ...
    # 1. High-confidence scale decisions → dispatchable "promote_winning_offer" actions
    ...
    for d in scale_decisions.scalars().all():
        action = await emit_action(
            ...
            action_payload={
                "autonomy_level": "autonomous" if (d.confidence or 0) >= 0.8 else "assisted",
    ...
    # 2. Recovery decisions → dispatchable "reduce_dead_channel" actions
