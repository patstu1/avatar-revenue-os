...
async def route_task(db: AsyncSession, brand_id: uuid.UUID, task_description: str, platform: str, content_type: str = "text", is_promoted: bool = False, campaign_type: str = "organic") -> dict[str, Any]:
    ...
    try:
        ...
        if alloc.get("starved"):
            hero_override = False
            # Starved targets get downgraded to bulk tier
            allocation_tier_override = "bulk"
        # Enforce budget ceiling: if allocated_budget > 0, check spend vs budget
        allocated_budget = alloc.get("allocated_budget", 0)
        if allocated_budget > 0:
            from packages.scoring.tiered_routing_engine import check_budget_remaining
            remaining = check_budget_remaining(str(brand_id), platform)
            if remaining is not None and remaining <= 0:
                # Budget exhausted: block hero/standard, force bulk only
                hero_override = False
                allocation_tier_override = "bulk"
                allocation_blocked = True
                logger.info("capital_allocation.budget_exhausted",
                            brand_id=str(brand_id), platform=platform,
                            allocated=allocated_budget, remaining=remaining)
            elif remaining is not None and remaining < allocated_budget * 0.2:
                # Under 20% remaining: downgrade from hero to standard
                hero_override = False
                allocation_tier_override = "standard"
                logger.info("capital_allocation.budget_low",
                            brand_id=str(brand_id), platform=platform,
                            remaining=remaining)
    except Exception:
    ...
    # Apply capital allocation tier enforcement
    if allocation_tier_override:
        original_tier = result.get("quality_tier", "standard")
        tier_rank = {"hero": 3, "standard": 2, "bulk": 1}
        if tier_rank.get(allocation_tier_override, 0) < tier_rank.get(original_tier, 0):
            result["quality_tier"] = allocation_tier_override
            result["explanation"] = result.get("explanation", "") + f" [downgraded {original_tier}->{allocation_tier_override} by capital allocation]"
            if allocation_blocked:
                result["explanation"] += " [budget exhausted]"

    if hero_override and not is_promoted and not allocation_tier_override:
