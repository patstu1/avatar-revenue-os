...
async def gov_rules(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _ro(org_id, current_user, db); return await svc.list_governance_rules(db, org_id)
...
async def banned(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _ro(org_id, current_user, db); return await svc.list_banned(db, org_id)
...
async def approvals(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _ro(org_id, current_user, db); return await svc.list_approvals(db, org_id)
...
async def risk_flags(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _ro(org_id, current_user, db); return await svc.list_risk_flags(db, org_id)
...
async def partners(org_id: uuid.UUID, current_user: CurrentUser, db: DBSession):
    await _ro(org_id, current_user, db); return await svc.list_partners(db, org_id)

@router.post("/orgs/{org_id}/affiliate/governance/recompute", response_model=RecomputeSummaryOut)
async def recompute_gov(org_id: uuid.UUID, current_user: OperatorUser, db: DBSession, _rl=Depends(recompute_rate_limit)):
    await _ro(org_id, current_user, db); r = await svc.recompute_governance(db, org_id); await db.commit(); return RecomputeSummaryOut(**r)

@router.post("/orgs/{org_id}/affiliate/partners/recompute", response_model=RecomputeSummaryOut)
async def recompute_partners(org_id: uuid.UUID, current_user: OperatorUser, db: DBSession, _rl=Depends(recompute_rate_limit)):
    await _ro(org_id, current_user, db); r = await svc.recompute_partner_scores(db, org_id); await db.commit(); return RecomputeSummaryOut(**r)
