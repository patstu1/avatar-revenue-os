...
class TestScoring:
    ...
    def test_low_signal_low_score(self):
        ...
        scores = score_opportunity(signal, {"niche": "tech"})
        # Low velocity and not-new signal still gets moderate score due to
        # self-relative revenue formula; verify it stays below high-signal range
        assert scores["composite_score"] < 0.5
        assert scores["velocity_score"] < 0.1
